...
> `v3.13.0` ｜ 43 神煞 + 五派镜片 + 作用裁决 + 八字MBTI ｜ MIT ｜ 📜 [CHANGELOG](./CHANGELOG.md)
...
| 10 | 海报长图 | 综合 / 八字 / MBTI / 紫微四种单文件 HTML 长图；中式回纹与纸本层次统一视觉，正文面向小白说人话，所有正向或偏正向描述逐处绿色粗体。八字版的十神不再一句带过，罕见结构另有独立板块说明现实表现。 |
| 11 | 八字 MBTI | 把先天格局翻成你熟悉的 MBTI 语言，措辞永远是「最像」，不是测评。 |
| 12 | 流年问答直达 | 「我 2027 年适合跳槽吗」直接问，能细到月份、能多年对比。 |
| 13 | 自带质检 | 四条海报线都有体检器把关；除字段、句数与模板外，还会拦截“正向句子仍是普通字体”的漏标。17 个回归入口一条命令跑完。 |
...
- 🧭 **顺逆双轴 + 助长焦点**——运势不再一锅烩吉凶：「事情推不推得动」和「你的状态接不接得住」分开讲；顺风标兑现与扩张，逆风/体弱也会标出补结构、练边界、恢复承载等助长重点。
- ⚖️ **格局相神裁决 + 旺衰七项账本**——身旺但伤官/七杀特别重的盘不再被「身旺忌印」一刀切；旺衰加入可追溯的耗方群势与冲根修正。韦千里 22 例目前为 17 个扶抑方向完全对齐、4 个部分一致，另 1 个寒湿例按调候/扶抑双轴分列，对照与审计随包可查。
...
- 🙋 **小白模式**——全盘分析先问「通俗易懂的结果，还是专业细节更完整的详解」；选通俗就把流派藏起来，用白话先给结果。
- 📋 **详解状态说清楚**——首回合的 11 章是可展开目录，不冒充已经生成的 11 章全文；同时要海报时会分别说明「海报已生成 / 对话详解当前为总览」。
- 💬 **随处可问 AI**——聊天收尾和四类海报都提示：对任何一句有疑惑或想进一步了解，可以直接把原句或海报内容发给 AI。
- 🤝 **对象画像不锁死年龄**——传统年长/年轻/同龄信号优先翻译为成熟承接、细腻鲜活、平等同频的相处气质，不拿身份证年龄硬卡对象。
- 🟢 **正向语义全标绿**——四类海报不再只挑一两句做绿色重点；优势、好性格、适配方向、贵人助力、顺风窗口、改善与收益结果逐处绿色粗体，正负混合句拆开着色，漏标会被体检器直接打回。
- 🔎 **十神与罕见现象说透**——十神白话解读升级为至少 6 句、200 字的完整段落，落到能力、决策、协作与工作场景；算法识别出的罕见结构另设板块，逐项标明罕见程度、偏正向／偏提醒／两面性、现实表现和可执行建议，没有罕象时整块隐藏。
- 🛡️ **质量与安全**——四条海报线都有体检器把关 + 随包金标样例、22 例冻结快照和 5000 盘分布门进回归；移除了 2.7 的启动自动更新（不再有远程代码执行面）；发版脚本成为版本号唯一入口。
