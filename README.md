...
- [写给普通用户：这是什么、能帮你做什么](#-写给普通用户这是什么能帮你做什么)
...
**一句话：装上它，你的 AI 就变成一位「排盘绝不出错、说话有出处、不吓唬人」的命理师。**

你只需要告诉 AI 三件事——出生日期、出生时刻、性别（例如「我是 2000 年 1 月 1 日中午 12 点出生的，男，帮我算算」），剩下的它来。完全不懂八字也没关系：它认得出新手，会改用白话引导你，只问一个问题就开工。
...
- **排盘从不交给 AI**。直接让大模型算八字，日柱大运经常排错——盘一错，后面全是错的。这里排盘由程序精确计算（实测对照过专业排盘软件），AI 只负责解读。
- **说法有出处**。解读依据《穷通宝鉴》《滴天髓》等典籍原文，查不到出处的神煞会老实标「待核」，绝不现编。
- **不搞恐吓**。每次开头先讲成长心态，全程不说「必然」「注定」「大凶」这类宿命话——命理在这里是了解自己的工具，不是吓自己的理由。
- **出生时间拿不准也有办法**。如果你的出生时刻离时辰交界很近，它会主动停下来跟你核对（比如用过去几年的大事年份帮你判断是哪个时辰），而不是硬着头皮解错盘。
...
精准排盘四柱十神大运（含胎元/命宫）、紫微十二宫四化大限、格局旺衰调候（穷通宝鉴 120 格）、合冲刑害作用裁决、运岁引动等补层；支持八字 / 紫微 / 综合印证，另有流年问答直达通道；长文与 HTML 长图海报（综合印证 / 八字独立 / MBTI / 紫微独立四种主题）两种形态输出。
