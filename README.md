...
> **版本 `v1.1.0`** ｜ 神煞 + 流派镜片 · 超级魔改版 ｜ 协议 MIT ｜ 📜 [更新日志 CHANGELOG](./CHANGELOG.md)

> 一个**精准排盘 + 多流派解读**的命理 AI 基板（base board）。
> 核心理念一句话：**排盘交给算法，解读交给大模型，绝不让 AI 自己瞎排盘。**
...
普通大模型直接「帮我算八字」，常会把日柱、大运排错——一步错、满盘垮。
本基板用一套内置算法层（Yiqi core + enrich 补层 + 神煞引擎）把四柱 / 十神 / 大运 / 神煞**算准**，再把这份「命盘文本」喂给大模型只做**解读**。
...
| 3 | **成长心态声明置顶** | 每次八字分析开头先讲「为什么会越算越不好」+「以成长心态看命」，反焦虑、反宿命。 |
| 4 | **总领速览 + 按需下钻** | 第一回合先给一页全局速览（含章号菜单），你说「详细展开第 6 章」才深写那一章——不一次性灌长文、不诱导你继续算。 |
| 5 | **文献核验 + 防编造铁律** | 起法有分歧/查不到的，宁可标「⚠起法待核」也**绝不编**。文昌/福星还做**古法交叉校验**（通行版命中时，再核一遍《三命通会》古法，没有就提示「古法无」）。 |
| 6 | **自带回归测试** | `schema-check`（配置自检）+ 9 例神煞 fixtures，改完一键验证不跑偏。 |
...
**3) 把 `chart.txt` ＋ 提示词喂给大模型**：打开 `prompts/bazi-prompt.md`，连同 `chart.txt` 一起发给模型，它就会按「成长心态声明 → 五行 → 十神 → 神煞 → 大运 → 六维事项」输出解读。
...
把整个 `bazi-ziwei-v2/` 文件夹放进 Claude 的 skills 目录（Claude Code / Cowork）。之后直接说「我是 1990 年 6 月 15 日下午 2 点半出生的男生，看八字」，skill 会自动跑算法层、选流派、出解读，无需手动敲命令。

---

## 🟢 在 ChatGPT 上安装（做成一个 Custom GPT）

> 需要 ChatGPT **Plus / Team / Enterprise**。ChatGPT 不能在云端跑排盘脚本，所以**排盘在你电脑上跑（上面通用三步的 1–2 步）**，把 `chart.txt` 贴进对话即可。

1. 左下角点头像 → **My GPTs** → **Create a GPT** → 切到 **Configure** 标签。
2. **Name**：八字命理助手（v2）。
3. **Instructions**：把 `prompts/bazi-prompt.md` 全文粘进去；再把 `prompts/disclaimer-preamble.md`、`prompts/output-mode-B.md` 两份内容附在后面（让它每次先出成长心态声明、走总领速览）。
4. **Knowledge** → **Upload files**：上传 `prompts/` 里的提示词、`calculator/lineages.json`、`calculator/shensha.json`，以及一两份示例 `chart.txt`（最多 20 个文件）。
5. 右侧 **Preview** 里贴一份 `chart.txt` 测一下 → 满意后点右上 **Create / Update**，选「仅自己」或「有链接可用」。
6. 以后每次：电脑上跑排盘得到 `chart.txt` → 丢给这个 GPT。

📎 官方说明：[Creating and editing GPTs](https://help.openai.com/en/articles/8554397-creating-and-editing-gpts)

---

## 🔵 在 Gemini 上安装（做成一个 Gem）

> 用 Google **Gemini** 的「Gems」自定义助手。同样：排盘在本地跑，`chart.txt` 喂进去。

1. 打开 Gemini → 左侧 **Gems / Gem manager** → **New Gem**。
2. **Instructions**：粘贴 `prompts/bazi-prompt.md`（＋ disclaimer ＋ output-mode-B 两份内容）。可点魔法棒让 Gemini 帮你润色扩写。
3. **Knowledge** → **Add files**：上传 `prompts/`、`lineages.json`、`shensha.json`、示例 `chart.txt`（每个 Gem 最多 10 个文件，支持 PDF/Docs/TXT 等）。
4. 右侧 **Preview** 测试 → 点 **Save**（注意：只预览不点保存不会存）。
5. 以后：本地排盘出 `chart.txt` → 在这个 Gem 里发给它。

📎 官方说明：[Tips for creating custom Gems](https://support.google.com/gemini/answer/15235603)

---

## 🟠 在 DeepSeek 上使用

> DeepSeek 网页版没有「自定义助手」入口，但支持**文件上传**与**系统提示**。用法是每次把提示词 + 命盘一起给它。

1. 本地跑通用三步，拿到 `chart.txt`。
2. 打开 [chat.deepseek.com](https://chat.deepseek.com)，新建对话。
3. 第一条消息：**先粘贴 `prompts/bazi-prompt.md` 全文**（＋ disclaimer ＋ output-mode-B），再**上传或粘贴 `chart.txt`**，并说明「按上面的提示词解读这份命盘，先出成长心态声明和总领速览」。
4. 想换流派：重排一次（改 `--lineage`）拿新的 `chart.txt` 再发。
5. 小技巧：把这套提示词存进浏览器的「常用语 / 收藏」，或用 DeepSeek 提示词类插件，免得每次复制。

📎 模型/系统提示参考：[DeepSeek API Docs](https://api-docs.deepseek.com/)

---

## 🟡 在 Kimi 上使用

> Kimi（Moonshot）长上下文、支持一次传多份文件，很适合贴长提示词 + 命盘。同样在本地排盘。

1. 本地跑通用三步，拿到 `chart.txt`。
2. 打开 [kimi.com](https://www.kimi.com)，新建对话。
3. **上传文件**：把 `prompts/bazi-prompt.md`、`disclaimer-preamble.md`、`output-mode-B.md` 和 `chart.txt` 一起拖进去（Kimi 支持一次多份 PDF/Docs/TXT）。
4. 发一句：「严格按 bazi-prompt 的提示词与流派镜片，解读 chart.txt 这份命盘；开头先出成长心态声明，再走总领速览。」
5. 换流派同样是重排 `chart.txt` 再传。

📎 使用指南：[How to use Kimi](https://kimi-ai.chat/guide/how-to-use-kimi-ai/)
...
> 流派是**解读层镜片**：换流派只改「展开哪些神煞、用神看哪派」，**四柱/十神/大运/神煞命中本身永不变**。
...
- 本版经核验后，`needs_review` 已全部消化（红艳对齐三命通会原文、国印纳入、元辰实现、三奇启用严格顺布、魁罡戊辰/羊刃阴干刃按文献默认关闭）。详见 `CHANGES-v2.md`。
...
│   ├── bazi-prompt.md            # 八字解读（流派+神煞+disclaimer+模式B）
