...
description: 八字 + 紫微斗数 AI 排盘与综合分析。当用户提供生辰（阳历/农历日期、时辰、性别）询问八字、紫微、命理、命盘、流年大运相关问题时使用。基于 Yiqi 算法库 + enrichBazi 补全层精准排盘（杜绝 LLM 自行排盘出错），支持按需独立分析八字 / 独立分析紫微 / 两盘交叉印证。八字解读支持【流派镜片】（子平/滴天髓/神峰/盲派/段氏/不限）与【神煞】补层，开头加【成长心态声明】，采用【总领速览+按需下钻】输出。全盘分析开场先让用户选择“通俗易懂的结果”或“专业细节更完整的详解”；完全不懂八字的用户可走极简引导。
---
...
> **🐣 例外·小白模式旁路**：用户选「通俗易懂的结果」，或消息含「不懂 / 没接触过 / 第一次 / 随便看看 / 帮我算算就行」类新手信号，或对下方菜单表现出困惑时 → **加载 `prompts/novice-mode.md`**。小白入口只问上面的阅读方式，不再用「全面了解还是具体问题」代替它；用户已经提出的事业 / 感情 / 健康等问题直接作为重点。其余缺项按默认（八字独立 + 不限流派 + 对话速览）补齐并用一句白话声明；速览用双层锚点（人话版先出、技术版收起）与重排后的章节菜单，末尾加核对点环节。
...
> 呈现形态：A. **📜 对话详解版**（默认；首回合先给总领速览与 11 章展开入口，章节正文需回复章号后逐章生成） / B. **🎴 结构化海报版**（单文件 HTML，可截图分享） / C. **两种都要**（海报本轮生成；对话详解本轮先生成总览，11 章不是已经完成的全文） / D. **🧬 八字 MBTI 海报**（用荣格八维×十神把你的盘翻译成 MBTI 语言；可主动提供实测 MBTI 解锁「实测×底盘」差异版块；可与 A/B 叠加）"
...
| 用户选 | 加载提示词 | 模板 | 输出 |
|---|---|---|---|
| 1 + A（对话详解·默认） | `prompts/disclaimer-preamble.md` ＋ `prompts/bazi-prompt.md`（注入所选流派 + `output-mode-B`） | — | Markdown（开头必出 disclaimer；首回合仅总领速览，按章下钻） |
| 1 + B（海报） | `prompts/bazi-poster.md` | `templates/report-bazi-poster.html` | `<name>-bazi.html`（`render --mode=bazi`） |
| 1 + C（两者） | 上述两份都跑 | 同上 | HTML 海报 + Markdown 总领速览；必须明说 11 章仅为展开入口、尚未生成章节全文 |
...
> **八字独立（选 1）专属**：①**输出最开头必先整段输出 `prompts/disclaimer-preamble.md`**（成长心态两段），再进正文；②按 `prompts/output-mode-B.md` 走【总领速览 + 按需下钻】——默认先出速览（顶部告示 + 定调锚点 + 固定章号菜单），用户回「详细展开第 X 章」再只深写该章并在顶部复述锚点；③正文按 **五行→十神→神煞→大运** 骨架，注入所选流派的 `pai_panci` / `pillar_emphasis` / `literature`，**严格在该派方法论内、以该派文献为核心分析、不串派**（见 bazi-prompt「流派忠实度」一节；仅『不限/open』才多派并陈），并强制神煞铁律；④除非用户在 Step0 选"完整报告"，否则不一次性下灌全文。**绝不能把 11 章菜单说成“11 章详解已经生成”**；状态须写成「总览已生成 / 11 章入口已列出 / 章节正文待用户点选后生成」。用户选“详解 + 海报”时，再单列「海报：已生成」「对话详解：当前为总览」两行。⑤底部可给一条常驻帮助说明：「有疑惑可直接把原句发给我；想深挖就回复章号。」这不是追问，不用问号逼用户继续。
...
> **海报文字规则【用户定】**：海报默认服务完全不懂命理的读者，必须先白话结论、再给盘面证据；每个术语在同句翻成现实语言，去掉括号内术语后正文仍完整。标签/表格/条目类字段守字数上限保版式；**四大解读段（〔段落〕：性格/事业/婚恋/健康类）每段须 ≥6 句、≥200 字、不设上限，2~3 句不合格；精读段（合冲刑害/运岁/神煞）3~7 句讲透重点**；海报为长图、不限一页高度，宁详勿略。**口吻一律第二人称对话式（「你是…」「你会…」），不用「命主」播报腔；所有正向或偏正向语义（优势、能力、好性格、适配方向、助力、顺风/上行窗口、改善与收益）逐处用完整的 `hl-good` 短语或子句标成绿色粗体，不能只挑每段一两句；正负混合句拆开着色**。八字四大段每段至少 3 处绿只是机器底线。输出前删除全部已标绿内容复读，剩余文字仍有任何正向判断即补标。每段须闭合「你是什么样→现实表现→帮助/代价→具体动作」，让用户无需再问 AI 即可看懂，同时在阅读指引中明确「有疑惑或想进一步了解，可直接问 AI」。八字海报含「作用关系·合冲刑害」「运岁引动」两个区块（盘面行由算法注入，解读段由 LLM 产出）；**顺风、平路、逆风与体强/中/弱都要同时给出“助长焦点”**：顺风重兑现与扩张，逆风重补结构、练边界与换挡，体弱重恢复与承载，不能把逆风或体弱写成“没有成长”。
