...
description: 八字 + 紫微斗数 AI 排盘与综合分析。当用户提供生辰（阳历/农历日期、时辰、性别）询问八字、紫微、命理、命盘、流年大运相关问题时使用。基于 Yiqi 算法库 + enrichBazi 补全层精准排盘（杜绝 LLM 自行排盘出错），支持按需独立分析八字 / 独立分析紫微 / 两盘交叉印证。八字解读支持【流派镜片】（子平/滴天髓/神峰/盲派/段氏/不限）与【神煞】补层，开头加【成长心态声明】，采用【总领速览+按需下钻】输出。完全不懂八字的用户可直接说"帮我算算"，走极简引导（小白模式自动触发，仅一问即开工）。
---
...
**如果用户选 2（紫微独立），追问呈现形态**
> "紫微可以这样输出：
> A. **📜 长文深度版**（Markdown，命格主轴 + 大限 + 流年三档）
> B. **🎴 结构化海报版**（单文件 HTML，十二宫盘可视化 + 命格解读，可截图分享）
> C. **两种都要**"
...
> 呈现形态：A. **📜 长文深度版**（默认；总领速览 + 按需下钻，完整报告仅 Cowork 线建议） / B. **🎴 结构化海报版**（单文件 HTML，可截图分享） / C. **两种都要** / D. **🧬 八字 MBTI 海报**（用荣格八维×十神把你的盘翻译成 MBTI 语言；可主动提供实测 MBTI 解锁「实测×底盘」差异版块；可与 A/B 叠加）"
...
| 2 + A（长文·默认） | `prompts/disclaimer-preamble.md` ＋ `prompts/ziwei-prompt.md` | — | Markdown 对话回复（开头必出 disclaimer） |
| 2 + B（海报） | `prompts/ziwei-poster.md` | `templates/report-ziwei-poster.html` | `<name>-ziwei.html`（`render --mode=ziwei`） |
| 2 + C（两者） | 上述两份都跑 | 同上 | Markdown + HTML 两份 |
...
> **海报文字规则【用户定】**：海报默认服务完全不懂命理的读者，必须先白话结论、再给盘面证据；每个术语在同句翻成现实语言，去掉括号内术语后正文仍完整。标签/表格/条目类字段守字数上限保版式；**四大解读段（〔段落〕：性格/事业/婚恋/健康类）每段须 ≥6 句、≥200 字、不设上限，2~3 句不合格；精读段（合冲刑害/运岁/神煞）3~7 句讲透重点**；海报为长图、不限一页高度，宁详勿略。**口吻一律第二人称对话式（「你是…」「你会…」），不用「命主」播报腔；所有正向或偏正向语义（优势、能力、好性格、适配方向、助力、顺风/上行窗口、改善与收益）逐处用完整的 `hl-good` 短语或子句标成绿色粗体，不能只挑每段一两句；正负混合句拆开着色**。八字四大段每段至少 3 处绿只是机器底线。输出前删除全部已标绿内容复读，剩余文字仍有任何正向判断即补标。每段须闭合「你是什么样→现实表现→帮助/代价→具体动作」，让用户无需再问 AI 即可看懂。八字海报含「作用关系·合冲刑害」「运岁引动」两个区块（盘面行由算法注入，解读段由 LLM 产出）。
