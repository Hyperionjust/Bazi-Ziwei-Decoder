...
function getYinYang(yearGan: string, gender: 'male' | 'female'): string {
  // 阴阳取「农历年干」：甲丙戊庚壬为阳、乙丁己辛癸为阴。
  // v3.5 修复：此前误按公历年 (year-4)%10 取干，公历1月~春节前出生者阴阳判反、大限方向全错。
...
export function createZiweiChart(birthInfo: BirthInfo): ZiweiChart {
  try {
    ...
    // v3.5 修复：晚子时（23:00-24:00）与八字侧同口径——日界按 23 点换日，
    // 紫微的农历年/月/日一律取次日（跨月/跨年自动连带）。此前紫微用原日，与八字日柱自相矛盾。
    const effectiveSolar = birthInfo.hour === 23 ? solar.next(1) : solar;
    const lunar: Lunar = effectiveSolar.getLunar();
    
    const lunarYear = lunar.getYear();
    // v3.5 修复：闰月按本月论（lunar-typescript 闰月返回负月份，直接代入会算错宫位）
    ...
    // 3. 判断阴阳男女（v3.5：取农历年干，与安星/四化同源）
    const yinYang = getYinYang(yearGan, birthInfo.gender) as any;
    ...
    // 12. 计算大运信息（v3.5：虚岁按农历年差计，春节前出生不再少算一岁）
    const _now = new Date();
