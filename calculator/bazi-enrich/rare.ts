...
export interface RareItem {
  ...
  // 批4 修:体检器判「解读有没有点名这个罕象」时,允许的写法。
  //   此前体检器拿「名字前 3 字」硬匹配,而「原局天克地冲」的前 3 字是「原局天」——
  //   文里自然写的是「天克地冲」,于是明明点名了也判 FAIL。什么算「提到了」,
  //   应由定义罕象的这里说了算,不该由体检器猜前缀。
  //   缺省(不写)时体检器回退到「去括号、去『原局』前缀的全名」,故只有别名与全名差得远的才需要写。
  匹配词?: string[];
}
...
export function detectRarePatterns(siZhu: Record<Pillar, GZ>, shenshaHits: any[], zhiRels: any[], ganRels: any[]): RareItem[] {
  ...
  // ---- 地支组合 ----
  if (has4(['辰', '戌', '丑', '未'])) out.push({ 名: '四库全(四大墓库齐全)', 匹配词: ['四库', '四墓库', '辰戌丑未'], 罕见度: '极罕', 涉及: '四支辰戌丑未', 说明: '财官印食皆有库藏,古论「人元用事,包藏四方」,主收纳格局宏大、中晚年蓄势极厚,亦主一生多聚散开合之课题' });
  if (has4(['寅', '申', '巳', '亥'])) out.push({ 名: '四生全(四长生齐)', 匹配词: ['四生', '四长生', '寅申巳亥'], 罕见度: '极罕', 涉及: '四支寅申巳亥', 说明: '四驿马长生之地齐聚,主一生动象极强、开创不休,宜动不宜守' });
  if (has4(['子', '午', '卯', '酉'])) out.push({ 名: '四正全(四败/遍野桃花)', 匹配词: ['四正', '四败', '遍野桃花', '子午卯酉'], 罕见度: '极罕', 涉及: '四支子午卯酉', 说明: '四旺之气纯而不杂,古有「遍野桃花」之名,主气性极专、魅力与是非同重' });
  ...
  if (new Set(gans).size === 1) out.push({ 名: '天干一字', 罕见度: '极罕', 涉及: `四干皆${gans[0]}`, 说明: '天元一气,古法专论之格,气纯而志一' });
    ...
    out.push({ 名: '三朋(三干一字)', 匹配词: ['三朋', '三干一字'], 罕见度: '罕见', 涉及: `三干皆${g}`, 说明: '三干同气并立,主其五行之性格外突出' });
  }
  // 天干双合(两组五合,不同柱位)
  {
    const pairs: string[] = [];
    ...
    if (pairs.length >= 2) out.push({ 名: '天干双合', 匹配词: ['双合'], 罕见度: '少见', 涉及: pairs.join('、'), 说明: '两组干合并见,主人际粘性强、诸事多以「合」成局,亦多绊' });
  }
  ...
  for (let i = 0; i < 4; i++) for (let j = i + 1; j < 4; j++) {
    ...
    if (a.gan === b.gan && a.zhi === b.zhi)
      out.push({ 名: '原局伏吟', 匹配词: ['伏吟'], 罕见度: '少见', 涉及: `${P[i]}${P[j]}柱${a.gan}${a.zhi}`, 说明: '两柱干支全同,该宫位之事多重复再现,宜以变应静' });
    ...
    if (ke && LIU_CHONG[a.zhi] === b.zhi)
      out.push({ 名: '原局天克地冲', 匹配词: ['天克地冲'], 罕见度: '罕见', 涉及: `${P[i]}柱${a.gan}${a.zhi}×${P[j]}柱${b.gan}${b.zhi}`, 说明: '两柱对冲对克,所涉两宫一生互相牵动,大动大成之枢纽' });
  }

  // ---- 关系层(三合三会全等由检测层直接取) ----
  for (const r of (zhiRels || [])) {
    if (r.type === '三合' ) out.push({ 名: '三合局全', 匹配词: ['三合局', '三合'], 罕见度: '少见', 涉及: (r.zhi || []).join(''), 说明: `${r.detail || '三合成局'},该五行之势贯穿全局` });
    if (r.type === '三会') out.push({ 名: '三会方全', 匹配词: ['三会方', '三会'], 罕见度: '少见', 涉及: (r.zhi || []).join(''), 说明: `${r.detail || '三会成方'},方局之力大于三合,一方之气独大` });
  }
  // 双冲对峙
  {
  ...
  if (dx && (dx.pillars || []).length >= 3) out.push({ 名: '德秀满盘', 匹配词: ['德秀'], 罕见度: '极罕', 涉及: `德秀贵人见于${dx.pillars.join('')}柱`, 说明: '德秀本为复合贵格,遍布三柱以上尤罕,主聪明温厚之气贯全局,遇财官则贵' });
  if (hit('tianyi_guiren') && hit('tiande_guiren') && hit('yuede_guiren'))
    out.push({ 名: '三德会聚', 匹配词: ['三德'], 罕见度: '罕见', 涉及: '天乙+天德+月德并见', 说明: '至尊之贵与二德同显,凶煞难近,遇难呈祥之力为诸格之最' });
