...
export type ZhiRelation = {
  type: '六冲'|'六合'|'三合'|'半合'|'三会'|'半会'|'相刑'|'自刑'|'六害'|'相破'|'暗合'|'拱合'|'拱会';
  zhi: Dizhi[];      // 参与的地支
...
export function detectZhiRelations(zhis: Record<Pillar, Dizhi>): ZhiRelation[] {
  ...
  for (const sh of SAN_HUI) {
    ...
    if (distinctZhi.length === 3) {
      ...
      if (missIdx === 1) {
        out.push({type:'拱会', zhi: distinctZhi as Dizhi[], pillars: present.map(p=>p.pillar), detail: `拱会${missing}(${sh.wuxing}方·夹拱)`});
      } else {
        out.push({type:'半会', zhi: distinctZhi as Dizhi[], pillars: present.map(p=>p.pillar), detail: `${sh.wuxing}方半会(方气两支,缺${missing})`});
      }
    }
  }
  ...
  const explicitTypes: ZhiRelation['type'][] = ['六冲','六合','六害','三合','半合','三会','半会','拱合','拱会','相刑','自刑','相破'];
