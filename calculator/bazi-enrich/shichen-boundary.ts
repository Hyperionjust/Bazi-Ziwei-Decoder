...
// ---------------------------------------------------------------------------
// 晚子时约定披露(P0-B) — 仅出生时刻 ∈ [23:00, 24:00) 时输出。
// 实测确认(临时脚本 2026-07-28): yiqi-core 对 23:00-23:59 出生采用【换日】约定——
// 日柱按次日推,整盘等同次日早子时盘(2000-01-01 23:30 日柱己未=次日盘;紫微农历日同步廿六)。
// ---------------------------------------------------------------------------
export interface ZishiConvention {
  window: string;
  约定: string;
  说明: string;
}

/** 出生时刻(归一化后)在晚子时窗口时返回约定披露块,否则 null。 */
export function zishiConventionNote(hour: number): ZishiConvention | null {
