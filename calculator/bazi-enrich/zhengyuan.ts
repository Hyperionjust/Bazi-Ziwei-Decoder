// zhengyuan.ts — 正缘倾向判定(算法层) v2
// ---------------------------------------------------------------------------
// 通行断法原本输出年长/年轻/同龄。它更适合作为「关系角色气质」的传统信号，
// 不能锁死现实年龄差：年长→成熟承接，年轻→细腻鲜活，同龄→平等同频。
// 保留年龄倾向供兼容/校准，新增相处气质作为用户文案的首选锚点。
// 通行断法(非铁律,置信度随证据强弱):
...
export function judgeSpouseProfile(siZhu: Record<Pillar, GZ>, gender: 'male'|'female'): SpouseProfile {
  ...
  const 气质映射: Record<SpouseProfile['年龄倾向'], SpouseProfile['相处气质']> = {
    年长: '成熟承接型',
    年轻: '细腻鲜活型',
    同龄: '平等同频型',
  };
  const 解读口径 = '优先解读心智与相处气质，不限定实际年龄；如提年龄须明确仅作传统参考';

  return { 年龄倾向: 倾向, 相处气质: 气质映射[倾向], 解读口径, 置信, 夫妻星: starLabel, 星位, 宫坐: gongSS || '-', 依据 };
}
