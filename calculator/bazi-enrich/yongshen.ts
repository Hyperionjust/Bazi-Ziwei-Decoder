...
export function adviseYongShen(dayMaster: Tiangan, ws: WangShuaiResult, tiaoHouGans: string[], geju: GeJuResult, wuxingCount?: Record<string, number>, siZhu?: Record<'年'|'月'|'日'|'时', { gan: Tiangan; zhi: Dizhi }>): YongShenAdvice {
  ...
  // v3.12 批B1·轴冲突扩义:交集对象从「扶抑线所忌」扩为【出口忌神终值】——质检盘(1991 丁火申月)
  // 实锤盲区:调候取火(丙照)而出口忌火(格局线财格忌比劫,经临界回退进忌),旧定义(调候∩扶抑忌)
  // 抓不到,「宜火」「忌火」在同一篇各说各话。jiShen 是三线裁决终值(已滤开运/喜神/裁决救回),
  // 语义上正是「这盘最终忌什么」,与调候所取取交集才是完整的两面性清单。
  const 冲突五行 = [...new Set(thWx)].filter(w => jiShen.includes(w) && !(caiJue?.自扶抑忌救回 || []).includes(w));
  const 轴冲突 = 冲突五行.length ? {
    五行: 冲突五行,
    调候侧: `调候取${thWx.join('、')}(${(tiaoHouGans || []).join('')})——${冲突五行.join('、')}是护体所需`,
    扶抑侧: 冲突五行.every(w => fuYi.忌.includes(w))
      ? `扶抑忌${fuYi.忌.join('、')}——同一个${冲突五行.join('、')}又是发用所忌`
      : `出口忌${jiShen.join('、')}(${fuYi.忌.length ? `扶抑忌${fuYi.忌.join('、')}` : '扶抑临界回退比劫'}${caiJue ? '/重神转忌' : ''})——同一个${冲突五行.join('、')}又是发用所忌`,
    ...
    说明: '轴冲突=调候线所取 ∩ 出口忌神(v3.12 起,原为扶抑线所忌——出口忌是三线裁决终值,含临界回退与重神转忌)。这是确定性事实(直接取交集),不是判断。',
  } : undefined;
