...
function get(url: string, timeout: number, binary = false): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { timeout, headers: { 'User-Agent': 'bazi-ziwei-decoder-selfupdate' } }, res => {
      if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        get(res.headers.location, timeout, binary).then(resolve, reject); res.resume(); return;
      }
...
async function main() {
  ...
  let remote = '';
  try { remote = (await get(`https://raw.githubusercontent.com/${repo}/${branch}/VERSION`, timeout)).toString('utf-8').trim(); }
  ...
  try {
    ...
    out({ local, remote, update_available: true, skip: `新版下载/解压失败(${(e as Error).message}),继续用当前版本` });
  }
}
main();
