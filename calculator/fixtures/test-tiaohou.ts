...
// 祝福记录: a2c05e1a…(M4 全表基线) → bd7a2033…(v3.12 批C2,2026-07-31):
//   词表 v2 柱位谓词回收 53 条「若」(判定备注均带【已回收】前后对照,分行方案见三份
//   huishou-out 清单,子会话产出+主会话应用),另 19 条不回收留痕(节气类/同柱语义/
//   五虎遁历法约束下回收即死条例——历法约束是本批新发现);bless 关系重算变更 8 处。
//   4000 盘审计:死条例 0/恒真 0;蕴含一致性全过。
const EXPECTED_TIAOLI_HASH = 'bd7a2033ba21c6350186ccac7ed155a9';   // v3.11.0 M4 十干全表 120 格 / 1396 条
{
...
// 为前提,中和盘命中「身弱」字样条例名不副实(质检报告 P1-2)。前提=身:弱 过滤后:
{
  ...
  ok(中和.词表版本 === 'v2', `词表版本已升 v2(v1.1 身势前提 + v2 柱位谓词) (得到 ${中和.词表版本})`);
}
