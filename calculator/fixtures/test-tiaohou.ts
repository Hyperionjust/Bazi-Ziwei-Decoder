...
// ── ⓓ 求值器行为(手工盘,正例 + 反例都断言) ────────────────────────────────
{
  ...
  // (随批次推进换一个尚未吸收的日干:M1 乙 → M2 戊 → M3 辛;M4 吸收完十干后本断言改为断言全表已覆盖)
  const 未吸收 = evalTiaoLi(盘('丙寅 庚寅 辛亥 癸未') as any);
  ok(未吸收.有条例 === false && 未吸收.命中.length === 0 && 未吸收.格 === '辛/寅',
    '未吸收的格(辛/寅)静默返回空壳,不抛错也不虚构命中');
}
...
const EXPECTED_TIAOLI_HASH = 'bd4deb7c102269607e08bf9d1ebed8aa';   // v3.11.0 M3 甲乙丙丁戊己庚 84 格 / 968 条
{
