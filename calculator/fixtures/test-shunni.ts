...
//   ① 1926 丙寅年  丙(调候第二字)透干而火不在出口 likes → 期望 体强用平
//                  —— 单轴时代这里只能判「平」,把体的状态一起抹掉,双轴的起因
//   ② 1926-10 戊戌月 戌冲提纲辰 → 期望 振幅=剧动 —— 冲提纲曾在两条降级路径里都不生效
//   ③ 1926-04 壬辰月 偏财透干 → 期望 事件轴命中且发用不被污染(比肩格财星不入用神,
//                  发用线上必然非正,只能靠事件轴承载(方案 C))
...
ok(m04?.发用 === -1, `1926-04 发用=-1(辰土忌坐支;事件轴不参与方向分值,方向未被事件污染) (得到 ${m04?.发用})`);
