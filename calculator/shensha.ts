...
import { getNaYin, getNaYinWuXing } from './yiqi-core/nayin';
...
function nayinElem(gan: string, zhi: string): string {
  try { return getNaYinWuXing(getNaYin(gan as any, zhi as any)); } catch { return ''; }
}
...
// ---- 各 method 分派 --------------------------------------------------------
function evalDef(c: Chart, d: ShenshaDef, cfg: ShenshaConfig['config']): { pillars: string[]; via?: string } | null {
  switch (d.method) {
