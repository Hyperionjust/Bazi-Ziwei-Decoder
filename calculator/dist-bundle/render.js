...
function chartToFlat(chart, currentYear) {
  ...
  const ziweiDaxian = zw.gongs.filter((g) => g.daXian).map((g) => ({ startAge: g.daXian.startAge, endAge: g.daXian.endAge, gong: g.gong, isCurrent: g.daXian.isCurrent })).sort((a, b) => a.startAge - b.startAge).slice(0, 7);
  for (let i = 0; i < 7; i++) {
