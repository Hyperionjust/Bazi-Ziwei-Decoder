...
var _Foto = class {
  ...
  getOtherFestivals() {
    const l = [];
    const fs2 = FotoUtil.OTHER_FESTIVAL[this.getMonth() + "-" + this.getDay()];
    if (fs2) {
      fs2.forEach((f) => {
        l.push(f);
      });
...
var _Tao = class {
  ...
  getFestivals() {
    const l = [];
    const fs2 = TaoUtil.FESTIVAL[this.getMonth() + "-" + this.getDay()];
    if (fs2) {
      fs2.forEach((f2) => {
        l.push(f2);
      });
...
var Lunar = class _Lunar {
  ...
  getOtherFestivals() {
    const l = [];
    const fs2 = LunarUtil.OTHER_FESTIVAL[this._month + "-" + this._day];
    if (fs2) {
      fs2.forEach((f) => {
        l.push(f);
      });
...
var _Solar = class {
  ...
  getOtherFestivals() {
    const l = [];
    const fs2 = SolarUtil.OTHER_FESTIVAL[this._month + "-" + this._day];
    if (fs2) {
      fs2.forEach((f) => {
        l.push(f);
      });
...
function getAccurateMonthGanZhi(solar) {
  ...
  const isJieQiDay = solar.getYear() === jieQiSolar.getYear() && solar.getMonth() === jieQiSolar.getMonth() && solar.getDay() === jieQiSolar.getDay();
  if (!isJieQiDay) {
    return parseGanZhi(defaultMonthGZ);
  }
  ...
  if (solarTime < jieQiTime) {
    const prevDaySolar = solar.next(-1);
...
function createBaziChart(birthInfo) {
  try {
    ...
    const siZhu = {
      ...
      day: {
        gan: baZiForDay.getDay().substring(0, 1),
        // 使用调整后的日柱（处理晚子时）
        zhi: baZiForDay.getDay().substring(1, 2)
        // 使用调整后的日柱（处理晚子时）
      },
      hour: {
    ...
    const dayunList = yun.getDaYun().slice(1, 11);
...
function calculateXuSui(birthYear, currentYear = (/* @__PURE__ */ new Date()).getFullYear()) {
...
function calculateWuXingJu(tiangan, dizhi) {
  const ganNum = Math.floor(TIANGAN.indexOf(tiangan) / 2) + 1;
  const zhiMap = {
    "\u5B50": 1,
    "\u4E11": 1,
    "\u5348": 1,
  ...
  return {
    name: juMap[juNum],
    number: [0, 3, 4, 2, 6, 5][juNum]
    // 转换为局数：木3,金4,水2,火6,土5
  };
}
...
function installZiweiSeries(ziweiPos) {
  const stars = /* @__PURE__ */ new Map();
  stars.set("\u7D2B\u5FAE", ziweiPos);
  stars.set("\u5929\u673A", (ziweiPos - 1 + 12) % 12);
...
function createZiweiChart(birthInfo) {
  try {
    ...
    const effectiveSolar = birthInfo.hour === 23 ? solar.next(1) : solar;
    ...
    const gongsWithSiHua = addBenMingSiHua(gongs, yearGan);
    const _now = /* @__PURE__ */ new Date();
    const currentLunarYear = Solar.fromYmd(_now.getFullYear(), _now.getMonth() + 1, _now.getDate()).getLunar().getYear();
    const currentAge = calculateXuSui(lunarYear, currentLunarYear);
    const daXianList = calculateDaXian(mingGongIndex, yinYang, wuXingJu.number);
...
function resolveSolarClock(birthInfo) {
  ...
  if (birthInfo.longitude != null && Number.isFinite(birthInfo.longitude)) {
    const dayOfYear = Math.floor((Date.UTC(year, month - 1, day) - Date.UTC(year, 0, 1)) / 864e5) + 1;
...
function createChart(birthInfo) {
  try {
    const info = resolveSolarClock(birthInfo);
...
var tiaohou_default = {
  _readme: [
    "\u8C03\u5019\u7528\u795E\u8868(\u65E5\u5E72\xD7\u6708\u652F 120 \u683C) \u2014 \u4E0E shensha.json \u540C\u89C4\u683C\u7684\u6570\u636E\u5916\u7F6E\u5C42\u3002v3.10.0 \u4ECE tables.ts \u8FC1\u51FA\u3002",
    "\u4E3A\u4EC0\u4E48\u8FC1:\u795E\u715E\u65E9\u5C31\u662F\u300C\u6570\u636E+\u51FA\u5904+needs_review\u300D\u7684\u5916\u7F6E JSON,\u8C03\u5019\u5374\u786C\u7F16\u7801\u5728 .ts \u91CC\u3001\u65E0\u51FA\u5904\u3001\u65E0\u6821\u52D8\u72B6\u6001\u2014\u2014\u540C\u7C7B\u6570\u636E\u4E24\u5957\u5F85\u9047\u3002",
    "\u26A0 \u5E95\u672C\u5C1A\u672A\u786E\u8BC1:\u672C\u8868\u5185\u5BB9\u539F\u6837\u6765\u81EA tables.ts,\u6765\u6E90\u4E0D\u8BE6\u3002\u300A\u7A77\u901A\u5B9D\u9274\u300B\u81EA\u300A\u680F\u6C5F\u7F51\u300B\u2192\u4F59\u6625\u53F0\u672C\u2192\u5F90\u4E50\u543E\u8BC4\u6CE8\u672C\u6709\u591A\u4E2A\u6587\u672C\u7CFB\u7EDF,",
    "   \u4E14\u539F\u6587\u662F\u6563\u6587\u5F0F\u8BBA\u8FF0(\u300C\u6B63\u6708\u7532\u6728,\u521D\u6625\u72B9\u6709\u4F59\u5BD2,\u5F97\u4E19\u7678\u900F\u2026\u300D),\u538B\u6210\u6570\u7EC4\u672C\u8EAB\u5E26\u7F16\u8F91\u5224\u65AD\u2014\u2014",
    "   \u6240\u8C13\u300C\u9010\u683C\u5BF9\u539F\u6587\u300D\u5B9E\u9645\u662F\u300C\u590D\u6838\u4E00\u6B21\u62BD\u53D6\u300D,\u4E0D\u662F\u6284\u5199\u6BD4\u5BF9\u3002\u5728\u62FF\u5230\u6743\u5A01\u5E95\u672C\u524D,\u5168\u8868\u6309\u3010\u5F85\u6838\u3011\u5904\u7406,\u4E0D\u5F97\u5BF9\u5916\u5BA3\u79F0\u4E3A\u91D1\u6807\u3002",
    "\u5982\u4F55\u8BA4\u9886:\u62FF\u5230\u5E95\u672C\u540E,\u628A\u8BE5\u683C\u7684\u539F\u6587\u4E0E\u51FA\u5904\u586B\u8FDB \u6821\u52D8.\u5DF2\u6838,\u5E76\u628A fixtures/test-tiaohou.ts \u7684\u5FEB\u7167\u91CD\u65B0\u795D\u798F(bless)\u3002",
    "\u6821\u52D8\u5206\u4E24\u7EA7:\u3010\u5E95\u672C\u3011=\u6709\u51FA\u7248\u793E/\u7248\u6B21/\u9875\u7801\u7684\u53EF\u786E\u8BC1\u7248\u672C;\u3010\u4F50\u8BC1\u3011=\u53EF\u7528\u4E8E\u4EA4\u53C9\u6838\u5BF9\u4F46\u8FBE\u4E0D\u5230\u5E95\u672C\u6807\u51C6\u7684\u6587\u732E(\u5982\u73B0\u4EE3\u91CD\u6392\u6458\u7F16\u672C)\u3002\u4F50\u8BC1\u7EA7\u53EA\u80FD\u89E3\u9664\u7591\u70B9\u3001\u4E0D\u80FD\u5355\u72EC\u5B9A\u6848\u3002"
  ],
  \u5E95\u672C: {
    \u72B6\u6001: "\u672A\u786E\u8BC1",
    \u5F52\u5C5E\u672A\u5B9A: "\u26A0 \u5E95\u672C\u4ECD\u672A\u786E\u8BC1(\u4E24\u4F50\u8BC1\u672C\u5747\u975E\u6709\u7248\u6B21\u9875\u7801\u7684\u6B63\u89C4\u51FA\u7248\u7269)\u3002\u4F46\u539F\u300C\u4E24\u7CFB\u5206\u6B67\u300D\u62C5\u5FE7\u5DF2\u7F13\u89E3:\u6807\u5FD7\u683C \u7532/\u5BC5 \u5728\u9020\u5316\u5143\u94A5\u7CFB(\u674E\u94C1\u7B14\u5F71\u5370\u672C)\u4E0E\u7A77\u901A\u5B9D\u9274\u7CFB(\u77E5\u781A\u658B\u672C)\u5B9E\u6838\u4E00\u81F4\u3002",
    "\u7248\u672C\u6E90\u6D41(\u636E\u516C\u5F00\u68C0\u7D22\u6240\u5F97,\u672C\u8EAB\u4EA6\u5F85\u5E95\u672C\u4F50\u8BC1)": [
      "\u300A\u680F\u6C5F\u7F51\u300B\u6C11\u95F4\u6D41\u4F20 \u2192 \u6E05\u5EB7\u7199\u5E74\u95F4\u56E0\u907F\u8BB3\u6539\u540D\u300A\u9020\u5316\u5143\u94A5\u300B",
      "\u6E05\u5149\u7EEA\u5E74\u95F4 \u4F59\u6625\u53F0 \u6574\u7406\u520A\u884C \u2192 \u300A\u7A77\u901A\u5B9D\u9274\u300B",
      "\u6C11\u56FD\u4E8C\u5341\u516D\u5E74(1937)\u300A\u7A77\u901A\u5B9D\u9274\u300B\u901A\u884C\u672C\u8BB9\u8BEF\u751A\u591A(\u6709\u300C\u5357\u300D\u6284\u4F5C\u300C\u897F\u300D\u4E00\u7C7B\u5B57\u8BEF\u7684\u8BB0\u8F7D)",
      "\u6C11\u56FD\u4E8C\u5341\u516B\u5E74(1939)\u5F90\u4E50\u543E\u5F97\u4E00\u7CBE\u6821\u672C\u4F5C\u6CE8,\u4ECD\u540D\u300A\u9020\u5316\u5143\u94A5\u300B,\u4E00\u822C\u8BA4\u4E3A\u51C6\u786E\u7387\u66F4\u9AD8",
      "\u5F90\u4E50\u543E\u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u6210\u7A3F\u540E\u672A\u53CA\u51FA\u7248\u5373\u79BB\u4E16,\u4EE5\u624B\u6284\u672C\u6D41\u4F20;\u540E\u7531\u97E6\u5343\u91CC\u6821\u5B9A,\u4E19\u5348\u5E74(1966)\u520A\u884C",
      "\u73B0\u4EE3\u91CD\u5370\u672C\u5982 \u6881\u6E58\u6DA6 \u7F16\xB7\u5B8F\u4E1A\u7CBE\u5370\u672C(\u68C0\u7D22\u5230 ISBN 9786523266832),\u7248\u672C\u4FE1\u606F\u9700\u5B9E\u7269\u6838\u5BF9"
    ],
    \u5DF2\u77E5\u4E89\u8BAE: "\u6709\u8BBA\u8005\u6307\u5F90\u4E50\u543E\u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u6DF7\u8BA2\u300A\u9020\u5316\u5143\u94A5\u300B\u4E0E\u300A\u7A77\u901A\u5B9D\u9274\u300B\u4E24\u7CFB\u800C\u672A\u52A0\u533A\u5206,\u4E3E\u7684\u4F8B\u5B50\u6B63\u662F\u300C\u6B63\u6708\u7532\u6728\u300D\u7684\u7528\u795E\u53D6\u6CD5\u4E24\u672C\u4E0D\u540C\u3002",
    \u9700\u8981\u7684\u4FE1\u606F: [
      "\u4E66\u540D\u4E0E\u7F16\u8005/\u6CE8\u8005",
      "\u51FA\u7248\u793E\u4E0E\u7248\u6B21/\u5E74\u4EFD",
      "\u9875\u7801\u6216\u5377\u6B21",
    ...
    \u8BF4\u660E: "\u7F51\u7EDC\u6D41\u901A\u7535\u5B50\u7248\u8F6C\u5F55\u9519\u8BEF\u7387\u9AD8,\u4E0D\u4F5C\u4E3A\u5E95\u672C\u91C7\u4FE1;\u767E\u79D1/\u95EE\u7B54\u7C7B\u9875\u9762\u53EA\u80FD\u63D0\u4F9B\u7248\u672C\u4FE1\u606F,\u4E0D\u80FD\u63D0\u4F9B\u53EF\u6838\u7684\u539F\u6587\u3002"
  },
  \u53D6\u5E72: {
    \u7532: {
      \u5BC5: [
  ...
  \u6821\u52D8: {
    \u5DF2\u6838: {
      "\u4E01/\u5BC5": {
        ...
        \u539F\u6587: "\u975E\u5E9A\u4E0D\u80FD\u5288\u7532,\u975E\u7532\u4E0D\u80FD\u5F15\u4E01",
        \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u4E01\u706B\u7BC7(\u5F15\u6587\u89C1 fixtures/test-boundary.ts,v3.5 \u4FEE\u590D\u8BB0\u5F55:\u4FEE\u590D\u524D\u8BEF\u4F5C\u5E9A\u58EC)",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u5E95\u672C"
      },
      "\u5E9A/\u672A": {
        ...
        \u539F\u6587: "\u516D\u6708\u5E9A\u91D1\u2026\u571F\u65FA\u91D1\u987D,\u6545\u5148\u7528\u4E01\u706B,\u6B21\u53D6\u7532\u6728,\u8EAB\u65FA\u56FA\u559C\u8D22\u5B98,\u8EAB\u5F31\u559C\u5370,\u4EA6\u53D6\u7532\u6728\u758F\u571F,\u4EE5\u663E\u5E9A\u91D1\u4E4B\u7528\u4E5F",
        \u51FA\u5904: "\u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u6458\u7F16\u672C \u4E09\u590F\u5E9A\u91D1\u6BB5(\u4F50\u8BC1\u7EA7,\u89C1 \u4F50\u8BC1[\u9020\u5316\u5143\u94A5\u8BC4\u6CE8-\u6458\u7F16-2024];\u5E95\u672C\u5F85\u8865)",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u4F50\u8BC1"
      },
      "\u7532/\u5BC5": {
        ...
        \u539F\u6587: "\u6B63\u6708\u7532\u6728,\u521D\u6625\u5C1A\u6709\u4F59\u5BD2,\u5F97\u4E19\u7678\u900F,\u5BCC\u8D35\u53CC\u5168\u3002\u7678\u85CF\u4E19\u900F,\u540D\u5BD2\u6728\u5411\u9633,\u4E3B\u5927\u5BCC\u8D35",
        \u51FA\u5904: "\u53CC\u6E90:\u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u674E\u94C1\u7B14\u5F71\u5370\u672C \u4E66\u987572(\u7B2C\u4E03\u8282 \u4E09\u6625\u7532\u6728\u603B\u8BBA\xB7\u539F\u6587\u6BB5) + \u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u4F50\u8BC1",
        \u5907\u6CE8: "\u2B50\u4E24\u7CFB\u4E00\u81F4\u2014\u2014\u6B64\u683C\u66FE\u88AB\u5217\u4E3A\u4E24\u7CFB\u5206\u6B67\u6807\u5FD7\u683C(\u6709\u8BBA\u8005\u4EE5\u300C\u6B63\u6708\u7532\u6728\u300D\u4E3E\u4F8B\u6DF7\u8BA2),\u5B9E\u6838\u7ED3\u679C\u4E24\u7CFB\u7686\u4E19\u7678,\u5206\u6B67\u4F20\u8A00\u5728\u6B64\u4E24\u4EFD\u6587\u732E\u95F4\u4E0D\u6210\u7ACB\u3002"
      },
      "\u4E01/\u5DF3": {
        \u53D6\u5E72: [
          "\u7532",
        ...
        \u539F\u6587: "\u56DB\u6708\u4E01\u706B\u4E58\u65FA,\u867D\u53D6\u7532\u5F15\u4E01,\u5FC5\u7528\u5E9A\u5288\u7532\u2026\u4F46\u56DB\u67F1\u5FCC\u89C1\u7678\u6C34,\u7678\u6C34\u4E00\u89C1,\u6CC4\u5E9A\u3001\u6E7F\u7532\u3001\u4F24\u4E01,\u6545\u4EE5\u7678\u4E3A\u75C5",
        \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C \u56DB\u6708\u4E01\u706B",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u4F50\u8BC1",
        \u5907\u6CE8: "\u590F\u6708\u4E0D\u53D6\u6C34\u7684\u4F8B\u5916\u6709\u660E\u786E\u5178\u636E:\u7678\u5728\u6B64\u683C\u662F\u300C\u75C5\u300D\u4E0D\u662F\u836F\u3002\u5BD2\u6696\u4E0D\u53D8\u5F0F\u4F8B\u5916\u6E05\u5355\u5408\u6CD5\u3002"
      },
      "\u5DF1/\u672A": {
        \u53D6\u5E72: [
          "\u7678",
        ...
        \u539F\u6587: "\u4E09\u590F\u5DF1\u571F,\u6742\u6C14\u624D\u5B98,\u79BE\u7A3C\u5728\u7530,\u6700\u559C\u7518\u6C9B,\u53D6\u7678\u4E3A\u8981,\u6B21\u7528\u4E19\u706B\u3002\u590F\u65E0\u592A\u9633,\u79BE\u7A3C\u4E0D\u957F,\u6545\u65E0\u7678\u66F0\u65F1\u7530,\u65E0\u4E19\u66F0\u5B64\u9634",
        \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C \u4E09\u590F\u5DF1\u571F",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u4F50\u8BC1",
        \u5907\u6CE8: "\u6B64\u524D\u767B\u8BB0\u7684\u300C\u5DF1/\u672A \u5206\u6B67(\u58EC\u5E9A)\u300D\u662F\u62BD\u53D6\u9519\u8BEF:\u6458\u7F16\u672C\u90A3\u6BB5\u300C\u5DF1\u571F\u5F53\u6743\u2026\u4E13\u7528\u58EC\u6C34\u300D\u5B9E\u4E3A\u3010\u516D\u6708\u8F9B\u91D1\u3011\u6761\u76EE(\u300C\u5DF1\u571F\u5F53\u6743\u300D\u662F\u6708\u4EE4\u63CF\u8FF0\u975E\u4E3B\u8BED),\u672C\u672C\u300C\u516D\u6708\u8F9B\u91D1,\u5DF1\u571F\u5F53\u6743\u2026\u5148\u7528\u58EC\u6C34,\u53D6\u5E9A\u4F50\u4E4B\u300D\u9010\u53E5\u5BF9\u5E94\u3002\u5047\u5206\u6B67,\u6536\u56DE\u3002"
      },
      "\u4E01/\u5348": {
        \u53D6\u5E72: [
          "\u58EC",
        ...
        \u539F\u6587: "\u4E94\u6708\u4E01\u706B,\u65F6\u5F52\u5EFA\u7984,\u4E0D\u5B9C\u4E71\u7528\u7532\u6728\u3002\u9047\u5E74\u900F\u9694\u4F4D\u4E4B\u58EC,\u4E0D\u8D2A\u4E01\u5408\u8005,\u5FE0\u800C\u4E14\u539A\u2026\u5F97\u5E9A\u58EC\u4E24\u900F\u8005,\u79D1\u7532\u5B9A\u7136\u2026\u5F97\u4E00\u7678\u900F,\u540D\u72EC\u6740\u5F53\u6743,\u51FA\u4EBA\u5934\u5730",
        \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C \u4E94\u6708\u4E01\u706B",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u4F50\u8BC1",
        \u5907\u6CE8: "\u58EC\u5E9A\u7678\u4E09\u8005\u5747\u6709\u539F\u6587\u4F9D\u636E;\u6458\u7F16\u672C\u53EA\u89C1\u58EC\u662F\u56E0\u6458\u7F16\u4E0D\u5168\u3002"
      },
      "\u8F9B/\u672A": {
        \u53D6\u5E72: [
          "\u58EC",
        ...
        \u539F\u6587: "\u516D\u6708\u8F9B\u91D1,\u5DF1\u571F\u5F53\u6743,\u8F85\u52A9\u592A\u591A,\u6050\u63A9\u91D1\u5149,\u5148\u7528\u58EC\u6C34,\u53D6\u5E9A\u4F50\u4E4B\u2026\u4F46\u5FCC\u620A\u51FA,\u5F97\u7532\u5236\u4E4B,\u65B9\u5409",
        \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C \u516D\u6708\u8F9B\u91D1",
        \u72B6\u6001: "\u5DF2\u6838",
        \u7EA7\u522B: "\u4F50\u8BC1"
      }
    },
    \u5F85\u6838\u7591\u70B9: [
      {
        ...
        \u7591\u70B9: "\u884C\u5185\u91CD\u590D\u5EA6\u504F\u9AD8",
        \u5224\u636E: "\u91CD\u590D\u5EA6\u6307\u6807",
        \u5907\u6CE8: "\u4E19\u706B\u884C\u540C\u6837\u4EE5\u58EC\u8D2F\u7A7F\u4F46\u6709\u5178\u636E(\u4E19\u706B\u731B\u70C8\u4E13\u7528\u58EC\u6C34),\u4E01\u884C\u662F\u5426\u540C\u7406\u9700\u5E95\u672C\u786E\u8BA4\u2014\u2014\u4E0D\u9884\u8BBE\u4E3A\u9519"
      },
      {
        \u4F4D\u7F6E: "\u620A \u884C",
        \u73B0\u503C: "12 \u683C\u4EC5 4 \u79CD\u53D6\u6CD5",
      ...
      },
      {
        \u4F4D\u7F6E: "\u7532/\u8FB0",
        \u73B0\u503C: "\u53D6\u5E72 \u5E9A\u4E01\u58EC",
        \u7591\u70B9: "\u4E24\u672C\u539F\u6587\u5747\u4F5C\u300C\u5148\u5E9A\u6B21\u58EC\u300D,\u4E14\u660E\u8BF4\u300C\u652F\u6210\u91D1\u5C40\u65B9\u53EF\u7528\u4E01,\u4E0D\u7136\u4E09\u6708\u65E0\u7528\u4E01\u4E4B\u6CD5\u300D\u2014\u2014\u4E01\u662F\u6761\u4EF6\u6027\u7528\u795E,\u6392\u5728\u58EC\u524D\u5B58\u7591",
        \u5224\u636E: "\u77E5\u781A\u658B\u672C + \u674E\u94C1\u7B14\u5F71\u5370\u672C \u4E66\u987588 \u53CC\u6E90\u540C\u6587",
        \u5907\u6CE8: "\u672C\u8F6E\u53EA\u5438\u6536\u6761\u4F8B,\u4E0D\u52A8\u53D6\u5E72(\u53D6\u5E72\u6539\u52A8\u7275\u52A8\u6392\u76D8\u4E0E\u5FEB\u7167\u9501,\u987B\u5355\u5F00)\u3002\u6761\u4F8B \u7532\u8FB0-\u91D1\u5C40\u7528\u4E01 \u5DF2\u628A\u6761\u4EF6\u5982\u5B9E\u8BB0\u4E0B\u3002"
      },
      {
        \u4F4D\u7F6E: "\u7532/\u5DF3(\u53CA\u4E09\u590F\u7532\u6728\u6574\u6BB5)",
        \u73B0\u503C: "\u53D6\u5E72 \u7678\u4E01\u5E9A",
        \u7591\u70B9: "\u2696\u4E24\u7CFB:\u5F71\u5370\u672C\u4E09\u590F\u603B\u7EB2\u4F5C\u300C\u7528\u795E\u5148\u4E01\u6B21\u5E9A\u300D\u4E0D\u63D0\u7678,\u77E5\u781A\u658B\u9010\u6708\u4F5C\u300C\u56DB\u6708\u2026\u5148\u7678\u5F8C\u4E01\u300D",
        \u5224\u636E: "\u674E\u94C1\u7B14\u5F71\u5370\u672C \u4E66\u987590 \u7B2C\u516B\u8282\u5F00\u7BC7",
        \u5907\u6CE8: "\u5F71\u5370\u672C\u9010\u6708\u6B63\u6587\u672A\u901A\u8BFB,\u672C\u8F6E\u53EA\u767B\u8BB0\u4E0D\u5B9A\u6848,\u7559 M5 \u590D\u6838\u3002"
      },
      {
        \u4F4D\u7F6E: "\u7532/\u4E11",
        \u73B0\u503C: "\u6761\u4F8B \u7532\u4E11-\u5E9A\u900F\u4E01\u85CF / \u7532\u4E11-\u4E01\u900F\u5E9A\u85CF \u6863\u4F4D",
        \u7591\u70B9: "\u2696\u4E24\u7CFB:\u4E24\u6761\u4F8B\u7684\u9AD8\u4E0B\u5728\u4E24\u672C\u4E2D\u4E92\u6613",
        \u5224\u636E: "\u77E5\u781A\u658B\u672C\u300C\u5E9A\u900F\u4E01\u85CF,\u5C0F\u8D35/\u4E01\u900F\u5E9A\u85CF,\u5C0F\u5BCC\u8D35\u300Dvs \u5F71\u5370\u672C \u4E66\u9875126\u300C\u5E9A\u900F\u4E01\u85CF,\u5C0F\u5BCC\u8CB4/\u4E01\u900F\u5E9A\u85CF,\u5BCC\u8CB4\u300D",
        \u5907\u6CE8: "\u5DF2\u5728\u6761\u4F8B \u4E24\u7CFB\u5206\u6B67 \u5B57\u6BB5\u5E76\u5B58;\u89E3\u8BFB\u5C42\u4E0D\u5F97\u53EA\u5F15\u4E00\u65B9\u3002"
      }
    ],
    \u5F71\u5370\u672C\u62BD\u67E5: {
      \u53E3\u5F84: "\u5DE5\u5355 v3.11 \xA72.4:\u6BCF\u884C\u62BD 4~5 \u683C\u89C6\u89C9\u6838\u5BF9,\u4E14\u5FC5\u542B\u300C\u6761\u4F8B\u6700\u591A\u7684\u683C\u300D\u4E0E\u300C\u4E0E\u77E5\u781A\u658B\u672C\u6709\u51FA\u5165\u7684\u683C\u300D\u3002\u5F71\u5370\u672C\u65E0\u6587\u672C\u5C42,\u9010\u9875\u89C6\u89C9\u8BFB\u3002",
      "\u7532 \u884C": {
        \u5DF2\u62BD\u683C\u6570: 5,
        \u683C: [
          "\u7532/\u5BC5(\u6B63\u6708\xB7\u6761\u4F8B\u6700\u591A)",
        ...
        \u4E66\u9875: "72 / 73\xB776\xB777 / 86\xB787\xB788 / 90\xB791 / 126\xB7127(PDF \u9875 = (\u4E66\u9875+4)/2;\u53F3\u9875\u4E3A\u5C0F\u53F7,\u5DE6\u9875\u4E3A\u5927\u53F7)",
        \u7ED3\u8BBA: "\u4E00\u81F4 2 \u683C(\u5BC5\u3001\u536F) / \u6709\u5206\u6B67 3 \u683C(\u8FB0\u3001\u5DF3\u3001\u4E11),\u5206\u6B67\u5747\u5DF2\u767B\u8BB0\u4E3A\u300C\u4E24\u7CFB\u5206\u6B67\u300D\u4E14\u4E24\u7248\u5E76\u5B58,\u672A\u62E9\u4E00\u3002",
        \u672A\u62BD\u683C: [
          "\u7532/\u5348",
          "\u7532/\u672A",
          "\u7532/\u7533",
  ...
  \u4F50\u8BC1: [
    {
      id: "\u9020\u5316\u5143\u94A5\u8BC4\u6CE8-\u6458\u7F16-2024",
      \u6587\u732E: "\u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B(\u5F90\u4E50\u543E) \u6458\u7F16\u672C PDF",
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u4E3A\u4EC0\u4E48\u4E0D\u662F\u5E95\u672C: [
        "\u73B0\u4EE3\u91CD\u6392:PDF \u5143\u6570\u636E\u4E3A Microsoft Word 2024 \u751F\u6210\u3001\u4F5C\u8005\u7F72\u540D\u300C\u5FF5\u6614\u96EA\u300D,\u975E\u5F71\u5370\u672C,\u5B58\u5728\u91CD\u6253\u5B57\u8F6C\u5F55\u98CE\u9669",
        "\u65E0\u51FA\u7248\u793E/\u7248\u6B21/\u9875\u7801,\u6765\u6E90\u4E3A\u5F71\u5B50\u56FE\u4E66\u9986,\u8FBE\u4E0D\u5230 references/calibration.md \u5B9A\u7684\u5E95\u672C\u6807\u51C6",
        "\u6B8B\u672C:\u5168\u6587 20 \u9875(\u76EE\u5F55 2 \u9875 + \u6B63\u6587 18 \u9875, \u6B63\u6587\u4EC5\u7EA6 1.8 \u4E07\u5B57),\u76EE\u5F55\u627F\u8BFA\u7684\u6761\u76EE\u8FDC\u591A\u4E8E\u6B63\u6587\u5B9E\u6709",
        "\u6587\u672C\u7CFB\u7EDF\u9700\u5148\u5B9A\u5F52\u5C5E:\u5F90\u4E50\u543E\u8BA4\u4E3A\u300A\u9020\u5316\u5143\u94A5\u300B\u662F\u539F\u672C\u3001\u300A\u7A77\u901A\u5B9D\u9274\u300B\u4E3A\u574A\u95F4\u6539\u672C,\u4E24\u8005\u6709\u51FA\u5165;\u672C\u8868\u6807\u7684\u662F\u300A\u7A77\u901A\u5B9D\u9274\u300B"
      ],
      \u8986\u76D6: "\u4E25\u683C\u5339\u914D\u300CN\u6708+\u65E5\u5E72+\u8BE5\u5E72\u4E94\u884C\u300D\u6761\u76EE\u5934,\u6B63\u6587\u5B9E\u9645\u8986\u76D6 51/120 \u683C(42%)",
      \u4EA4\u53C9\u6838\u5BF9\u65B9\u6CD5: "\u53D6\u6761\u76EE\u5934\u540E 220 \u5B57,\u7528\u300C\u7528/\u53D6/\u5148/\u6B21/\u4E13 + \u5929\u5E72\u300D\u542F\u53D1\u5F0F\u62BD\u7528\u795E\u5B57,\u4E0E\u672C\u8868\u53D6\u5E72\u6BD4\u5BF9\u3002\u542F\u53D1\u5F0F\u6709\u660E\u663E\u5047\u9634\u6027(\u5982\u300C\u58EC\u6C34\u4E3A\u5C0A\u300D\u4E0D\u542B\u89E6\u53D1\u8BCD),\u6545\u7ED3\u679C\u53EA\u5206\u6863\u3001\u4E0D\u4F5C\u5224\u636E\u3002",
      \u7ED3\u679C: {
        \u5168\u90E8\u547D\u4E2D: 19,
        \u90E8\u5206\u547D\u4E2D: 23,
        \u672A\u547D\u4E2D: 9,
    ...
    {
      id: "\u7A77\u901A\u5B9D\u9274-\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C",
      \u6587\u732E: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406 \u7535\u5B50\u6392\u7248\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf, 53\u9875, PDFlib \u751F\u6210)",
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u4E3A\u4EC0\u4E48\u4E0D\u662F\u5E95\u672C: [
        "\u7535\u5B50\u91CD\u6392\u975E\u5F71\u5370,\u65E0\u51FA\u7248\u793E/\u7248\u6B21/\u9875\u7801"
      ],
      \u8986\u76D6: "\u4E25\u683C\u6761\u76EE\u5934 93/120 \u683C(78%)",
      \u4EA4\u53C9\u6838\u5BF9\u65B9\u6CD5: "\u540C\u6458\u7F16\u672C\u65B9\u6CD5(\u6761\u76EE\u5934\u540E 260 \u5B57,\u300C\u7528/\u53D6/\u5148/\u6B21/\u4E13/\u5C0A/\u4F50\u300D\u542F\u53D1\u5F0F),\u7ED3\u679C\u53EA\u5206\u6863\u4E0D\u4F5C\u5224\u636E",
      \u7ED3\u679C: {
        \u5168\u90E8\u547D\u4E2D: 36,
        \u90E8\u5206\u547D\u4E2D: 47,
        \u672A\u547D\u4E2D: 10
    ...
    {
      id: "\u9020\u5316\u5143\u94A5\u8BC4\u6CE8-\u674E\u94C1\u7B14\u5F71\u5370\u672C",
      \u6587\u732E: "\u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u674E\u94C1\u7B14 \u5F71\u5370\u626B\u63CF\u4EF6(\u7D20\u6750/xzb2100653, 232 PDF\u9875=464\u4E66\u9875, \u7AD6\u6392\u7E41\u4F53, \u53E4\u7C4D\u4E66\u9601\u626B\u63CF)",
      \u7EA7\u522B: "\u4F50\u8BC1(\u5F71\u5370,\u4F50\u8BC1\u7EA7\u4E2D\u6700\u9AD8;\u7248\u6743\u9875\u51FA\u7248\u793E/\u7248\u6B21\u672A\u6838,\u6545\u672A\u8FBE\u5E95\u672C)",
      \u8986\u76D6: "\u5F71\u5370\u65E0\u6587\u672C\u5C42,\u6309\u9700\u89C6\u89C9\u6838\u5BF9;\u5DF2\u6838:\u4E66\u987572 \u6B63\u6708\u7532\u6728\u300173/76/77 \u6B63\u4E8C\u6708\u300186/87/88 \u4E09\u6708\u300190/91 \u4E09\u590F\u603B\u8BBA+\u56DB\u6708\u9996\u3001126/127 \u5341\u4E8C\u6708(\u7532\u884C 5 \u683C,v3.11.0 M1)",
      \u7ED3\u679C: "\u7532/\u5BC5 \u4E0E\u7A77\u901A\u5B9D\u9274\u7CFB\u4E00\u81F4(\u300C\u5F97\u4E19\u7678\u900F\u300D,\u77E5\u781A\u658B\u7535\u5B50\u672C\u4F5C\u300C\u9022\u300D\u4E3A\u8BB9\u5B57);\u7532/\u8FB0\u3001\u7532/\u5DF3\u3001\u7532/\u4E11 \u4E09\u5904\u6709\u7CFB\u95F4\u5F02\u6587,\u5DF2\u767B\u8BB0\u4E24\u7CFB\u5206\u6B67\u5E76\u4E24\u7248\u5E76\u5B58\u3002"
    }
  ],
  \u6761\u4F8B: {
    _\u8BCD\u8868: {
      \u7248\u672C: "v1",
      \u7BA1\u8F96\u8FB9\u754C: "\u53D7\u63A7\u8BCD\u8868\u53EA\u7BA1\u3010\u5C421 \u76D8\u9762\u5224\u5B9A\u3011\u2014\u2014\u5373\u300C\u82E5\u300D\u5B57\u6BB5\u3002\u5C422(\u5219/\u540D/\u539F\u6587)\u7167\u5F55\u5178\u7C4D\u4E0D\u6539\u5199;\u5C423(\u7528\u6237\u89E3\u8BFB)\u81EA\u7531,\u786C\u7EA6\u675F\u4EC5\u4E24\u6761:\u2460\u4E0D\u5F97\u5F15\u7528\u672A\u547D\u4E2D\u7684\u6761\u4F8B\u540D \u2461\u6863\u4F4D\u4E0D\u5F97\u5199\u6210\u5BBF\u547D\u65AD\u8BED\u3002schema \u7ED9\u7684\u662F\u300C\u610F\u8C61\u300D(\u6982\u5FF5\u7EA7\u7D20\u6750),\u4E0D\u662F\u53E5\u5F0F\u6A21\u677F\u2014\u2014\u4E25\u7981\u5728\u63D0\u793A\u8BCD\u91CC\u4E3A\u4EFB\u4F55\u6761\u4F8B\u914D\u56FA\u5B9A\u767D\u8BDD\u53E5\u5F0F(v3.7.1 \u6B63\u7F18\u753B\u50CF\u6559\u8BAD)\u3002",
      \u6C42\u503C\u5668: "calculator/bazi-enrich/tiaohou-tiaoli.ts(\u8BCD\u8868\u5916\u7684\u5199\u6CD5\u5728 npm test \u91CC\u76F4\u63A5 FAIL)",
      \u8C13\u8BCD: {
        \u900F: "\u5929\u5E72X\u51FA\u73B0\u5728\u56DB\u67F1\u5929\u5E72(\u542B\u65E5\u5E72\u2014\u2014\u65E5\u5E72\u672C\u8EAB\u4E5F\u5728\u5929\u5E72\u4E0A,\u662F\u5B57\u9762\u4E8B\u5B9E)",
        \u85CF: "\u5929\u5E72X\u51FA\u73B0\u5728\u56DB\u652F\u85CF\u5E72(\u672C\u6C14/\u4E2D\u6C14/\u4F59\u6C14\u4EFB\u4E00)",
        \u6709: "\u900F \u6216 \u85CF(\u5168\u76D8\u53EF\u89C1)",
        \u65E0: "\u975E\u300C\u6709\u300D,\u5168\u76D8\u4E0D\u89C1\u6B64\u5B57",
        \u5236: "\u8BED\u4E49\u7B49\u540C\u300C\u6709\u300D,\u5BF9\u5E94\u5178\u7C4D\u300C\u6709X\u5236\u4E4B\u300D\u7684\u5199\u6CD5",
        \u652F: "\u5730\u652FZ\u51FA\u73B0\u5728\u56DB\u652F",
        \u652F\u4E94\u884C: "\u5730\u652F\u5B57\u9762\u4E94\u884C\u4E3AW\u7684\u652F(\u4E0D\u542B\u85CF\u5E72)",
        \u4F1A\u5C40: "\u56DB\u652F\u6210\u4E09\u5408/\u534A\u5408/\u4E09\u4F1A/\u534A\u4F1A\u8BE5\u4E94\u884C\u5C40(\u8868\u53D6\u81EA zhi-relations \u7684 SAN_HE/SAN_HUI,\u5355\u4E00\u4E8B\u5B9E\u6E90);\u571F\u5C40\u7279\u6307\u8FB0\u620C\u4E11\u672A\u22653",
        \u6210\u6D3E: "\u300C\u4E00\u6D3EX\u300D,\u9608\u503C\u89C1\u4E0B\u3002\u53C2\u6570\u987B\u4E3A\u540C\u4E94\u884C\u7684\u5929\u5E72\u7EC4,\u5982 \u5E9A\u8F9B/\u58EC\u7678/\u620A\u5DF1/\u4E19\u4E01",
        \u5341\u795E: "\u56DB\u67F1(\u4E0D\u542B\u65E5\u5E72\u672C\u8EAB)\u5929\u5E72+\u85CF\u5E72\u4E2D\u5B58\u5728\u8BE5\u5341\u795E\u6216\u5341\u795E\u7C7B(\u6BD4\u52AB/\u98DF\u4F24/\u8D22/\u5B98\u6740/\u5370)",
        \u5341\u795E\u900F: "\u540C\u4E0A\u4F46\u53EA\u770B\u5929\u5E72(\u4E0D\u542B\u85CF\u5E72\u3001\u4E0D\u542B\u65E5\u5E72\u672C\u8EAB)\u2014\u2014\u5BF9\u5E94\u5178\u7C4D\u300C\u5E72\u900F\u6BD4\u52AB\u300D\u8FD9\u7C7B\u660E\u8BF4\u900F\u5E72\u7684\u5199\u6CD5"
      },
      \u8BA1\u6570\u5199\u6CD5: "\u8C13\u8BCD\u52A0\u300C\u6570\u300D\u7F00 + \u6BD4\u8F83\u5F0F,\u5982 \u900F\u6570:\u4E19>=2 / \u6709\u6570:\u7678>=2 / \u5341\u795E\u6570:\u6BD4\u52AB>=2 / \u652F\u4E94\u884C\u6570:\u6C34>=2\u3002\u6BD4\u8F83\u7B26 >= > = <= <\u3002\u65E0/\u5236/\u4F1A\u5C40/\u6210\u6D3E \u4E0D\u53EF\u8BA1\u6570\u3002",
      \u7EC4\u5408\u5B50: "\u4E14(AND) \u6216(OR) \u975E(NOT) \u62EC\u53F7();\u4F18\u5148\u7EA7 \u975E > \u4E14 > \u6216",
      \u6210\u6D3E\u9608\u503C: {
        \u5B9A\u4E49: "\u8BE5\u4E94\u884C\u542B\u85CF\u5E72\u52A0\u6743\u5206 \u2265 3.0 \u4E14\u81F3\u5C11\u4E00\u5B57\u900F\u5E72",
        \u52A0\u6743\u53E3\u5F84: "\u5929\u5E72\u6BCF\u5B571;\u5730\u652F\u672C\u6C141\u3001\u4E2D\u6C140.5\u3001\u4F59\u6C140.3(\u4E0E countWuXing.withCangGan \u540C\u53E3\u5F84)",
        "\u26A0\u514D\u8D23": "\u5178\u7C4D\u53EA\u8BF4\u300C\u4E00\u6D3E\u5E9A\u8F9B\u300D,\u6CA1\u7ED9\u5224\u636E\u3002\u6B64\u9608\u503C\u662F\u3010\u6211\u4EEC\u7684\u64CD\u4F5C\u5316\u5B9A\u4E49\u3011,\u4E0D\u662F\u5178\u7C4D\u539F\u6587\u3002\u547D\u4E2D\u65F6\u4E00\u5E76\u8F93\u51FA\u4E94\u884C\u52A0\u6743\u5206,\u4FBF\u4E8E\u5BA1\u8BA1;\u6539\u9608\u503C=\u6539\u8BCD\u8868\u7248\u672C\u3002"
      },
      \u6052\u771F\u6052\u5047\u5B88\u5219: "\u6708\u4EE4\u85CF\u5E72\u662F\u6052\u5B9A\u7684(\u5BC5\u6052\u85CF\u4E19\u620A\u3001\u8FB0\u6052\u85CF\u7678\u3001\u5DF3\u6052\u85CF\u5E9A\u3001\u7533\u6052\u85CF\u5E9A\u58EC\u620A\u3001\u5348\u6052\u85CF\u4E01\u3001\u620C\u6052\u85CF\u620A)\u3002\u628A\u5178\u7C4D\u7684\u300C\u65E0\u4E19\u300D\u7167\u5B57\u9762\u5199\u6210\u5168\u76D8 \u65E0:\u4E19,\u5728\u5BC5\u6708\u3010\u6C38\u8FDC\u4E3A\u5047\u3011,\u6210\u6B7B\u6761\u4F8B;\u628A\u300C\u987B\u89C1\u620A\u571F\u300D\u5199\u6210 \u6709:\u620A,\u5728\u7533\u6708\u3010\u6C38\u8FDC\u4E3A\u771F\u3011,\u6761\u4EF6\u7B49\u4E8E\u6CA1\u5224\u3002\u4E24\u5934\u90FD\u662F\u5751,\u4E00\u5F8B\u6309\u3010\u5929\u5E72\u900F\u51FA\u3011\u5224\u5E76\u5199 \u5224\u5B9A\u5907\u6CE8\u3002test-tiaohou.ts \u7684 \u24D5 \u5BA1\u8BA1\u5BF9\u6BCF\u683C\u8DD1 4000 \u968F\u673A\u76D8,\u6B7B\u6761\u4F8B\u4E0E\u6052\u771F\u6761\u4F8B\u90FD\u4F1A FAIL\u3002",
      \u5173\u7CFB\u4E0E\u8574\u542B: "\u300C\u7EC6\u5316\u4E8E\u300D\u6709\u4E24\u79CD\u542B\u4E49\u76F8\u53CD\u7684\u5173\u7CFB:\u5B50\u96C6(\u6BCD\u5B50\u540C\u65F6\u547D\u4E2D,\u6863\u4F4D\u8BA1\u53CC\u8BA1) / \u4E92\u65A5(\u6BCD\u6761\u7684\u4F8B\u5916,\u6C38\u4E0D\u540C\u65F6\u547D\u4E2D)\u3002\u5173\u7CFB\u4E0E\u300C\u8574\u542B\u300D(\u672A\u58F0\u660E\u7EC6\u5316\u4E8E\u4F46\u5B9E\u4E3A\u771F\u5B50\u96C6)\u7531 fixtures/bless-tiaoli-relations.ts \u91C7\u6837\u7B97\u51FA\u5E76\u56DE\u5199,\u4E0D\u624B\u5199;\u6539\u4E86\u4EFB\u4F55\u300C\u82E5\u300D\u4E4B\u540E\u987B\u91CD\u8DD1\u8BE5\u811A\u672C,\u5426\u5219 test-tiaohou \u7684 \u24D6 \u4F1A FAIL\u3002",
      \u6863\u4F4D: "\u4E0A/\u4E2D/\u4E0B/\u5FCC \u2014\u2014 \u662F\u4E66\u7684\u5224\u65AD,\u4E0D\u662F\u6211\u4EEC\u7684\u5B9E\u8BC1\u3002\u89E3\u8BFB\u5C42\u5F15\u7528\u65F6\u987B\u8F6C\u5199\u4E3A\u503E\u5411\u6027\u8868\u8FF0(\u5DE5\u5355 2.6)\u3002"
    },
    _\u8986\u76D6: {
      \u5DF2\u5438\u6536\u884C: [
        "\u7532"
      ...
      \u5DF2\u5438\u6536\u683C\u6570: 12,
      \u603B\u683C\u6570: 120,
      \u8BF4\u660E: "\u5206\u6279\u5438\u6536(\u5DE5\u5355 v3.11 \xA72.5 M1~M4)\u3002\u672A\u5438\u6536\u7684\u683C\u6C42\u503C\u5668\u9759\u9ED8\u8FD4\u56DE\u7A7A,\u4E0D\u62A5\u9519\u3002"
    },
    "\u7532/\u5BC5": {
      \u6708: "\u6B63\u6708",
      \u53D6\u5E72: [
      ...
      \u6761\u4F8B: [
        ...
        },
        {
          ...
          \u6863: "\u4E0A",
          \u540D: "\u5BD2\u6728\u5411\u9633",
          \u610F\u8C61: "\u6839\u4E0B\u81EA\u6709\u6DA6\u6CFD\uFF0C\u5934\u4E0A\u4E00\u8F6E\u6696\u9633\uFF0C\u5411\u660E\u800C\u751F"
        },
        {
          id: "\u7532\u5BC5-\u65E0\u4E19\u7678",
          \u82E5: "\u975E \u900F:\u4E19 \u4E14 \u975E \u900F:\u7678",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u5982\u65E0\u4E19\u7678\u300D\u6309\u5929\u5E72\u4E0D\u900F\u5224:\u5BC5\u6708\u85CF\u5E72\u6052\u542B\u4E19,\u6309\u5168\u76D8\u53E3\u5F84\u6B64\u6761\u6C38\u4E0D\u547D\u4E2D\u3002\u524D\u4E24\u6761(\u53CC\u900F/\u7678\u85CF\u4E19\u900F)\u90FD\u4EE5\u900F\u4E3A\u51C6,\u6B64\u6761\u4F5C\u5176\u4F59\u9879\u624D\u81EA\u6D3D\u3002",
          \u5219: "\u5982\u65E0\u4E19\u7678\uFF0C\u5E73\u5E38\u4EBA\u4E5F",
          \u6863: "\u4E0B",
          \u540D: "\u6625\u5BD2\u65E0\u9633",
          \u610F\u8C61: "\u4F59\u5BD2\u672A\u9000\u800C\u4E0D\u89C1\u65E5\uFF0C\u6728\u6C14\u4E0D\u8212"
        ...
        {
          id: "\u6B63\u4E8C\u6708-\u6C34\u6CDB\u6728\u6D6E",
          \u82E5: "\u975E \u900F:\u4E19 \u4E14 \u975E \u900F:\u4E01 \u4E14 \u6210\u6D3E:\u58EC\u7678 \u4E14 \u975E \u900F:\u620A \u4E14 \u975E \u900F:\u5DF1",
          \u5224\u5B9A\u5907\u6CE8: "\u5178\u7C4D\u300C\u65E0X\u300D\u5728\u6B64\u6309\u3010\u5929\u5E72\u4E0D\u900F\u3011\u5224,\u4E0D\u6309\u3010\u5168\u76D8\u4E0D\u89C1\u3011\u5224\u2014\u2014\u672C\u683C\u6708\u4EE4\u85CF\u5E72\u91CC\u6052\u6709\u8BE5\u5B57(\u5982\u5BC5\u85CF\u4E19\u620A\u3001\u8FB0\u85CF\u7678\u3001\u5DF3\u85CF\u5E9A),\u6309\u5168\u76D8\u53E3\u5F84\u8BE5\u6761\u6C38\u4E0D\u53EF\u80FD\u547D\u4E2D,\u6210\u6B7B\u6761\u4F8B\u3002\u4E14\u539F\u6587\u8BED\u4E49\u662F\u300C\u65E0X\u6765\u5236/\u6765\u7834\u300D,\u5236\u4E0E\u7834\u987B\u900F\u5E72\u65B9\u8BBA\u3002",
          \u5219: "\u5982\u65E0\u4E19\u4E01\uFF0C\u4E00\u6D3E\u58EC\u7678\uFF0C\u53C8\u65E0\u620A\u5DF1\u5236\u4E4B\uFF0C\u540D\u6C34\u6CDB\u6728\u6D6E\uFF0C\u6B7B\u65E0\u68FA\u6901",
          \u6863: "\u5FCC",
          \u540D: "\u6C34\u6CDB\u6728\u6D6E",
          \u610F\u8C61: "\u6C34\u591A\u65E0\u5824\uFF0C\u6839\u4E0D\u7740\u571F\uFF0C\u5F62\u968F\u6D41\u8F6C",
          \u9002\u7528: "\u6B63\u4E8C\u6708\u5171\u6BB5"
        },
        {
          id: "\u6B63\u4E8C\u6708-\u8D22\u591A\u8EAB\u5F31",
          \u82E5: "\u6210\u6D3E:\u620A\u5DF1 \u4E14 \u4F1A\u5C40:\u91D1",
          \u5219: "\u5982\u4E00\u6D3E\u620A\u5DF1\uFF0C\u652F\u4F1A\u91D1\u5C40\uFF0C\u4E3A\u8D22\u591A\u8EAB\u5F31\uFF0C\u5BCC\u5C4B\u8D2B\u4EBA\uFF0C\u7EC8\u751F\u52B3\u82E6\uFF0C\u59BB\u665A\u5B50\u8FDF",
          \u6863: "\u5FCC",
          \u540D: "\u8D22\u591A\u8EAB\u5F31",
          \u610F\u8C61: "\u6EE1\u76EE\u7686\u8D22\u800C\u529B\u4E0D\u80FD\u4EFB\uFF0C\u5C4B\u534E\u800C\u4EBA\u8D2B",
          \u9002\u7528: "\u6B63\u4E8C\u6708\u5171\u6BB5"
        },
        {
          id: "\u6B63\u4E8C\u6708-\u6728\u706B\u901A\u660E",
          \u82E5: "\u65E0:\u5E9A \u4E14 \u900F:\u4E01",
          \u5219: "\u6216\u65E0\u5E9A\u91D1\uFF0C\u6709\u4E01\u900F\uFF0C\u4EA6\u5C5E\u6587\u661F\uFF0C\u4E3A\u6728\u706B\u901A\u660E\u4E4B\u8C61\uFF0C\u53C8\u540D\u4F24\u5B98\u751F\u8D22\u683C\uFF0C\u4E3B\u806A\u660E\u96C5\u79C0",
          \u6863: "\u4E0A",
          \u540D: "\u6728\u706B\u901A\u660E",
          \u610F\u8C61: "\u6625\u6728\u5F15\u4E01\uFF0C\u5149\u534E\u7531\u5185\u800C\u5916",
          \u9002\u7528: "\u6B63\u4E8C\u6708\u5171\u6BB5"
        ...
        },
        {
          id: "\u6B63\u4E8C\u6708-\u591A\u7678\u706D\u4E01",
          \u82E5: "\u6709\u6570:\u7678>=2 \u4E14 \u6709:\u4E01",
          \u5219: "\u6216\u67F1\u4E2D\u591A\u7678\uFF0C\u6ECB\u52A9\u6728\u795E\uFF0C\u4F24\u706D\u4E01\u706B\uFF0C\u5176\u4EBA\u5978\u96C4\u67AD\u9669\uFF0C\u66F9\u64CD\u4E4B\u5F92\uFF0C\u8A00\u6E05\u884C\u6D4A\uFF0C\u7B11\u91CC\u85CF\u5200",
          \u6863: "\u5FCC",
          \u540D: "\u591A\u7678\u706D\u4E01",
          \u610F\u8C61: "\u6C34\u76DB\u800C\u706B\u7184\uFF0C\u6728\u72EC\u8302\u800C\u65E0\u660E",
          \u9002\u7528: "\u6B63\u4E8C\u6708\u5171\u6BB5"
        ...
        {
          id: "\u6B63\u4E8C\u6708-\u6728\u88AB\u91D1\u4F24",
          \u82E5: "\u4F1A\u5C40:\u91D1 \u4E14 (\u900F:\u5E9A \u6216 \u900F:\u8F9B) \u4E14 \u975E \u900F:\u4E19 \u4E14 \u975E \u900F:\u4E01",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u82E5\u65E0\u4E19\u4E01\u7834\u91D1\u300D\u2014\u2014\u7834\u91D1\u987B\u4E19\u4E01\u900F\u5E72\u65B9\u8BBA;\u5BC5\u6708\u85CF\u5E72\u6052\u542B\u4E19,\u6309\u5168\u76D8\u53E3\u5F84\u6B64\u6761\u6C38\u4E0D\u547D\u4E2D\u3002",
          \u5219: "\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u591A\u900F\u5E9A\u8F9B\uFF0C\u6B64\u53C8\u4E0D\u5409\uFF0C\u53F7\u66F0\u6728\u88AB\u91D1\u4F24\uFF0C\u82E5\u65E0\u4E19\u4E01\u7834\u91D1\uFF0C\u5FC5\u4E3B\u6B8B\u75BE",
          \u6863: "\u5FCC",
          \u540D: "\u6728\u88AB\u91D1\u4F24",
          \u610F\u8C61: "\u91D1\u5C40\u5F53\u524D\u800C\u65E0\u706B\u89E3\uFF0C\u6728\u4F53\u53D7\u5236",
          \u9002\u7528: "\u6B63\u4E8C\u6708\u5171\u6BB5"
        },
        {
          id: "\u6B63\u4E8C\u6708-\u706B\u5C40\u6CC4\u6C14",
          \u82E5: "\u4F1A\u5C40:\u706B",
          \u5219: "\u6216\u652F\u6210\u706B\u5C40\uFF0C\u6CC4\u9732\u592A\u8FC7\uFF0C\u5B9A\u4E3B\u611A\u61E6\uFF0C\u5E38\u6709\u557E\u5527\u707E\u75C5\u7F20\u8EAB\uFF0C\u7EC8\u6709\u6697\u75BE",
          \u6863: "\u5FCC",
          \u540D: "\u6CC4\u6C14\u592A\u8FC7",
          \u610F\u8C61: "\u6728\u5C3D\u5316\u5149\uFF0C\u5F62\u795E\u4FF1\u8017",
          \u9002\u7528: "\u6B63\u4E8C\u6708\u5171\u6BB5"
        ...
        },
        {
          ...
          \u540D: "\u6C34\u5C40\u65E0\u5824",
          \u82E5: "\u4F1A\u5C40:\u6C34 \u4E14 \u975E \u900F:\u620A",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u5982\u65E0\u620A\u5236\u300D\u2014\u2014\u5236\u6C34\u987B\u620A\u900F\u5E72\u65B9\u8BBA;\u5BC5\u6708\u85CF\u5E72\u6052\u542B\u620A,\u6309\u5168\u76D8\u53E3\u5F84\u6B64\u6761\u6C38\u4E0D\u547D\u4E2D\u3002\u4E0E\u300C\u6C34\u5C40\u5F97\u620A\u300D(\u900F:\u620A)\u6070\u6210\u4E92\u8865\u3002",
          \u5219: "\u5982\u65E0\u620A\u5236\uFF0C\u4E0D\u4F46\u8D2B\u8D31\uFF0C\u4E14\u6B7B\u65E0\u68FA\u6728",
          \u6863: "\u5FCC",
          \u7EC6\u5316\u4E8E: "\u6B63\u4E8C\u6708-\u6C34\u5C40\u5F97\u620A",
          \u610F\u8C61: "\u6C34\u6F2B\u65E0\u5CB8\uFF0C\u6839\u8DB3\u96BE\u7ACB",
    ...
    "\u7532/\u536F": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) + \u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u674E\u94C1\u7B14\u5F71\u5370\u672C(\u7D20\u6750/xzb2100653) \u4E66\u987573/76/77(\u5F71\u5370\u62BD\u67E5\xB7\u6B63\u4E8C\u6708\u5171\u6BB5)",
      \u539F\u6587: "\u4E8C\u6708\u7532\u6728\uFF0C\u5E9A\u91D1\u5F97\u6240\uFF0C\u540D\u9633\u5203\u9A7E\u6740\uFF0C\u53EF\u4E91\u5C0F\u8D35\uFF0C\u5F02\u9014\u663E\u8FBE\uFF0C\u6216\u4E3B\u6B66\u804C\uFF0C\u4F46\u8981\u8D22\u8D44\u4E4B\u3002\u67F1\u4E2D\u9022\u624D\uFF0C\u82F1\u96C4\u72EC\u538B\u4E07\u4EBA\u3002\u82E5\u89C1\u7678\u6C34\uFF0C\u56F0\u4E86\u624D\u6740\uFF0C\u4E3B\u4E3A\u5149\u68CD\uFF0C\u91CD\u5203\u5FC5\u5B9A\u906D\u51F6\uFF0C\u6027\u60C5\u51F6\u66B4\u3002\u4E66\u66F0\uFF1A\u6728\u65FA\u5B9C\u706B\u4E4B\u5149\u8F89\uFF0C\u79CB\u95F1\u53EF\u8BD5\u3002\u6728\u5411\u6625\u751F\uFF0C\u5904\u4E16\u5B89\u7136\u6709\u5BFF\u3002\u65E5\u4E3B\u65E0\u4F9D\uFF0C\u5374\u559C\u8FD0\u884C\u624D\u5730\u3002\uFF0F\u6B63\u4E8C\u6708\u7532\u6728\uFF0C\u7D20\u65E0\u53D6\u4ECE\u624D\u4ECE\u6740\u4ECE\u5316\u4E4B\u7406\u3002\u6216\u4E00\u6D3E\u5E9A\u8F9B\uFF0C\u4E3B\u4E00\u751F\u52B3\u82E6\uFF0C\u514B\u5B50\u5211\u59BB\uFF0C\u518D\u652F\u4F1A\u91D1\u5C40\uFF0C\u975E\u8D2B\u5373\u592D\u3002\u5982\u65E0\u4E19\u4E01\uFF0C\u4E00\u6D3E\u58EC\u7678\uFF0C\u53C8\u65E0\u620A\u5DF1\u5236\u4E4B\uFF0C\u540D\u6C34\u6CDB\u6728\u6D6E\uFF0C\u6B7B\u65E0\u68FA\u6901\u3002\u5982\u4E00\u6D3E\u620A\u5DF1\uFF0C\u652F\u4F1A\u91D1\u5C40\uFF0C\u4E3A\u8D22\u591A\u8EAB\u5F31\uFF0C\u5BCC\u5C4B\u8D2B\u4EBA\uFF0C\u7EC8\u751F\u52B3\u82E6\uFF0C\u59BB\u665A\u5B50\u8FDF\u3002\u6216\u65E0\u5E9A\u91D1\uFF0C\u6709\u4E01\u900F\uFF0C\u4EA6\u5C5E\u6587\u661F\uFF0C\u4E3A\u6728\u706B\u901A\u660E\u4E4B\u8C61\uFF0C\u53C8\u540D\u4F24\u5B98\u751F\u8D22\u683C\uFF0C\u4E3B\u806A\u660E\u96C5\u79C0\u3002\u4E00\u89C1\u7678\u6C34\u4F24\u4E01\uFF0C\u4F46\u4F5C\u539A\u9053\u8FC2\u5112\u3002\u6216\u67F1\u4E2D\u591A\u7678\uFF0C\u6ECB\u52A9\u6728\u795E\uFF0C\u4F24\u706D\u4E01\u706B\uFF0C\u5176\u4EBA\u5978\u96C4\u67AD\u9669\uFF0C\u66F9\u64CD\u4E4B\u5F92\uFF0C\u8A00\u6E05\u884C\u6D4A\uFF0C\u7B11\u91CC\u85CF\u5200\u3002\u82E5\u5E9A\u7533\u3001\u620A\u5BC5\u3001\u7532\u5BC5\u3001\u4E19\u5BC5\u3002\u4E00\u884C\u91D1\u6C34\u8FD0\uFF0C\u53D1\u8FDB\u58EB\u3002\u6216\u7532\u5348\u65E5\u5E9A\u5348\u65F6\uFF0C\u6B64\u4EBA\u5FC5\u8D35\u3002\u4F46\u8981\u597D\u8FD0\u76F8\u50AC\uFF0C\u4E0D\u5B9C\u5236\u4E86\u5E9A\u4E01\u3002\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u591A\u900F\u5E9A\u8F9B\uFF0C\u6B64\u53C8\u4E0D\u5409\uFF0C\u53F7\u66F0\u6728\u88AB\u91D1\u4F24\uFF0C\u82E5\u65E0\u4E19\u4E01\u7834\u91D1\uFF0C\u5FC5\u4E3B\u6B8B\u75BE\u3002\u6216\u652F\u6210\u706B\u5C40\uFF0C\u6CC4\u9732\u592A\u8FC7\uFF0C\u5B9A\u4E3B\u611A\u61E6\uFF0C\u5E38\u6709\u557E\u5527\u707E\u75C5\u7F20\u8EAB\uFF0C\u7EC8\u6709\u6697\u75BE\u3002\u652F\u6210\u6728\u5C40\uFF0C\u5F97\u5E9A\u4E3A\u8D35\uFF0C\u65E0\u5E9A\u5FC5\u51F6\uFF0C\u82E5\u975E\u50E7\u9053\uFF0C\u7537\u4E3B\u9CCF\u5B64\uFF0C\u5973\u4E3B\u5BE1\u72EC\u3002\u652F\u6210\u6C34\u5C40\uFF0C\u620A\u900F\u4E3A\u8D35\uFF0C\u5982\u65E0\u620A\u5236\uFF0C\u4E0D\u4F46\u8D2B\u8D31\uFF0C\u4E14\u6B7B\u65E0\u68FA\u6728\u3002\u6545\u4E66\u66F0\uFF1A\u7532\u6728\u82E5\u65E0\u6839\uFF0C\u5168\u8D56\u7533\u5B50\u8FB0\uFF0C\u5E72\u5F97\u624D\u6740\u900F\uFF0C\u5E73\u6B65\u4E0A\u9752\u4E91\u3002\u51E1\u4E09\u6625\u7532\u6728\uFF0C\u7528\u5E9A\u8005\uFF0C\u571F\u4E3A\u59BB\uFF0C\u91D1\u4E3A\u5B50\u3002\u7528\u4E01\u8005\uFF0C\u6728\u4E3A\u59BB\uFF0C\u706B\u4E3A\u5B50\u3002\u603B\u4E4B\u6B63\u4E8C\u6708\u7532\u6728\uFF0C\u6709\u5E9A\u620A\u8005\u4E0A\u547D\u3002\u5982\u6709\u4E01\u900F\uFF0C\u5927\u5BCC\u5927\u8D35\u4E4B\u547D\u4E5F\u3002",
      \u5F71\u5370\u672C\u62BD\u67E5: {
        \u4E66\u9875: "73/76/77(PDF 38/40)",
        \u7ED3\u8BBA: "\u5927\u4F53\u4E00\u81F4\xB7\u63AA\u8F9E\u6709\u51FA\u5165",
        \u8BB0\u5F55: "\u5F71\u5370\u672C\u6B63\u4E8C\u6708\u6BB5\u4F5C\u300C\u6216\u7121\u5E9A\u91D1\uFF0C\u6709\u4E01\u900F\uFF0C\u4EA6\u5C6C\u6587\u661F\uFF0C\u70BA\u6728\u706B\u901A\u660E\u4E4B\u8C61\uFF0C\u53C8\u540D\u50B7\u5B98\u751F\u8CA1\u683C\uFF0C\u4E3B\u8070\u660E\u96C5\u79C0\u3002\u4E00\u898B\u7678\u6C34\u50B7\u4E01\uFF0C\u4F46\u4F5C\u539A\u9053\u8FC2\u5112\u3002\u6216\u67F1\u4E2D\u591A\u7678\uFF0C\u6ECB\u52A9\u6728\u795E\uFF0C\u50B7\u6EC5\u4E01\u706B\uFF0C\u5176\u4EBA\u5978\u96C4\u689F\u96AA\uFF0C\u8A00\u6E05\u884C\u6FC1\u4E4B\u6D41\u300D\u2014\u2014\u6761\u4EF6\u4E0E\u5224\u65AD\u540C\u77E5\u781A\u658B,\u552F\u77E5\u781A\u658B\u591A\u300C\u66F9\u64CD\u4E4B\u5F92\u2026\u7B11\u88CF\u85CF\u5200\u300D\u4E00\u53E5\u3002\u6761\u4F8B\u4E0D\u53D7\u5F71\u54CD\u3002"
      },
      \u6761\u4F8B: [
        {
          id: "\u7532\u536F-\u9633\u5203\u9A7E\u6740",
          \u82E5: "\u900F:\u5E9A",
          \u5219: "\u5E9A\u91D1\u5F97\u6240\uFF0C\u540D\u9633\u5203\u9A7E\u6740\uFF0C\u53EF\u4E91\u5C0F\u8D35\uFF0C\u5F02\u9014\u663E\u8FBE\uFF0C\u6216\u4E3B\u6B66\u804C",
          \u6863: "\u4E2D",
          \u540D: "\u9633\u5203\u9A7E\u6740",
          \u610F\u8C61: "\u5203\u81EA\u6709\u950B\uFF0C\u5F97\u6740\u76F8\u9A7E\u5219\u53EF\u4EFB\u91CD"
        },
        {
          ...
          \u540D: "\u5203\u6740\u5F97\u8D22",
          \u82E5: "\u900F:\u5E9A \u4E14 \u5341\u795E:\u8D22 \u4E14 \u65E0:\u7678",
          \u5224\u5B9A\u5907\u6CE8: "\u539F\u6587\u300C\u4F46\u8981\u8D22\u8D44\u4E4B\u2026\u67F1\u4E2D\u9022\u624D\uFF0C\u82F1\u96C4\u72EC\u538B\u4E07\u4EBA\u3002\u3010\u82E5\u89C1\u7678\u6C34\u3011\uFF0C\u56F0\u4E86\u624D\u6740\u300D\u2014\u2014\u5F8C\u53E5\u662F\u524D\u53E5\u7684\u4F8B\u5916,\u6545\u52A0\u6392\u4ED6\u5B88\u95E8,\u5426\u5219\u89C1\u7678\u4E4B\u76D8\u4F1A\u540C\u65F6\u6253\u51FA[\u4E0A]\u82F1\u96C4\u72EC\u538B\u4E07\u4EBA\u4E0E[\u5FCC]\u4E3B\u4E3A\u5149\u68CD\u4E24\u6761\u76F8\u53CD\u8BBA\u65AD(\u7532/\u9149\u300C\u4E19\u900F\u65E0\u7678\u300D\u5DF2\u5982\u6B64\u5904\u7406)\u3002",
          \u5219: "\u4F46\u8981\u8D22\u8D44\u4E4B\u3002\u67F1\u4E2D\u9022\u624D\uFF0C\u82F1\u96C4\u72EC\u538B\u4E07\u4EBA",
          \u6863: "\u4E0A",
          \u7EC6\u5316\u4E8E: "\u7532\u536F-\u9633\u5203\u9A7E\u6740",
          \u610F\u8C61: "\u950B\u5203\u6709\u7CAE\u8349\u63A5\u6D4E\uFF0C\u65B9\u80FD\u4E45\u6218",
    ...
    "\u7532/\u8FB0": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) + \u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u674E\u94C1\u7B14\u5F71\u5370\u672C(\u7D20\u6750/xzb2100653) \u4E66\u987586/87/88(\u5F71\u5370\u62BD\u67E5\xB7\u4E00\u5904\u4E24\u7CFB\u5206\u6B67)",
      \u539F\u6587: "\u4E09\u6708\u7532\u6728\uFF0C\u6728\u6C14\u76F8\u7AED\u3002\u5148\u53D6\u5E9A\u91D1\uFF0C\u6B21\u7528\u58EC\u6C34\u3002\u5E9A\u58EC\u4E24\u900F\uFF0C\u4E00\u699C\u582A\u56FE\u3002\u4F46\u8981\u8FD0\u7528\u76F8\u751F\uFF0C\u98CE\u6C34\u9634\u5FB7\uFF0C\u65B9\u8BB8\u5BCC\u8D35\u3002\u6216\u89C1\u4E00\u4E8C\u5E9A\u91D1\uFF0C\u72EC\u53D6\u58EC\u6C34\u3002\u58EC\u900F\u6E05\u79C0\u4E4B\u4EBA\uFF0C\u624D\u5B66\u5FC5\u5BCC\u3002\u6216\u5929\u5E72\u900F\u51FA\u4E8C\u4E19\uFF0C\u5E9A\u85CF\u652F\u4E0B\uFF0C\u6B64\u949D\u65A7\u65E0\u94A2\uFF0C\u5BCC\u8D35\u96BE\u6C42\u3002\u82E5\u6709\u58EC\u7678\u7834\u706B\uFF0C\u582A\u4F5C\u79C0\u624D\u3002\u6216\u67F1\u4E2D\u5168\u65E0\u4E00\u6C34\uFF0C\u620A\u5DF1\u900F\u5E72\uFF0C\u652F\u6210\u571F\u5C40\uFF0C\u53C8\u4F5C\u5F03\u547D\u4ECE\u624D\uFF0C\u56E0\u4EBA\u800C\u81F4\u5BCC\u8D35\uFF0C\u59BB\u5B50\u6709\u80FD\u3002\u6216\u89C1\u620A\u5DF1\uFF0C\u53CA\u6BD4\u52AB\u591A\u8005\uFF0C\u540D\u4E3A\u6742\u6C14\u593A\u624D\uFF0C\u6B64\u4EBA\u52B3\u788C\u5230\u8001\uFF0C\u65E0\u9A6D\u5185\u4E4B\u6743\u3002\u5973\u547D\u5408\u6B64\uFF0C\u5973\u638C\u7537\u6743\uFF0C\u8D24\u80FD\u5185\u52A9\uFF0C\u82E5\u6BD4\u52AB\u91CD\u89C1\uFF0C\u6DEB\u6076\u4E0D\u582A\u3002\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u65B9\u53EF\u7528\u4E01\uFF0C\u4E0D\u7136\uFF0C\u4E09\u6708\u65E0\u7528\u4E01\u4E4B\u6CD5\u3002\u60DF\u6709\u5148\u5E9A\u5F8C\u58EC\u53D6\u7528\u3002",
      \u5F71\u5370\u672C\u62BD\u67E5: {
        \u4E66\u9875: "86/87/88(PDF 45/46)",
        \u7ED3\u8BBA: "\u2696 \u4E00\u5904\u5206\u6B67",
        \u8BB0\u5F55: "\u5F71\u5370\u672C\u4F5C\u300C\u6216\u898B\u6BD4\u80A9\u53CA\u4E59\u591A\u8005\uFF0C\u540D\u70BA\u6DF7\u596A\u8CA1\u795E\u300D,\u77E5\u781A\u658B\u4F5C\u300C\u6216\u89C1\u620A\u5DF1\uFF0C\u53CA\u6BD4\u52AB\u591A\u8005\uFF0C\u540D\u4E3A\u6742\u6C14\u593A\u624D\u300D\u2014\u2014\u82E5\u4E0E\u540D\u7686\u4E0D\u540C,\u4E24\u7248\u5E76\u5B58\u4E0D\u62E9\u4E00\u3002\u53E6:\u5F71\u5370\u672C\u300C\u6216\u5929\u5E72\u900F\u4E8C\u4E19\uFF0C\u5E9A\u85CF\u652F\u4E0B\uFF0C\u65AF\u70BA\u920D\u65A7\u7121\u92FC\uFF0C\u7121\u6709\u5B78\u554F\uFF0C\u5BCC\u8CB4\u8271\u96E3\uFF0C\u5F97\u58EC\u7678\u7834\u706B\u8005\u79C0\u58EB\u300D\u4E0E\u77E5\u781A\u658B\u540C\u4E49\u3002\u300C\u7E3D\u4E4B\u4E09\u6708\u7532\u6728\uFF0C\u5148\u5E9A\u6B21\u58EC\uFF0C\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u65B9\u53EF\u7528\u4E01\uFF0C\u4E0D\u7136\uFF0C\u4E09\u6708\u7121\u7528\u4E01\u4E4B\u6CD5\u300D\u4E24\u672C\u4E00\u81F4\u3002"
      },
      \u4E24\u7CFB\u5206\u6B67: [
        {
          \u6761\u4F8B: "\u7532\u8FB0-\u6742\u6C14\u593A\u8D22",
          \u7A77\u901A\u5B9D\u9274\u7CFB: "\u6216\u89C1\u620A\u5DF1\uFF0C\u53CA\u6BD4\u52AB\u591A\u8005\uFF0C\u540D\u4E3A\u6742\u6C14\u593A\u624D",
          \u9020\u5316\u5143\u94A5\u7CFB: "\u6216\u898B\u6BD4\u80A9\u53CA\u4E59\u591A\u8005\uFF0C\u540D\u70BA\u6DF7\u596A\u8CA1\u795E",
          \u8BF4\u660E: "\u82E5(\u6761\u4EF6\u5B57)\u4E0E\u540D\u7686\u4E0D\u540C;\u672C\u8868\u4EE5\u77E5\u781A\u658B\u672C\u4E3A\u6761\u4F8B\u4E3B\u4F53,\u5F71\u5370\u672C\u5F02\u6587\u5E76\u5B58\u4E8E\u6B64,\u4E0D\u64C5\u81EA\u62E9\u4E00\u3002"
        }
      ],
      \u6761\u4F8B: [
        {
        ...
        {
          id: "\u7532\u8FB0-\u5F03\u547D\u4ECE\u8D22",
          \u82E5: "\u652F\u4E94\u884C\u6570:\u6C34=0 \u4E14 \u975E \u900F:\u58EC \u4E14 \u975E \u900F:\u7678 \u4E14 (\u900F:\u620A \u6216 \u900F:\u5DF1) \u4E14 \u4F1A\u5C40:\u571F",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u67F1\u4E2D\u5168\u65E0\u4E00\u6C34\u300D\u6309\u5E72\u652F\u5B57\u9762\u65E0\u6C34\u5224(\u5929\u5E72\u4E0D\u89C1\u58EC\u7678 + \u5730\u652F\u65E0\u4EA5\u5B50):\u8FB0\u4E4B\u4F59\u6C14\u6052\u542B\u7678,\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u6761\u6C38\u4E0D\u547D\u4E2D\u3002",
          \u5219: "\u6216\u67F1\u4E2D\u5168\u65E0\u4E00\u6C34\uFF0C\u620A\u5DF1\u900F\u5E72\uFF0C\u652F\u6210\u571F\u5C40\uFF0C\u53C8\u4F5C\u5F03\u547D\u4ECE\u624D\uFF0C\u56E0\u4EBA\u800C\u81F4\u5BCC\u8D35\uFF0C\u59BB\u5B50\u6709\u80FD",
          \u6863: "\u4E2D",
          \u540D: "\u5F03\u547D\u4ECE\u8D22",
          \u610F\u8C61: "\u5DF1\u529B\u65E2\u5FAE\uFF0C\u968F\u52BF\u800C\u884C\u53CD\u5F97\u501A\u6258"
        },
        {
          id: "\u7532\u8FB0-\u6742\u6C14\u593A\u8D22",
          \u82E5: "(\u900F:\u620A \u6216 \u900F:\u5DF1) \u4E14 \u5341\u795E\u6570:\u6BD4\u52AB>=2",
          \u5224\u5B9A\u5907\u6CE8: "\u672C\u683C\u6708\u4EE4\u85CF\u5E72\u6052\u542B\u8BE5\u5B57(\u8FB0\u85CF\u620A),\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u5408\u53D6\u9879\u6052\u771F\u3001\u539F\u6587\u5199\u660E\u7684\u6761\u4EF6\u4E00\u6B21\u4E5F\u4E0D\u4F1A\u88AB\u68C0\u9A8C,\u6545\u6309\u3010\u5929\u5E72\u900F\u51FA\u3011\u5224\u3002\u26A0\u8FD9\u662F\u6211\u65B9\u7684\u64CD\u4F5C\u5316\u6536\u7A84,\u975E\u5178\u7C4D\u539F\u6587\u3002",
          \u5219: "\u540D\u4E3A\u6742\u6C14\u593A\u624D\uFF0C\u6B64\u4EBA\u52B3\u788C\u5230\u8001\uFF0C\u65E0\u9A6D\u5185\u4E4B\u6743",
          \u6863: "\u5FCC",
          \u540D: "\u6742\u6C14\u593A\u8D22",
          \u610F\u8C61: "\u4F17\u624B\u5206\u4E00\u4EFD\u53E3\u7CAE\uFF0C\u5FD9\u800C\u65E0\u4E3B",
          \u4E24\u7CFB\u5206\u6B67: "\u5F71\u5370\u672C\u4F5C\u300C\u6216\u898B\u6BD4\u80A9\u53CA\u4E59\u591A\u8005\uFF0C\u540D\u70BA\u6DF7\u596A\u8CA1\u795E\u300D"
        },
        {
          id: "\u7532\u8FB0-\u91D1\u5C40\u7528\u4E01",
          \u82E5: "\u4F1A\u5C40:\u91D1",
          \u5219: "\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u65B9\u53EF\u7528\u4E01\uFF0C\u4E0D\u7136\uFF0C\u4E09\u6708\u65E0\u7528\u4E01\u4E4B\u6CD5",
          \u6863: "\u4E2D",
          \u540D: "\u91D1\u5C40\u65B9\u53EF\u7528\u4E01",
          \u610F\u8C61: "\u91D1\u805A\u6210\u5BD2\uFF0C\u975E\u706B\u4E0D\u878D"
        }
    ...
    "\u7532/\u5DF3": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) + \u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u674E\u94C1\u7B14\u5F71\u5370\u672C(\u7D20\u6750/xzb2100653) \u4E66\u987590/91(\u5F71\u5370\u62BD\u67E5\xB7\u4E09\u590F\u603B\u8BBA\u6709\u5206\u6B67)",
      \u539F\u6587: "\u56DB\u6708\u7532\u6728\u9000\u6C14\uFF0C\u4E19\u706B\u53F8\u6743\uFF0C\u5148\u7678\u5F8C\u4E01\u3002\u5E9A\u91D1\u592A\u591A\uFF0C\u7532\u53CD\u53D7\u75C5\u3002\u82E5\u5F97\u58EC\u6C34\uFF0C\u65B9\u914D\u5F97\u4E2D\u548C\uFF0C\u6B64\u4EBA\u6027\u597D\u6E05\u9AD8\uFF0C\u5047\u88C5\u5BCC\u8D35\u3002\u5982\u4E00\u5E9A\u4E8C\u4E19\uFF0C\u7A0D\u6709\u5BCC\u8D35\u3002\u91D1\u591A\u706B\u591A\uFF0C\u53C8\u4E3A\u4E0B\u683C\u3002\u6216\u7678\u4E01\u4E0E\u5E9A\u9F50\u900F\u5929\u5E72\uFF0C\u6B64\u547D\u53EF\u8A00\u79D1\u7532\uFF0C\u5373\u98CE\u6C34\u6D45\u8584\uFF0C\u4EA6\u6709\u9009\u62D4\u4E4B\u624D\u3002\u7678\u6C34\u4E0D\u51FA\uFF0C\u867D\u6709\u5E9A\u91D1\u4E01\u706B\uFF0C\u4E0D\u8FC7\u5BCC\u4E2D\u53D6\u8D35\u5F02\u9014\u5B98\u804C\u800C\u5DF2\u3002\u58EC\u900F\u53EF\u4E91\u4E00\u5BCC\uFF0C\u82E5\u5168\u65E0\u70B9\u6C34\uFF0C\u53C8\u65E0\u5E9A\u91D1\u4E01\u706B\uFF0C\u4E00\u6D3E\u4E19\u620A\uFF0C\u6B64\u65E0\u7528\u4E4B\u4EBA\u4E5F\u3002",
      \u5F71\u5370\u672C\u62BD\u67E5: {
        \u4E66\u9875: "90/91(PDF 47)",
        \u7ED3\u8BBA: "\u2696 \u603B\u8BBA\u5C42\u5206\u6B67",
        \u8BB0\u5F55: "\u5F71\u5370\u672C\u7B2C\u516B\u7BC0\u4E09\u590F\u7532\u6728\u7E3D\u8AD6\u5F00\u7BC7\u4F5C\u300C\u4E09\u590F\u7532\u6728\uFF0C\u7528\u795E\u5148\u4E01\u6B21\u5E9A\uFF0C\u7121\u4E01\u4E0D\u5F97\u901A\u660E\uFF0C\u7121\u5E9A\u5288\u7532\uFF0C\u4E01\u706B\u4E0D\u71C3\u300D\u2014\u2014\u5B63\u9996\u603B\u7EB2\u4E0D\u63D0\u7678;\u77E5\u781A\u658B\u9010\u6708\u4F5C\u300C\u56DB\u6708\u7532\u6728\u9000\u6C14\uFF0C\u4E19\u706B\u53F8\u6743\uFF0C\u5148\u7678\u5F8C\u4E01\u300D\u3002\u4EC5\u6838\u5230\u56DB\u6708\u689D\u9996\u53E5\u300C\u56DB\u6708\u6728\u6C23\u5DF2\u9000\uFF0C\u4E19\u706B\u53F8\u6B0A\u300D,\u9010\u6708\u6B63\u6587\u672A\u901A\u8BFB,\u6545\u53EA\u767B\u8BB0\u4E0D\u5B9A\u6848\u3002"
      },
      \u4E24\u7CFB\u5206\u6B67: [
        {
          \u6761\u4F8B: "(\u5B63\u9996\u603B\u7EB2)",
          \u7A77\u901A\u5B9D\u9274\u7CFB: "\u56DB\u6708\u7532\u6728\u9000\u6C14\uFF0C\u4E19\u706B\u53F8\u6743\uFF0C\u5148\u7678\u5F8C\u4E01",
          \u9020\u5316\u5143\u94A5\u7CFB: "\u4E09\u590F\u7532\u6728\uFF0C\u7528\u795E\u5148\u4E01\u6B21\u5E9A\uFF0C\u7121\u4E01\u4E0D\u5F97\u901A\u660E\uFF0C\u7121\u5E9A\u5288\u7532\uFF0C\u4E01\u706B\u4E0D\u71C3",
          \u8BF4\u660E: "\u603B\u7EB2\u5C42\u53D6\u7528\u5148\u5F8C\u4E0D\u540C(\u7678 vs \u4E01\u5E9A)\u3002\u5F71\u5370\u672C\u9010\u6708\u6B63\u6587\u672A\u901A\u8BFB,\u672C\u8F6E\u4E0D\u5B9A\u6848,\u767B\u8BB0\u5F85 M5 \u590D\u6838\u3002"
        }
      ],
      \u6761\u4F8B: [
        {
          id: "\u7532\u5DF3-\u7678\u4E01\u5E9A\u9F50\u900F",
          \u82E5: "\u900F:\u7678 \u4E14 \u900F:\u4E01 \u4E14 \u900F:\u5E9A",
          \u5219: "\u6216\u7678\u4E01\u4E0E\u5E9A\u9F50\u900F\u5929\u5E72\uFF0C\u6B64\u547D\u53EF\u8A00\u79D1\u7532\uFF0C\u5373\u98CE\u6C34\u6D45\u8584\uFF0C\u4EA6\u6709\u9009\u62D4\u4E4B\u624D",
          \u6863: "\u4E0A",
          \u540D: "\u7678\u4E01\u5E9A\u9F50\u900F",
          \u610F\u8C61: "\u6691\u6728\u5F97\u6DA6\u3001\u5F97\u660E\u3001\u5F97\u88C1\uFF0C\u4E09\u8005\u4FF1\u5907"
        },
        {
          id: "\u7532\u5DF3-\u7678\u6C34\u4E0D\u51FA",
          \u82E5: "\u65E0:\u7678 \u4E14 \u900F:\u5E9A \u4E14 \u6709:\u4E01",
          \u5224\u5B9A\u5907\u6CE8: "\u672C\u683C\u6708\u4EE4\u85CF\u5E72\u6052\u542B\u8BE5\u5B57(\u5DF3\u85CF\u5E9A),\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u5408\u53D6\u9879\u6052\u771F\u3001\u539F\u6587\u5199\u660E\u7684\u6761\u4EF6\u4E00\u6B21\u4E5F\u4E0D\u4F1A\u88AB\u68C0\u9A8C,\u6545\u6309\u3010\u5929\u5E72\u900F\u51FA\u3011\u5224\u3002\u26A0\u8FD9\u662F\u6211\u65B9\u7684\u64CD\u4F5C\u5316\u6536\u7A84,\u975E\u5178\u7C4D\u539F\u6587\u3002",
          \u5219: "\u7678\u6C34\u4E0D\u51FA\uFF0C\u867D\u6709\u5E9A\u91D1\u4E01\u706B\uFF0C\u4E0D\u8FC7\u5BCC\u4E2D\u53D6\u8D35\u5F02\u9014\u5B98\u804C\u800C\u5DF2",
          \u6863: "\u4E2D",
          \u540D: "\u4E4F\u7678",
          \u610F\u8C61: "\u6709\u706B\u6709\u65A7\u800C\u65E0\u6C34\uFF0C\u6691\u4E2D\u505A\u5DE5\uFF0C\u5F97\u5229\u4E0D\u5F97\u6E05"
        },
        {
          id: "\u7532\u5DF3-\u5E9A\u591A\u6728\u75C5",
          \u82E5: "\u900F\u6570:\u5E9A>=2",
        ...
        {
          id: "\u7532\u5DF3-\u65E0\u7528\u4E4B\u4EBA",
          \u82E5: "\u652F\u4E94\u884C\u6570:\u6C34=0 \u4E14 \u975E \u900F:\u58EC \u4E14 \u975E \u900F:\u7678 \u4E14 \u975E \u900F:\u5E9A \u4E14 \u975E \u900F:\u4E01 \u4E14 \u900F:\u4E19 \u4E14 \u900F:\u620A",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u5168\u65E0\u70B9\u6C34\uFF0C\u53C8\u65E0\u5E9A\u91D1\u4E01\u706B\u300D\u6309\u5E72\u652F\u5B57\u9762/\u5929\u5E72\u4E0D\u900F\u5224:\u5DF3\u4E4B\u4E2D\u6C14\u6052\u542B\u5E9A,\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u6761\u6C38\u4E0D\u547D\u4E2D\u3002",
          \u5219: "\u82E5\u5168\u65E0\u70B9\u6C34\uFF0C\u53C8\u65E0\u5E9A\u91D1\u4E01\u706B\uFF0C\u4E00\u6D3E\u4E19\u620A\uFF0C\u6B64\u65E0\u7528\u4E4B\u4EBA\u4E5F",
          \u6863: "\u4E0B",
          \u540D: "\u4E19\u620A\u65E0\u6C34",
          \u610F\u8C61: "\u70C8\u65E5\u7126\u571F\uFF0C\u6728\u65E0\u6240\u8D44"
        }
    ...
    "\u7532/\u5348": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u4E09\u590F\u7532\u6728\xB7\u4E94\u516D\u6708\u6BB5",
      \u539F\u6587: "\u4E94\u516D\u6708\u7532\u6728\uFF0C\u6728\u6027\u865A\u7126\uFF0C\u4E00\u7406\u5171\u63A8\u3002\u4E94\u6708\u5148\u7678\u5F8C\u4E01\u5E9A\u91D1\u6B21\u4E4B\u3002\u6216\u4E94\u6708\u4E4F\u7678\uFF0C\u7528\u4E01\u4EA6\u53EF\uFF0C\u8981\u8FD0\u884C\u5317\u5730\u4E3A\u96B9\u3002\u4E94\u516D\u6708\u7532\u6728\uFF0C\u6728\u76DB\u5148\u5E9A\uFF0C\u5E9A\u76DB\u5148\u4E01\u3002\u4E94\u6708\u7678\u5E9A\u4E24\u900F\uFF0C\u4E3A\u4E0A\u4E0A\u4E4B\u683C\u3002\u7528\u795E\u65E2\u900F\uFF0C\u6728\u706B\u901A\u660E\uFF0C\u81EA\u7136\u5927\u5BCC\u5927\u8D35\u3002\u6216\u4E01\u706B\u592A\u591A\uFF0C\u7678\u6C34\u4EA6\u591A\uFF0C\u53CD\u4F5C\u5E73\u4EBA\u3002\u82E5\u67F1\u4E2D\u591A\u91D1\uFF0C\u540D\u66F0\u6740\u91CD\u8EAB\u8F7B\uFF0C\u5148\u5BCC\u5F8C\u8D2B\uFF0C\u8FD0\u4E0D\u76F8\u6276\uFF0C\u975E\u8D2B\u5373\u592D\u3002\u6216\u5E9A\u591A\uFF0C\u6709\u4E00\u4E8C\u4E19\u4E01\u5236\u4F0F\uFF0C\u53C8\u6709\u58EC\u7678\u900F\u5E72\uFF0C\u6CC4\u91D1\u4E4B\u6C14\uFF0C\u6B64\u53C8\u4E3A\u5148\u8D2B\u5F8C\u5BCC\u3002\u6216\u6EE1\u67F1\u4E19\u706B\uFF0C\u53C8\u52A0\u4E01\u706B\uFF0C\u4E0D\u89C1\u5B98\u6740\uFF0C\u8C13\u4E4B\u4F24\u5B98\u4F24\u5C3D\u6700\u4E3A\u5947\uFF0C\u53CD\u6210\u6E05\u8D35\uFF0C\u5B9A\u4E3B\u624D\u5B66\u8FC7\u4EBA\uFF0C\u79D1\u7532\u6709\u671B\uFF0C\u4F46\u5C81\u8FD0\u4E0D\u5B9C\u89C1\u6C34\u3002\u82E5\u67F1\u4E2D\u6709\u58EC\u6C34\uFF0C\u8FD0\u53C8\u9022\u6C34\uFF0C\u5FC5\u8D2B\u592D\u6B7B\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u5348-\u7678\u5E9A\u4E24\u900F",
          \u82E5: "\u900F:\u7678 \u4E14 \u900F:\u5E9A",
        ...
        },
        {
          id: "\u7532\u5348-\u4E4F\u7678\u7528\u4E01",
          \u82E5: "\u65E0:\u7678 \u4E14 \u900F:\u4E01",
          \u5224\u5B9A\u5907\u6CE8: "\u672C\u683C\u6708\u4EE4\u85CF\u5E72\u6052\u542B\u8BE5\u5B57(\u5348\u85CF\u4E01),\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u5408\u53D6\u9879\u6052\u771F\u3001\u539F\u6587\u5199\u660E\u7684\u6761\u4EF6\u4E00\u6B21\u4E5F\u4E0D\u4F1A\u88AB\u68C0\u9A8C,\u6545\u6309\u3010\u5929\u5E72\u900F\u51FA\u3011\u5224\u3002\u26A0\u8FD9\u662F\u6211\u65B9\u7684\u64CD\u4F5C\u5316\u6536\u7A84,\u975E\u5178\u7C4D\u539F\u6587\u3002",
          \u5219: "\u6216\u4E94\u6708\u4E4F\u7678\uFF0C\u7528\u4E01\u4EA6\u53EF\uFF0C\u8981\u8FD0\u884C\u5317\u5730\u4E3A\u96B9",
          \u6863: "\u4E2D",
          \u540D: "\u4E4F\u7678\u7528\u4E01",
          \u610F\u8C61: "\u65E0\u6CC9\u53EF\u6C72\uFF0C\u53EA\u5F97\u5C31\u706B\u6210\u660E\uFF0C\u987B\u5F85\u5317\u5730\u4E4B\u51C9"
        },
        {
          id: "\u7532\u5348-\u6740\u91CD\u8EAB\u8F7B",
          \u82E5: "\u6210\u6D3E:\u5E9A\u8F9B",
          \u5219: "\u82E5\u67F1\u4E2D\u591A\u91D1\uFF0C\u540D\u66F0\u6740\u91CD\u8EAB\u8F7B\uFF0C\u5148\u5BCC\u5F8C\u8D2B\uFF0C\u8FD0\u4E0D\u76F8\u6276\uFF0C\u975E\u8D2B\u5373\u592D",
          \u6863: "\u5FCC",
          \u540D: "\u6740\u91CD\u8EAB\u8F7B",
          \u610F\u8C61: "\u62C5\u5B50\u8FDC\u91CD\u4E8E\u80A9"
        },
        {
          id: "\u7532\u5348-\u5148\u8D2B\u5F8C\u5BCC",
          \u540D: "\u706B\u6CC4\u91D1\u6C14",
          \u82E5: "\u6210\u6D3E:\u5E9A\u8F9B \u4E14 (\u900F:\u4E19 \u6216 \u900F:\u4E01) \u4E14 (\u900F:\u58EC \u6216 \u900F:\u7678)",
          \u5224\u5B9A\u5907\u6CE8: "\u672C\u683C\u6708\u4EE4\u85CF\u5E72\u6052\u542B\u8BE5\u5B57(\u5348\u85CF\u4E01),\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u5408\u53D6\u9879\u6052\u771F\u3001\u539F\u6587\u5199\u660E\u7684\u6761\u4EF6\u4E00\u6B21\u4E5F\u4E0D\u4F1A\u88AB\u68C0\u9A8C,\u6545\u6309\u3010\u5929\u5E72\u900F\u51FA\u3011\u5224\u3002\u26A0\u8FD9\u662F\u6211\u65B9\u7684\u64CD\u4F5C\u5316\u6536\u7A84,\u975E\u5178\u7C4D\u539F\u6587\u3002",
          \u5219: "\u6216\u5E9A\u591A\uFF0C\u6709\u4E00\u4E8C\u4E19\u4E01\u5236\u4F0F\uFF0C\u53C8\u6709\u58EC\u7678\u900F\u5E72\uFF0C\u6CC4\u91D1\u4E4B\u6C14\uFF0C\u6B64\u53C8\u4E3A\u5148\u8D2B\u5F8C\u5BCC",
          \u6863: "\u4E2D",
          \u7EC6\u5316\u4E8E: "\u7532\u5348-\u6740\u91CD\u8EAB\u8F7B",
          \u610F\u8C61: "\u91CD\u62C5\u5148\u538B\u8EAB\uFF0C\u4E45\u4E4B\u706B\u9500\u5176\u950B",
          \u5173\u7CFB: "\u5B50\u96C6"
        ...
        },
        {
          id: "\u7532\u5348-\u4F24\u5C3D\u9022\u6C34",
          \u540D: "\u4F24\u5C3D\u9022\u6C34",
          \u82E5: "\u6210\u6D3E:\u4E19\u4E01 \u4E14 \u65E0:\u5E9A \u4E14 \u65E0:\u8F9B \u4E14 \u6709:\u58EC",
          \u5219: "\u4F46\u5C81\u8FD0\u4E0D\u5B9C\u89C1\u6C34\u3002\u82E5\u67F1\u4E2D\u6709\u58EC\u6C34\uFF0C\u8FD0\u53C8\u9022\u6C34\uFF0C\u5FC5\u8D2B\u592D\u6B7B",
          \u6863: "\u5FCC",
          \u7EC6\u5316\u4E8E: "\u7532\u5348-\u4F24\u5B98\u4F24\u5C3D",
          \u610F\u8C61: "\u76DB\u7130\u6CFC\u6C34\uFF0C\u6C14\u7ACB\u65F6\u65AD",
          \u5173\u7CFB: "\u5B50\u96C6"
    ...
    "\u7532/\u672A": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u4E09\u590F\u7532\u6728\xB7\u4E94\u516D\u6708\u6BB5",
      \u539F\u6587: "\u516D\u6708\u4E09\u4F0F\u751F\u5BD2\uFF0C\u4E01\u706B\u9000\u6C14\u3002\u5148\u4E01\u5F8C\u5E9A\uFF0C\u65E0\u7678\u4EA6\u53EF\u3002\u516D\u6708\u5E9A\u4E01\u4E24\u900F\uFF0C\u4EA6\u4E3A\u4E0A\u4E0A\u4E4B\u683C\u3002\u7528\u795E\u65E2\u900F\uFF0C\u6728\u706B\u901A\u660E\uFF0C\u81EA\u7136\u5927\u5BCC\u5927\u8D35\u3002\u65F6\u6708\u4E24\u900F\u5DF1\u571F\uFF0C\u540D\u4E8C\u571F\u4E89\u5408\uFF0C\u7537\u4E3B\u5954\u6D41\uFF0C\u5973\u4E3B\u6DEB\u8D31\u3002\u89C1\u4E8C\u7532\u5219\u4E0D\u4E89\u77E3\uFF0C\u4EA6\u5C5E\u5E73\u5EB8\u4E4B\u8F88\u3002\u6216\u56DB\u67F1\u6709\u8FB0\uFF0C\u5E72\u89C1\u4E8C\u5DF1\u4E8C\u7532\uFF0C\u6B64\u4EBA\u540D\u5229\u53CC\u5168\uFF0C\u5927\u5BCC\u5927\u8D35\u3002\u82E5\u5728\u516D\u6708\uFF0C\u89C1\u8FB0\u652F\uFF0C\u540D\u4E3A\u9022\u65F6\u5316\u5408\u683C\u3002\u6216\u662F\u5DF1\u571F\uFF0C\u4E0D\u89C1\u620A\u571F\uFF0C\u4E43\u4E3A\u5047\u4ECE\uFF0C\u5176\u4EBA\u4E00\u751F\u7F29\u9996\uFF0C\u53CD\u754F\u59BB\u5B50\u3002\u82E5\u65E0\u5370\u7EF6\uFF0C\u4E00\u751F\u8D2B\u82E6\u3002\u516D\u6708\u5C24\u53EF\uFF0C\u4E94\u6708\u51B3\u4E0D\u53EF\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u672A-\u5E9A\u4E01\u4E24\u900F",
          \u82E5: "\u900F:\u5E9A \u4E14 \u900F:\u4E01",
          \u5219: "\u516D\u6708\u5E9A\u4E01\u4E24\u900F\uFF0C\u4EA6\u4E3A\u4E0A\u4E0A\u4E4B\u683C\u3002\u7528\u795E\u65E2\u900F\uFF0C\u6728\u706B\u901A\u660E\uFF0C\u81EA\u7136\u5927\u5BCC\u5927\u8D35",
          \u6863: "\u4E0A",
          \u540D: "\u5E9A\u4E01\u4E24\u900F",
          \u610F\u8C61: "\u4E09\u4F0F\u751F\u5BD2\uFF0C\u65A7\u4E0E\u706B\u5E76\u81F3"
        },
    ...
    "\u7532/\u7533": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u4E09\u79CB\u7532\u6728\u603B\u8BBA + \u4E03\u6708\u7532\u6728",
      \u539F\u6587: "\u4E09\u79CB\u7532\u6728\uFF0C\u6728\u6027\u67AF\u69C1\uFF0C\u91D1\u571F\u4E58\u65FA\uFF0C\u5148\u4E01\u5F8C\u5E9A\uFF0C\u4E01\u5E9A\u4E24\u5168\u3002\u4E03\u6708\u7532\u582A\u4E3A\u621F\uFF0C\u975E\u4E01\u706B\u4E0D\u80FD\u9020\u5E9A\uFF0C\u975E\u5E9A\u4E0D\u80FD\u9020\u7532\uFF0C\u4E01\u5E9A\u4E24\u900F\uFF0C\u79D1\u7532\u5B9A\u7136\u3002\u5E9A\u7984\u5C45\u7533\uFF0C\u6740\u5370\u76F8\u751F\uFF0C\u8FD0\u884C\u91D1\u6C34\uFF0C\u8EAB\u4F34\u660E\u541B\u3002\u6216\u5E9A\u900F\u65E0\u4E01\uFF0C\u4E00\u5BCC\u800C\u5DF2\uFF0C\u4E3B\u4E3A\u4EBA\u64CD\u5FC3\u592A\u91CD\uFF0C\u4E0D\u80FD\u5750\u4EAB\u3002\u6216\u4E01\u900F\u5E9A\u85CF\uFF0C\u4EA6\u4E3B\u9752\u887F\u5C0F\u5BCC\u3002\u6216\u5E9A\u591A\u65E0\u4E01\uFF0C\u6B8B\u75BE\u4E4B\u4EBA\u82E5\u4E3A\u50E7\u9053\uFF0C\u707E\u5384\u53EF\u514D\u3002\u4E03\u6708\u7532\u6728\uFF0C\u4E01\u706B\u4E3A\u5C0A\uFF0C\u5E9A\u91D1\u6B21\u4E4B\uFF0C\u5E9A\u91D1\u4E0D\u53EF\u5C11\u3002\u706B\u9694\u6C34\u4E0D\u80FD\u7194\u91D1\uFF0C\u6545\u4E01\u706B\u7194\u91D1\uFF0C\u5FC5\u8D56\u7532\u6728\u5F15\u52A9\uFF0C\u65B9\u6210\u6D2A\u7089\u3002\u82E5\u6709\u7678\u6C34\u963B\u9694\uFF0C\u4FBF\u706D\u4E01\u706B\uFF0C\u58EC\u6C34\u65E0\u788D\uFF0C\u4E14\u80FD\u5408\u4E01\uFF0C\u4F46\u987B\u89C1\u620A\u571F\uFF0C\u65B9\u53EF\u5236\u6C34\u5B58\u706B\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u7533-\u4E01\u5E9A\u4E24\u900F",
          \u82E5: "\u900F:\u4E01 \u4E14 \u900F:\u5E9A",
          \u5219: "\u4E01\u5E9A\u4E24\u900F\uFF0C\u79D1\u7532\u5B9A\u7136\u3002\u5E9A\u7984\u5C45\u7533\uFF0C\u6740\u5370\u76F8\u751F",
          \u6863: "\u4E0A",
          \u540D: "\u4E01\u5E9A\u4E24\u900F",
          \u610F\u8C61: "\u7089\u706B\u4E0E\u94C1\u5E76\u5728\uFF0C\u7532\u8EAB\u81EA\u6210\u6D2A\u7089\u4E4B\u67F4"
        },
        ...
        },
        {
          id: "\u7532\u7533-\u58EC\u65E0\u788D\u89C1\u620A",
          \u82E5: "\u900F:\u58EC \u4E14 \u900F:\u620A",
          \u5224\u5B9A\u5907\u6CE8: "\u672C\u683C\u6708\u4EE4\u85CF\u5E72\u6052\u542B\u8BE5\u5B57(\u7533\u85CF\u58EC\u3001\u620A),\u6309\u85CF\u5E72\u53E3\u5F84\u6B64\u5408\u53D6\u9879\u6052\u771F\u3001\u539F\u6587\u5199\u660E\u7684\u6761\u4EF6\u4E00\u6B21\u4E5F\u4E0D\u4F1A\u88AB\u68C0\u9A8C,\u6545\u6309\u3010\u5929\u5E72\u900F\u51FA\u3011\u5224\u3002\u26A0\u8FD9\u662F\u6211\u65B9\u7684\u64CD\u4F5C\u5316\u6536\u7A84,\u975E\u5178\u7C4D\u539F\u6587\u3002\u6B64\u6761\u539F\u6309\u85CF\u5E72\u53E3\u5F84\u5199,\u5728\u7532/\u7533\u683C\u6052\u771F(4000 \u968F\u673A\u76D8 4000 \u547D\u4E2D),\u7B49\u4E8E\u300C\u987B\u89C1\u620A\u571F\u300D\u4ECE\u672A\u88AB\u68C0\u9A8C\u3002",
          \u5219: "\u58EC\u6C34\u65E0\u788D\uFF0C\u4E14\u80FD\u5408\u4E01\uFF0C\u4F46\u987B\u89C1\u620A\u571F\uFF0C\u65B9\u53EF\u5236\u6C34\u5B58\u706B",
          \u6863: "\u4E2D",
          \u540D: "\u620A\u5236\u6C34\u5B58\u706B",
          \u610F\u8C61: "\u6C34\u6709\u6240\u62E6\uFF0C\u706B\u5F97\u4EE5\u7559"
        }
      ...
      \u75C5: [
        {
          \u5B57: "\u7678",
          \u4F9D\u636E: "\u82E5\u6709\u7678\u6C34\u963B\u9694\uFF0C\u4FBF\u706D\u4E01\u706B(\u4E03\u6708\u7532\u6728\u7528\u4E01\u4E3A\u5C0A\uFF0C\u7678\u6765\u5219\u4E01\u706D)",
          \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728"
        }
      ]
    },
    "\u7532/\u9149": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u516B\u6708\u7532\u6728",
      \u539F\u6587: "\u516B\u6708\u7532\u6728\uFF0C\u6728\u56DA\u91D1\u65FA\u3002\u4E01\u706B\u4E3A\u5148\uFF0C\u6B21\u7528\u4E19\u706B\uFF0C\u5E9A\u91D1\u518D\u6B21\u3002\u4E00\u4E01\u4E00\u5E9A\uFF0C\u79D1\u7532\u5B9A\u663E\u3002\u7678\u6C34\u4E00\u900F\uFF0C\u79D1\u7532\u4E0D\u5168\u3002\u4E19\u5E9A\u4E24\u900F\uFF0C\u5BCC\u5927\u8D35\u5C0F\u3002\u4E19\u4E01\u5168\u65E0\uFF0C\u50E7\u9053\u4E4B\u547D\u3002\u4E19\u900F\u65E0\u7678\uFF0C\u5BCC\u8D35\u53CC\u5168\u3002\u6709\u7678\u5236\u4E19\uFF0C\u5BFB\u5E38\u4E4B\u4EBA\u3002\u652F\u6210\u706B\u5C40\uFF0C\u53EF\u8BB8\u5047\u8D35\uFF0C\u620A\u5DF1\u4E00\u900F\uFF0C\u53EF\u4F5C\u5BCC\u7FC1\u3002\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u5E72\u9732\u5E9A\u91D1\uFF0C\u4E3A\u6728\u88AB\u91D1\u4F24\uFF0C\u5FC5\u4E3B\u6B8B\u75BE\uFF0C\u5F97\u4E19\u4E01\u7834\u91D1\uFF0C\u4EA6\u4E3B\u8001\u6765\u6697\u75BE\u3002\u6216\u652F\u6210\u6728\u5C40\uFF0C\u5E72\u900F\u6BD4\u52AB\uFF0C\u53CD\u53D6\u5E9A\u91D1\u4E3A\u5148\uFF0C\u6B21\u7528\u4E01\u706B\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u9149-\u4E00\u4E01\u4E00\u5E9A",
          \u82E5: "\u900F:\u4E01 \u4E14 \u900F:\u5E9A",
        ...
        {
          id: "\u7532\u9149-\u6728\u88AB\u91D1\u4F24",
          \u82E5: "\u4F1A\u5C40:\u91D1 \u4E14 \u900F:\u5E9A \u4E14 \u65E0:\u4E19 \u4E14 \u65E0:\u4E01",
          \u5219: "\u6216\u652F\u6210\u91D1\u5C40\uFF0C\u5E72\u9732\u5E9A\u91D1\uFF0C\u4E3A\u6728\u88AB\u91D1\u4F24\uFF0C\u5FC5\u4E3B\u6B8B\u75BE",
          \u6863: "\u5FCC",
          \u540D: "\u6728\u88AB\u91D1\u4F24",
          \u610F\u8C61: "\u6EE1\u5C40\u950B\u5203\u800C\u65E0\u706B\u53EF\u56DE",
          \u8574\u542B: [
        ...
        },
        {
          id: "\u7532\u9149-\u6728\u5C40\u53D6\u5E9A",
          \u82E5: "\u4F1A\u5C40:\u6728 \u4E14 \u5341\u795E\u900F:\u6BD4\u52AB",
          \u5224\u5B9A\u5907\u6CE8: "\u539F\u6587\u660E\u5199\u300C\u5E72\u3010\u900F\u3011\u6BD4\u52AB\u300D;\u5199\u6210\u542B\u85CF\u5E72\u7684 \u5341\u795E:\u6BD4\u52AB \u5219\u56E0\u6728\u5C40\u5FC5\u542B\u536F\u3001\u536F\u85CF\u4E59(\u52AB\u8D22)\u800C\u6052\u771F,\u6761\u4EF6\u5F62\u540C\u865A\u8BBE\u3002",
          \u5219: "\u6216\u652F\u6210\u6728\u5C40\uFF0C\u5E72\u900F\u6BD4\u52AB\uFF0C\u53CD\u53D6\u5E9A\u91D1\u4E3A\u5148\uFF0C\u6B21\u7528\u4E01\u706B",
          \u6863: "\u4E2D",
          \u540D: "\u6728\u5C40\u53CD\u53D6\u5E9A",
          \u610F\u8C61: "\u4E1B\u6728\u81EA\u58EE\uFF0C\u53CD\u987B\u65A7\u65A4\u5F00\u8DEF"
        }
    ...
    "\u7532/\u620C": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u4E5D\u6708\u7532\u6728",
      \u539F\u6587: "\u4E5D\u6708\u7532\u6728\uFF0C\u6728\u661F\u51CB\u96F6\uFF0C\u72EC\u7231\u4E01\u706B\uFF0C\u58EC\u7678\u6ECB\u6276\uFF0C\u4E01\u58EC\u7678\u900F\uFF0C\u620A\u5DF1\u4EA6\u900F\uFF0C\u6B64\u547D\u914D\u5F97\u4E2D\u548C\uFF0C\u53EF\u8BB8\u4E00\u699C\u3002\u5E9A\u91D1\u5F97\u6240\uFF0C\u79D1\u7532\u5B9A\u7136\u3002\u6216\u89C1\u4E00\u4E8C\u6BD4\u80A9\uFF0C\u65E0\u5E9A\u91D1\u5236\u4E4B\uFF0C\u5E73\u5E38\u4EBA\u4E5F\u3002\u5018\u8FD0\u4E0D\u5F97\u7528\uFF0C\u8D2B\u65E0\u7ACB\u9525\u3002\uFF0F\u51E1\u56DB\u5B63\u7532\u6728\uFF0C\u603B\u4E0D\u5916\u4E4E\u5E9A\u91D1\u3002\uFF0F\u51E1\u7532\u6728\uFF0C\u591A\u89C1\u620A\u5DF1\uFF0C\u5B9A\u4F5C\u5F03\u547D\u4ECE\u624D\u800C\u770B\u3002\u6216\u89C1\u4E00\u6D3E\u4E19\u4E01\u4F24\u91D1\uFF0C\u4E0D\u8FC7\u5047\u9053\u65AF\u6587\uFF0C\u6709\u58EC\u7678\u7834\u4E86\u4E19\u4E01\uFF0C\u6280\u827A\u4E4B\u6D41\u3002\u65E0\u58EC\u7678\u7834\u706B\uFF0C\u652F\u53C8\u6210\u706B\u5C40\uFF0C\u4E43\u4E3A\u67AF\u673D\u4E4B\u6728\uFF0C\u6709\u5E9A\u4EA6\u4F55\u80FD\u4E3A\u529B\uFF0C\u5B9A\u4F5C\u5B64\u8D2B\u4E0B\u8D31\u4E4B\u8F88\u3002\uFF0F\u51E1\u7532\u591A\u5E9A\u900F\uFF0C\u5927\u8D35\u3002\u5E9A\u85CF\uFF0C\u5C0F\u8D35\u3002\uFF0F\u4E5D\u6708\u7532\u6728\uFF0C\u7AEF\u7528\u4E01\u7678\uFF0C\u89C1\u620A\u900F\u5FC5\u8D35\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u620C-\u914D\u5F97\u4E2D\u548C",
          \u82E5: "\u900F:\u4E01 \u4E14 \u900F:\u58EC \u4E14 \u900F:\u7678 \u4E14 (\u6709:\u620A \u6216 \u6709:\u5DF1)",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u4E01\u58EC\u7678\u900F\uFF0C\u620A\u5DF1\u4EA6\u900F\u300D\u56DB\u5B57\u5168\u900F + \u65E5\u5E72\u7532 = \u9700 5 \u4E2A\u5929\u5E72\u4F4D,\u56DB\u67F1\u53EA\u6709 4 \u4E2A,\u5B57\u9762\u6C38\u4E0D\u53EF\u80FD\u3002\u6545\u672B\u9879\u653E\u5BBD\u4E3A\u300C\u6709\u620A\u5DF1\u300D(\u900F\u6216\u85CF)\u3002\u26A0\u8FD9\u662F\u6211\u65B9\u7684\u64CD\u4F5C\u5316\u8BA9\u6B65,\u975E\u5178\u7C4D\u539F\u6587\u3002\u653E\u5BBD\u5F8C\u5728\u7532/\u620C\u683C\u8BE5\u9879\u6052\u771F(\u620C\u672C\u6C14\u5373\u620A),\u6574\u6761\u5B9E\u9645\u7B49\u4EF7\u4E8E\u300C\u4E01\u58EC\u7678\u4E09\u900F\u300D\u3002",
          \u5219: "\u4E01\u58EC\u7678\u900F\uFF0C\u620A\u5DF1\u4EA6\u900F\uFF0C\u6B64\u547D\u914D\u5F97\u4E2D\u548C\uFF0C\u53EF\u8BB8\u4E00\u699C",
          \u6863: "\u4E0A",
          \u540D: "\u914D\u5F97\u4E2D\u548C",
          \u610F\u8C61: "\u51CB\u96F6\u4E4B\u6728\uFF0C\u706B\u6696\u6C34\u6DA6\u571F\u5B9E\uFF0C\u5404\u5F97\u5176\u4F4D"
        },
        ...
        {
          id: "\u7532\u620C-\u67AF\u673D\u4E4B\u6728",
          \u82E5: "\u6210\u6D3E:\u4E19\u4E01 \u4E14 \u65E0:\u58EC \u4E14 \u65E0:\u7678 \u4E14 \u4F1A\u5C40:\u706B",
          \u5219: "\u65E0\u58EC\u7678\u7834\u706B\uFF0C\u652F\u53C8\u6210\u706B\u5C40\uFF0C\u4E43\u4E3A\u67AF\u673D\u4E4B\u6728\uFF0C\u6709\u5E9A\u4EA6\u4F55\u80FD\u4E3A\u529B\uFF0C\u5B9A\u4F5C\u5B64\u8D2B\u4E0B\u8D31\u4E4B\u8F88",
          \u6863: "\u5FCC",
          \u540D: "\u67AF\u673D\u4E4B\u6728",
          \u7EC6\u5316\u4E8E: "\u7532\u620C-\u4E19\u4E01\u4F24\u91D1",
          \u610F\u8C61: "\u706B\u5C3D\u6728\u7126\uFF0C\u65A7\u81F3\u4EA6\u65E0\u53EF\u88C1",
        ...
        },
        {
          id: "\u7532\u620C-\u7532\u591A\u5E9A\u900F",
          \u82E5: "\u5341\u795E\u6570:\u6BD4\u52AB>=2 \u4E14 \u900F:\u5E9A",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u7532\u591A\u300D\u7EDF\u4E00\u6309 \u5341\u795E\u6570:\u6BD4\u52AB>=2 \u5224(\u4E0E \u7532\u4EA5-\u7532\u591A\u5236\u620A \u540C\u53E3\u5F84);\u539F\u5199 \u5341\u795E:\u6BD4\u52AB(\u22651 \u4E14\u542B\u85CF\u5E72\u4E59)\u5219\u4E00\u4E2A\u85CF\u4E59\u5C31\u7B97\u300C\u591A\u300D\u3002",
          \u5219: "\u51E1\u7532\u591A\u5E9A\u900F\uFF0C\u5927\u8D35",
          \u6863: "\u4E0A",
          \u540D: "\u7532\u591A\u5E9A\u900F",
          \u610F\u8C61: "\u6797\u5BC6\u800C\u65A7\u5728\u624B",
    ...
    "\u7532/\u4EA5": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u4E09\u51AC\u7532\u6728\xB7\u5341\u6708",
      \u539F\u6587: "\u5341\u6708\u7532\u6728\uFF0C\u5E9A\u4E01\u4E3A\u8981\uFF0C\u4E19\u706B\u6B21\u4E4B\u3002\u5FCC\u58EC\u6C34\u6CDB\u8EAB\uFF0C\u987B\u620A\u571F\u5236\u4E4B\u3002\u82E5\u5E9A\u4E01\u4E24\u900F\uFF0C\u53C8\u52A0\u620A\u51FA\u5E72\uFF0C\u540D\u66F0\u53BB\u6D4A\u7559\u6E05\uFF0C\u5BCC\u8D35\u4E4B\u6781\uFF0C\u5373\u4E4F\u4E01\u706B\uFF0C\u4EA6\u7A0D\u6709\u5BCC\u8D35\u3002\u6216\u7532\u591A\u5236\u620A\uFF0C\u5E9A\u91D1\u65E0\u6839\uFF0C\u5E73\u5E38\u4EBA\u4E5F\u3002\u5E9A\u620A\u82E5\u900F\uFF0C\u867D\u51FA\u6BD4\u52AB\uFF0C\u5FC5\u5B9A\u5BCC\u800C\u5BFF\u3002\u6216\u591A\u6BD4\u52AB\uFF0C\u53EA\u4E00\u5E9A\u51FA\u5E72\uFF0C\u5750\u7984\u9022\u751F\u4E43\u4E3A\u820D\u4E01\u4ECE\u5E9A\uFF0C\u7565\u5BCC\u8D35\u3002\u6216\u652F\u89C1\u7533\u4EA5\uFF0C\u620A\u5DF1\u5F97\u6240\uFF0C\u4EE5\u6551\u5E9A\u4E01\uFF0C\u53EF\u8BB8\u79D1\u7532\u3002\u82E5\u5355\u5DF1\u900F\uFF0C\u5176\u529B\u5F31\u5C0F\uFF0C\u4E0D\u8FC7\u8D21\u76D1\u800C\u5DF2\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u4EA5-\u53BB\u6D4A\u7559\u6E05",
          \u82E5: "\u900F:\u5E9A \u4E14 \u900F:\u4E01 \u4E14 \u900F:\u620A",
          \u5219: "\u82E5\u5E9A\u4E01\u4E24\u900F\uFF0C\u53C8\u52A0\u620A\u51FA\u5E72\uFF0C\u540D\u66F0\u53BB\u6D4A\u7559\u6E05\uFF0C\u5BCC\u8D35\u4E4B\u6781",
          \u6863: "\u4E0A",
          \u540D: "\u53BB\u6D4A\u7559\u6E05",
          \u610F\u8C61: "\u6C34\u6709\u5824\u3001\u6728\u6709\u65A7\u3001\u6697\u5904\u6709\u706B\uFF0C\u6D4A\u6C14\u9000\u800C\u6E05\u6C14\u5B58",
          \u8574\u542B: [
            "\u7532\u4EA5-\u5E9A\u620A\u82E5\u900F"
          ]
        },
        ...
        {
          id: "\u7532\u4EA5-\u7532\u591A\u5236\u620A",
          \u82E5: "\u5341\u795E\u6570:\u6BD4\u52AB>=2 \u4E14 \u900F:\u620A \u4E14 \u65E0:\u5E9A",
          \u5224\u5B9A\u5907\u6CE8: "\u300C\u5E9A\u91D1\u65E0\u6839\u300D\u53D6\u6700\u5F3A\u8BFB\u6CD5=\u5168\u76D8\u4E0D\u89C1\u5E9A\u3002\u53D7\u63A7\u8BCD\u8868\u5C1A\u65E0\u300C\u6709\u5B57\u800C\u65E0\u6839\u6C14\u300D\u7684\u8C13\u8BCD,\u6545\u6B64\u6761\u6BD4\u539F\u6587\u4E25;\u5B81\u53EF\u5C11\u547D\u4E2D,\u4E0D\u53EF\u628A\u300C\u6709\u5E9A\u800C\u5F31\u300D\u8BEF\u5224\u6210\u300C\u65E0\u5E9A\u300D\u3002",
          \u5219: "\u6216\u7532\u591A\u5236\u620A\uFF0C\u5E9A\u91D1\u65E0\u6839\uFF0C\u5E73\u5E38\u4EBA\u4E5F",
          \u6863: "\u4E0B",
          \u540D: "\u7532\u591A\u5236\u620A",
          \u610F\u8C61: "\u5824\u4E3A\u4F17\u6728\u6240\u7834\uFF0C\u53C8\u65E0\u65A7\u53EF\u88C1"
        ...
        {
          id: "\u7532\u4EA5-\u7533\u4EA5\u5F97\u6551",
          \u82E5: "\u652F:\u7533 \u4E14 \u652F:\u4EA5 \u4E14 (\u900F:\u620A \u6216 \u900F:\u5DF1)",
          \u5219: "\u6216\u652F\u89C1\u7533\u4EA5\uFF0C\u620A\u5DF1\u5F97\u6240\uFF0C\u4EE5\u6551\u5E9A\u4E01\uFF0C\u53EF\u8BB8\u79D1\u7532",
          \u6863: "\u4E0A",
          \u540D: "\u620A\u5DF1\u6551\u5E9A\u4E01",
          \u610F\u8C61: "\u6C34\u4E61\u4E4B\u4E2D\u53E6\u6709\u5B9E\u5730"
        },
    ...
    "\u7532/\u5B50": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) \u4E09\u51AC\u7532\u6728\xB7\u5341\u4E00\u6708",
      \u539F\u6587: "\u5341\u4E00\u6708\u7532\u6728\uFF0C\u6728\u6027\u751F\u5BD2\uFF0C\u4E01\u5148\u5E9A\u5F8C\uFF0C\u4E19\u706B\u4F50\u4E4B\u3002\u7678\u6C34\u53F8\u6743\uFF0C\u4E3A\u706B\u91D1\u4E4B\u75C5\u3002\u5E9A\u4E01\u4E24\u900F\uFF0C\u652F\u89C1\u5DF3\u5BC5\uFF0C\u79D1\u7532\u6709\u51C6\uFF0C\u98CE\u6C34\u4E0D\u53CA\uFF0C\u9009\u62D4\u6709\u4E4B\u3002\u82E5\u7678\u900F\u4F24\u4E01\uFF0C\u65E0\u620A\u5DF1\u8F85\u6551\uFF0C\u6B8B\u75BE\u4E4B\u4EBA\u3002\u6216\u58EC\u6C34\u91CD\u51FA\uFF0C\u4E01\u706B\u5168\u65E0\u8005\uFF0C\u5EB8\u4EBA\u4E5F\uFF0C\u5F97\u4E19\u65B9\u5999\u3002\u6216\u652F\u6210\u6C34\u5C40\uFF0C\u52A0\u4EE5\u58EC\u900F\uFF0C\u540D\u4E3A\u6C34\u6CDB\u6728\u6D6E\uFF0C\u6B7B\u65E0\u68FA\u6728\u3002\u603B\u4E4B\u5341\u4E00\u6708\u7532\u6728\uFF0C\u4E3A\u5BD2\u679D\uFF0C\u4E0D\u6BD4\u6625\u6728\u6E05\u8302\uFF0C\u7AEF\u53D6\u5E9A\u4E01\u3002\u900F\u58EC\u65E0\u4E19\uFF0C\u4E0D\u8FC7\u5200\u7B14\u5F02\u9014\uFF0C\u6B66\u804C\u6709\u9A8C\u3002",
      \u6761\u4F8B: [
        {
          id: "\u7532\u5B50-\u5E9A\u4E01\u652F\u89C1\u5DF3\u5BC5",
          \u82E5: "\u900F:\u5E9A \u4E14 \u900F:\u4E01 \u4E14 \u652F:\u5DF3 \u4E14 \u652F:\u5BC5",
          \u5219: "\u5E9A\u4E01\u4E24\u900F\uFF0C\u652F\u89C1\u5DF3\u5BC5\uFF0C\u79D1\u7532\u6709\u51C6",
          \u6863: "\u4E0A",
          \u540D: "\u5E9A\u4E01\u5F97\u5DF3\u5BC5",
          \u610F\u8C61: "\u5BD2\u679D\u4E4B\u4E0B\u6697\u85CF\u706B\u6839"
    ...
    "\u7532/\u4E11": {
      ...
      \u7EA7\u522B: "\u4F50\u8BC1",
      \u51FA\u5904: "\u300A\u7A77\u901A\u5B9D\u9274\u300B\u8BB8\u660C\u77E5\u781A\u658B\u6574\u7406\u672C(\u7D20\u6750/\u7A77\u901A\u5B9D\u9274.pdf) \u4E09\u6625/\u4E09\u590F/\u4E09\u79CB/\u4E09\u51AC\u7532\u6728(\u6253\u5E95) + \u300A\u9020\u5316\u5143\u94A5\u8BC4\u6CE8\u300B\u674E\u94C1\u7B14\u5F71\u5370\u672C(\u7D20\u6750/xzb2100653) \u4E66\u9875126/127(\u5F71\u5370\u62BD\u67E5\xB7\u6863\u4F4D\u5206\u6B67)",
      \u539F\u6587: "\u5341\u4E8C\u6708\u7532\u6728\uFF0C\u5929\u5BD2\u6C14\u51BB\uFF0C\u6728\u6027\u6781\u5BD2\uFF0C\u65E0\u751F\u53D1\u4E4B\u8C61\uFF0C\u5148\u7528\u5E9A\u5288\u7532\uFF0C\u65B9\u5F15\u4E01\u706B\u59CB\u5F97\u6728\u706B\u6709\u901A\u660E\u4E4B\u8C61\uFF0C\u6545\u4E01\u6B21\u4E4B\u3002\u5E9A\u4E01\u4E24\u900F\uFF0C\u79D1\u7532\u6069\u5C01\u3002\u5E9A\u900F\u4E01\u85CF\uFF0C\u5C0F\u8D35\u3002\u4E01\u900F\u5E9A\u85CF\uFF0C\u5C0F\u5BCC\u8D35\u3002\u65E0\u5E9A\u8005\uFF0C\u8D2B\u8D31\u3002\u65E0\u4E01\u8005\uFF0C\u5BD2\u5112\u3002\u6216\u6709\u4E01\u900F\u91CD\u91CD\uFF0C\u4EA6\u662F\u5BCC\u8D35\u4E2D\u4EBA\uFF0C\u4F46\u987B\u6BD4\u80A9\uFF0C\u80FD\u53D1\u4E01\u4E4B\u7130\uFF0C\u81EA\u6709\u5FB7\u4E1A\u624D\u80FD\u3002\u5982\u65E0\u6BD4\u80A9\uFF0C\u5BFB\u5E38\u4E4B\u58EB\uFF0C\u7A0D\u6709\u8863\u98DF\u800C\u5DF2\u3002\u6216\u652F\u591A\u89C1\u6C34\uFF0C\u5373\u6709\u6BD4\u80A9\uFF0C\u4EA6\u5C5E\u5E73\u5E38\u3002\u603B\u4E4B\u814A\u6708\u7532\u6728\uFF0C\u867D\u6709\u5E9A\u91D1\uFF0C\u4E01\u4E0D\u53EF\u5C11\u3002\u4E4F\u5E9A\u7565\u53EF\uFF0C\u4E4F\u4E01\u65E0\u7528\u3002",
      \u5F71\u5370\u672C\u62BD\u67E5: {
        \u4E66\u9875: "126/127(PDF 65)",
        \u7ED3\u8BBA: "\u2696 \u6863\u4F4D\u5206\u6B67",
        \u8BB0\u5F55: "\u5F71\u5370\u672C\u4F5C\u300C\u5E9A\u4E01\u5169\u900F\uFF0C\u79D1\u7532\u6709\u51C6\uFF0C\u5E9A\u900F\u4E01\u85CF\uFF0C\u5C0F\u5BCC\u8CB4\uFF0C\u4E01\u900F\u5E9A\u85CF\uFF0C\u5BCC\u8CB4\uFF0C\u7121\u5E9A\u8005\u8CA7\u8CE4\uFF0C\u7121\u4E01\u8005\u5BD2\u5112\uFF0C\u5E9A\u91D1\u5288\u7532\u5F15\u4E01\uFF0C\u6545\u5E9A\u4E01\u4E0D\u76F8\u96E2\uFF0C\u4EE5\u5E9A\u53D6\u8CB4\uFF0C\u4EE5\u4E01\u53D6\u5BCC\u300D\u2014\u2014\u4E24\u652F\u6761\u4F8B\u7684\u9AD8\u4E0B\u4E0E\u77E5\u781A\u658B\u672C\u4E92\u6613(\u77E5\u781A\u658B:\u5E9A\u900F\u4E01\u85CF=\u5C0F\u8D35/\u4E01\u900F\u5E9A\u85CF=\u5C0F\u5BCC\u8D35)\u3002\u4E24\u7248\u5E76\u5B58,\u4E0D\u64C5\u81EA\u62E9\u4E00\u3002"
      },
      \u4E24\u7CFB\u5206\u6B67: [
        {
          \u6761\u4F8B: "\u7532\u4E11-\u5E9A\u900F\u4E01\u85CF / \u7532\u4E11-\u4E01\u900F\u5E9A\u85CF",
          \u7A77\u901A\u5B9D\u9274\u7CFB: "\u5E9A\u900F\u4E01\u85CF\uFF0C\u5C0F\u8D35\u3002\u4E01\u900F\u5E9A\u85CF\uFF0C\u5C0F\u5BCC\u8D35",
          \u9020\u5316\u5143\u94A5\u7CFB: "\u5E9A\u900F\u4E01\u85CF\uFF0C\u5C0F\u5BCC\u8CB4\uFF0C\u4E01\u900F\u5E9A\u85CF\uFF0C\u5BCC\u8CB4",
          \u8BF4\u660E: "\u6761\u4EF6\u76F8\u540C\u800C\u6863\u4F4D\u4E92\u6613;\u5F71\u5370\u672C\u53E6\u9644\u300C\u4EE5\u5E9A\u53D6\u8CB4\uFF0C\u4EE5\u4E01\u53D6\u5BCC\u300D\u4E3A\u5176\u5224\u636E\u3002\u672C\u8868\u6761\u4F8B\u4E3B\u4F53\u4ECE\u77E5\u781A\u658B\u672C,\u5F02\u6587\u5E76\u5B58\u3002"
        }
      ],
      \u6761\u4F8B: [
        {
        ...
        },
        {
          id: "\u7532\u4E11-\u6BD4\u80A9\u53D1\u4E01",
          \u82E5: "\u900F\u6570:\u4E01>=2 \u4E14 \u5341\u795E:\u6BD4\u80A9",
          \u5219: "\u6216\u6709\u4E01\u900F\u91CD\u91CD\uFF0C\u4EA6\u662F\u5BCC\u8D35\u4E2D\u4EBA\uFF0C\u4F46\u987B\u6BD4\u80A9\uFF0C\u80FD\u53D1\u4E01\u4E4B\u7130\uFF0C\u81EA\u6709\u5FB7\u4E1A\u624D\u80FD",
          \u6863: "\u4E0A",
          \u540D: "\u6BD4\u80A9\u53D1\u4E01",
          \u610F\u8C61: "\u6DFB\u67F4\u5219\u7130\u8D77\uFF0C\u540C\u7C7B\u6B63\u662F\u85AA"
        },
...
function getShiShen2(dayMaster, target) {
...
var CHANG_SHENG_ORDER = ["\u957F\u751F", "\u6C90\u6D74", "\u51A0\u5E26", "\u4E34\u5B98", "\u5E1D\u65FA", "\u8870", "\u75C5", "\u6B7B", "\u5893", "\u7EDD", "\u80CE", "\u517B"];
function getChangSheng2(gan, zhi) {
  const start = CHANG_SHENG_START[gan];
  const startIdx = DIZHI2.indexOf(start);
  const zhiIdx = DIZHI2.indexOf(zhi);
...
var TIAO_HOU = tiaohou_default.\u53D6\u5E72;
...
function detectZhiRelations(zhis) {
  ...
  for (const sh of SAN_HE) {
    ...
    if (distinctZhi.length === 3) {
      ...
      const missIdx = sh.zhi.indexOf(missing);
      if (missIdx === 1) {
        out.push({ type: "\u62F1\u5408", zhi: distinctZhi, pillars: present.map((p) => p.pillar), detail: `\u751F\u5E93\u62F1\u65FA\u795E${missing}(${sh.wuxing}\u5C40\xB7\u865A\u62F1)` });
      } else {
        const kind = missIdx === 2 ? "\u751F\u65FA" : "\u65FA\u5E93";
        out.push({ type: "\u534A\u5408", zhi: distinctZhi, pillars: present.map((p) => p.pillar), detail: `${sh.wuxing}\u5C40\u534A\u5408(${kind},\u542B\u65FA\u795E)` });
      }
    }
  }
  for (const sh of SAN_HUI) {
    ...
    if (distinctZhi.length === 3) {
      ...
      const missIdx = sh.zhi.indexOf(missing);
      if (missIdx === 1) {
        out.push({ type: "\u62F1\u4F1A", zhi: distinctZhi, pillars: present.map((p) => p.pillar), detail: `\u62F1\u4F1A${missing}(${sh.wuxing}\u65B9\xB7\u5939\u62F1)` });
      } else {
        out.push({ type: "\u534A\u4F1A", zhi: distinctZhi, pillars: present.map((p) => p.pillar), detail: `${sh.wuxing}\u65B9\u534A\u4F1A(\u65B9\u6C14\u4E24\u652F,\u7F3A${missing})` });
      }
    }
  }
  const explicitTypes = ["\u516D\u51B2", "\u516D\u5408", "\u516D\u5BB3", "\u4E09\u5408", "\u534A\u5408", "\u4E09\u4F1A", "\u534A\u4F1A", "\u62F1\u5408", "\u62F1\u4F1A", "\u76F8\u5211", "\u81EA\u5211", "\u76F8\u7834"];
  function hasExplicit(za, zb) {
    return out.some(
      (r) => explicitTypes.includes(r.type) && r.zhi.includes(za) && r.zhi.includes(zb)
    );
  }
  for (const [a, b] of pairs) {
...
function detectGanRelations(gans) {
  ...
  for (const r of hePairs) {
    if (r.pillars.some((p) => seatCount[p] > 1)) {
      r.detail = (r.detail ? r.detail + "\xB7" : "") + "\u4E89\u5408(\u5992\u5408,\u5408\u529B\u5206\u6563,\u4E0D\u4F5C\u5316\u8BBA)";
    }
  }
  return out;
}
...
function countWuXing(siZhu, dayMaster) {
  ...
  for (const wx of allWx) {
    const ss = getShiShen2(dayMaster, repGanByWx[wx]);
    let group;
...
function scoreMonthOrder(dayMaster, monthZhi) {
  ...
  const ss = getShiShen2(dayMaster, benqi);
  const yuqi = cangGan.slice(1);
  let extra = 0;
  let extraDesc = [];
  for (const cg of yuqi) {
    const ssY = getShiShen2(dayMaster, cg.gan);
    if (ssY === "\u6BD4\u80A9" || ssY === "\u52AB\u8D22") {
      extra += 1;
...
function scoreGround(dayMaster, siZhu) {
  ...
  for (const p of ["\u5E74", "\u65E5", "\u65F6"]) {
    ...
    for (const cg of cangGan) {
      const ss = getShiShen2(dayMaster, cg.gan);
      if (ss === "\u6BD4\u80A9" || ss === "\u52AB\u8D22") {
...
function judgeGeJu(siZhu) {
  ...
  const cangGan = ZHI_CANG_GAN[monthZhi];
  const benqi = cangGan[0].gan;
  const benqiSS = getShiShen2(dm, benqi);
  ...
  const notes = [];
  const isPure = ["\u5B50", "\u536F", "\u9149"].includes(monthZhi);
  let primary;
  let basis;
  let confidence = "\u9AD8";
  if (isPure) {
    ...
    const touSS = tougan.map((g) => ({ gan: g, ss: getShiShen2(dm, g) })).filter((x) => x.ss !== "\u6BD4\u80A9" && x.ss !== "\u52AB\u8D22");
    if (touSS.length > 0) {
  ...
  if (benqiSS === "\u6BD4\u80A9") {
    ...
    confidence = "\u9AD8";
    notes.length = 0;
    const cs = getChangSheng2(dm, monthZhi);
    if (cs === "\u4E34\u5B98") {
...
// bazi-enrich/tiaohou-tiaoli.ts
var \u8BCD\u8868\u7248\u672C = "v1";
var \u6210\u6D3E\u9608\u503C = { \u52A0\u6743\u5206: 3, \u9700\u900F\u5E72: true };
var \u5341\u795E\u7C7B = (ss) => ss === "\u6BD4\u80A9" || ss === "\u52AB\u8D22" ? "\u6BD4\u52AB" : ss === "\u98DF\u795E" || ss === "\u4F24\u5B98" ? "\u98DF\u4F24" : ss === "\u504F\u8D22" || ss === "\u6B63\u8D22" ? "\u8D22" : ss === "\u4E03\u6740" || ss === "\u6B63\u5B98" ? "\u5B98\u6740" : "\u5370";
function buildFacts(siZhu, dayMaster) {
  const pillars = ["\u5E74", "\u6708", "\u65E5", "\u65F6"];
  const \u5929\u5E72 = pillars.map((p) => siZhu[p].gan);
  const \u5730\u652F = pillars.map((p) => siZhu[p].zhi);
  const \u975E\u65E5\u5E72\u5929\u5E72 = pillars.filter((p) => p !== "\u65E5").map((p) => siZhu[p].gan);
  const \u85CF\u5E72 = [];
  const \u4E94\u884C\u52A0\u6743 = { \u6728: 0, \u706B: 0, \u571F: 0, \u91D1: 0, \u6C34: 0 };
  ...
  for (const sh of SAN_HE) {
    ...
    if (\u5728.length === 3) \u4F1A\u5C40[sh.wuxing] = `\u4E09\u5408${sh.wuxing}\u5C40`;
    else if (\u5728.length === 2) {
      const missIdx = sh.zhi.findIndex((z) => !uniq.includes(z));
      if (missIdx !== 1 && !\u4F1A\u5C40[sh.wuxing]) \u4F1A\u5C40[sh.wuxing] = `${sh.wuxing}\u5C40\u534A\u5408`;
    }
  }
  for (const sh of SAN_HUI) {
    const \u5728 = sh.zhi.filter((z) => uniq.includes(z));
    if (\u5728.length === 3) \u4F1A\u5C40[sh.wuxing] = `\u4E09\u4F1A${sh.wuxing}\u65B9`;
    else if (\u5728.length === 2 && !\u4F1A\u5C40[sh.wuxing]) {
      const missIdx = sh.zhi.findIndex((z) => !uniq.includes(z));
      if (missIdx !== 1) \u4F1A\u5C40[sh.wuxing] = `${sh.wuxing}\u65B9\u534A\u4F1A`;
    }
  }
  const \u5E93 = \u5730\u652F.filter((z) => ["\u8FB0", "\u620C", "\u4E11", "\u672A"].includes(z));
  if (\u5E93.length >= 3) \u4F1A\u5C40["\u571F"] = `\u56DB\u5E93\u6210\u571F\u5C40(${\u5E93.join("")})`;
  const \u5341\u795E\u96C6 = /* @__PURE__ */ new Set();
  for (const g of [...\u975E\u65E5\u5E72\u5929\u5E72, ...\u85CF\u5E72]) {
    const ss = getShiShen2(dayMaster, g);
    \u5341\u795E\u96C6.add(ss);
    \u5341\u795E\u96C6.add(\u5341\u795E\u7C7B(ss));
  }
  return { \u5929\u5E72, \u975E\u65E5\u5E72\u5929\u5E72, \u5730\u652F, \u85CF\u5E72, \u4E94\u884C\u52A0\u6743, \u4F1A\u5C40, \u5341\u795E\u96C6: [...\u5341\u795E\u96C6] };
}
function \u8BA1\u6570(pred, arg, f, dayMaster) {
  switch (pred) {
    case "\u900F":
      return f.\u5929\u5E72.filter((g) => g === arg).length;
    case "\u85CF":
      return f.\u85CF\u5E72.filter((g) => g === arg).length;
    case "\u6709":
      return \u8BA1\u6570("\u900F", arg, f, dayMaster) + \u8BA1\u6570("\u85CF", arg, f, dayMaster);
    case "\u652F":
      return f.\u5730\u652F.filter((z) => z === arg).length;
    case "\u652F\u4E94\u884C":
      return f.\u5730\u652F.filter((z) => ZHI_WUXING[z] === arg).length;
    case "\u5341\u795E":
    case "\u5341\u795E\u900F": {
    ...
    default:
      throw new Error(`\u8C13\u8BCD ${pred} \u4E0D\u652F\u6301\u8BA1\u6570`);
  }
}
function \u6C42\u503C\u539F\u5B50(pred, arg, cmp, num, f, dayMaster) {
  if (cmp) {
  ...
  switch (pred) {
    ...
    case "\u65E0":
      return \u8BA1\u6570("\u6709", arg, f, dayMaster) === 0;
    case "\u5236":
      return \u8BA1\u6570("\u6709", arg, f, dayMaster) >= 1;
    case "\u4F1A\u5C40":
      return !!f.\u4F1A\u5C40[arg];
    case "\u6210\u6D3E": {
      const wx = GAN_WUXING[arg[0]];
...
function tokenize(src) {
  ...
  while (s.length) {
    ...
    out.push({ t: "atom", pred, arg, cmp: m[3] || null, num: m[4] !== void 0 ? +m[4] : null });
...
function parse(toks) {
  ...
  const peek = () => toks[i];
  const eat = (t) => {
    if (toks[i]?.t !== t) throw new Error(`\u300C\u82E5\u300D\u8BED\u6CD5\u9519\u8BEF:\u671F\u671B ${t}`);
    i++;
  };
  function primary() {
    const tk = peek();
...
function \u6C42\u503C\u82E5(src, f, dayMaster) {
  const node = parse(tokenize(src));
  const go = (n) => n.k === "atom" ? \u6C42\u503C\u539F\u5B50(n.tok.pred, n.tok.arg, n.tok.cmp, n.tok.num, f, dayMaster) : n.k === "\u975E" ? !go(n.a) : n.k === "\u4E14" ? go(n.a) && go(n.b) : go(n.a) || go(n.b);
  return go(node);
}
var \u6761\u4F8B\u5E93 = tiaohou_default.\u6761\u4F8B || {};
function evalTiaoLi(siZhu, dayMaster) {
  ...
  base.\u51FA\u5904 = cell.\u51FA\u5904;
  base.\u7EA7\u522B = cell.\u7EA7\u522B;
  if (cell.\u4E24\u7CFB\u5206\u6B67?.length) base.\u4E24\u7CFB\u5206\u6B67 = cell.\u4E24\u7CFB\u5206\u6B67.map((d) => typeof d === "string" ? d : d.\u8BF4\u660E);
...
var DAY_HOURS = /* @__PURE__ */ new Set(["\u536F", "\u8FB0", "\u5DF3", "\u5348", "\u672A", "\u7533"]);
function nayinElem(gan, zhi) {
  try {
    return getNaYinWuXing(getNaYin(gan, zhi));
  } catch {
    return "";
  }
}
...
function zhiList(c, exclude = []) {
  return ["year", "month", "day", "hour"].filter((p) => !exclude.includes(p)).map((p) => ({ zhi: c.siZhu[p].zhi, p }));
}
...
function xunKongZhi(gan, zhi) {
  ...
  let idx = -1;
  for (let n = 0; n < 60; n++) if (n % 10 === gi && n % 12 === zi) {
    idx = n;
    break;
  }
  const xunHead = idx - idx % 10;
  const headZhiIdx = xunHead % 12;
  const a = ZHI[(headZhiIdx + 10) % 12];
...
function evalDef(c, d, cfg) {
  switch (d.method) {
    case "dayGan": {
      const seeds = [["\u65E5\u5E72", c.siZhu.day.gan]];
      ...
      for (const [seat, g] of seeds) {
        ...
        if (seat === "\u65E5\u5E72" && d.day_night) {
          ...
          part += `\xB7${isDay ? "\u663C" : "\u591C"}\u751F\u5F53\u503C${duty}${hitZhis.includes(duty) ? "(\u547D\u4E2D\u2713)" : "(\u672A\u89C1)"}\xB7${tags.join("/")}`;
        }
        parts.push(part);
        hitP.push(...p);
      }
    ...
    case "yueZhi_sanhe_to_gan": {
      ...
      let via = `\u6708\u4EE4${grp}`;
      if (d.day_gan_bonus) {
        via += [].concat(tgt).includes(c.siZhu.day.gan) ? "\xB7\u65E5\u5E72\u89C1(\u8DB3\u683C)" : "\xB7\u975E\u65E5\u5E72\u89C1(\u529B\u51CF)";
      }
    ...
    case "yueZhi_sanhe_dexiu": {
      ...
      if (dePil.length && xiuPil.length)
        return { pillars: [.../* @__PURE__ */ new Set([...dePil, ...xiuPil])], via: `\u6708\u4EE4${grp}\xB7\u5FB7+\u79C0\u4FF1\u89C1` };
      return null;
    }
    case "xunkong": {
    ...
    case "fixed_pillars": {
      ...
      if (d.hour_note && list.includes(hourGZ)) via += `\xB7\u65F6\u67F1${hourGZ}\u518D\u89C1(\u5E94\u52A0\u91CD)`;
      return { pillars: ["\u65E5"], via };
    }
    case "guchen_guasu": {
      const seedZhi = c.siZhu.year.zhi;
      for (const grp of Object.keys(d.table || {})) {
        if (grp.includes(seedZhi)) {
          ...
          if (gu.length || gua.length) {
            ...
            return { pillars: [.../* @__PURE__ */ new Set([...gu, ...gua])], via: `\u5E74\u652F${seedZhi}\xB7${parts.join("/")}` };
          }
        }
      }
      return null;
      ...
      for (const setName of Object.keys(d.table || {})) {
        ...
        for (let i = 0; i < triples.length; i++) {
          ...
          if (t.length === 3 && seq.length === 3 && t[0] === seq[0] && t[1] === seq[1] && t[2] === seq[2]) {
            return { pillars: [i === 0 ? "\u5E74\u6708\u65E5" : "\u6708\u65E5\u65F6"], via: `${setName}\xB7\u987A\u5E03` };
          }
        }
      }
      return null;
    ...
    case "gender_chong": {
      ...
      return p.length ? { pillars: p, via: `${isMale ? "\u7537" : "\u5973"}\xB7${yangYear ? "\u9633" : "\u9634"}\u5E74\u2192${target}` } : null;
    }
    // ================= v3 新增查法 =================
    case "nayin_xuetang": {
      ...
      return hitP.length ? { pillars: [...new Set(hitP)], via: `\u5E74\u547D\u7EB3\u97F3${yElem}\xB7${parts.join("/")}` } : null;
    }
    case "yearZhi_to": {
    ...
    case "tongzi": {
      ...
      for (const z of d.table?.season?.[season] || []) {
        const at = seat(z);
        if (at.length) {
          parts.push(`\u5B63\u8282\u6CD5(${season}\u751F\xB7${z}@${at.join("")})`);
          hitP.push(...at);
        }
      }
      const yElem = nayinElem(c.siZhu.year.gan, c.siZhu.year.zhi);
      for (const z of d.table?.nayin?.[yElem] || []) {
        const at = seat(z);
        if (at.length) {
          parts.push(`\u7EB3\u97F3\u6CD5(${yElem}\u547D\xB7${z}@${at.join("")})`);
          hitP.push(...at);
        }
      }
      return hitP.length ? { pillars: [...new Set(hitP)], via: parts.join("/") } : null;
    ...
    case "season_day": {
      ...
      const dayGZ = c.siZhu.day.gan + c.siZhu.day.zhi;
      return list.includes(dayGZ) ? { pillars: ["\u65E5"], via: `${season}\u751F\xB7${dayGZ}` } : null;
    }
    case "tianluo_diwang": {
      const parts = [];
      const hitP = [];
      ...
      if (gf) {
        ...
        if (at.length) {
          ...
          parts.push(`\u53E4\u6CD5(${yElem}\u547D\xB7${gf["\u540D"]}@${at.map((x) => PILLAR_CN[x.p]).join("")}${dayHit ? "\xB7\u65E5\u652F\u4E3A\u4E3B" : "\xB7\u975E\u65E5\u652F\u529B\u8F7B"})`);
      ...
      for (const [name, pair] of Object.entries(d.table?.hujian || {})) {
        ...
        if (a.length && b.length) {
          parts.push(`\u4E92\u89C1\u6CD5(${name}\xB7${pair[0]}${pair[1]}\u5E76\u89C1)`);
    ...
    case "dayGan_multi": {
      ...
      return hitP.length ? { pillars: [...new Set(hitP)], via: `\u65E5\u5E72${g}\xB7${parts.join("/")}` } : null;
    }
    default:
      return null;
  }
...
function evalClassical(c, cl) {
  ...
  if (cl.method === "yearGan_ganzhi") {
    ...
    const tgt = cl.table?.[key];
    if (!tgt) return `\u53E4\u6CD5\u3010${label}\u3011:\u672C\u76D8\u65E0`;
    const present = ["year", "month", "day", "hour"].filter((p) => tgt.includes(c.siZhu[p].gan + c.siZhu[p].zhi)).map((p) => PILLAR_CN[p]);
...
var LIU_PO2 = { \u5B50: "\u9149", \u9149: "\u5B50", \u5348: "\u536F", \u536F: "\u5348", \u8FB0: "\u4E11", \u4E11: "\u8FB0", \u620C: "\u672A", \u672A: "\u620C", \u5DF3: "\u7533", \u7533: "\u5DF3", \u5BC5: "\u4EA5", \u4EA5: "\u5BC5" };
var GAN_HE2 = { \u7532: "\u5DF1", \u5DF1: "\u7532", \u4E59: "\u5E9A", \u5E9A: "\u4E59", \u4E19: "\u8F9B", \u8F9B: "\u4E19", \u4E01: "\u58EC", \u58EC: "\u4E01", \u620A: "\u7678", \u7678: "\u620A" };
var SAN_HE2 = [
  { zhi: ["\u7533", "\u5B50", "\u8FB0"], wx: "\u6C34" },
  { zhi: ["\u4EA5", "\u536F", "\u672A"], wx: "\u6728" },
  { zhi: ["\u5BC5", "\u5348", "\u620C"], wx: "\u706B" },
...
var ZI_XING2 = /* @__PURE__ */ new Set(["\u8FB0", "\u5348", "\u9149", "\u4EA5"]);
var KU = /* @__PURE__ */ new Set(["\u8FB0", "\u620C", "\u4E11", "\u672A"]);
var GONG_HINT = { \u5E74: "\u5E74\u67F1(\u7956\u4E0A/\u65E9\u5E74\u5BAB)", \u6708: "\u6708\u67F1(\u63D0\u7EB2\xB7\u7236\u6BCD\u5144\u5F1F/\u4E8B\u4E1A\u5BAB)", \u65E5: "\u65E5\u652F(\u592B\u59BB\u5BAB)", \u65F6: "\u65F6\u67F1(\u5B50\u5973/\u665A\u5E74\u5BAB)" };
function gzVsChart(gz, siZhu, label) {
  ...
  for (const p of pillars) {
    ...
    if (ganKe && LIU_CHONG2[gz.zhi] === o.zhi) {
      hits.push({ vs: p + "\u67F1", type: "\u5929\u514B\u5730\u51B2", desc: `${label}${gz.gan}${gz.zhi}\u4E0E${GONG_HINT[p]}\u5929\u514B\u5730\u51B2(\u53CD\u541F),\u52A8\u8361\u6700\u70C8` });
      continue;
    }
    if (GAN_HE2[gz.gan] === o.gan) hits.push({ vs: p + "\u67F1", type: "\u5E72\u5408", desc: `${label}\u5929\u5E72${gz.gan}\u5408${p}\u5E72${o.gan}(\u5408\u7ECA${p === "\u65E5" ? ",\u5408\u52A8\u65E5\u4E3B\u672C\u4EBA" : ""})` });
    if (LIU_CHONG2[gz.zhi] === o.zhi) {
    ...
    if (LIU_HE2[gz.zhi] === o.zhi) hits.push({ vs: p + "\u67F1", type: "\u652F\u5408", desc: `${label}\u652F${gz.zhi}\u5408${GONG_HINT[p]}${o.zhi}(\u5F15\u52A8/\u5408\u7ECA\u8BE5\u5BAB)` });
    if (LIU_HAI2[gz.zhi] === o.zhi) hits.push({ vs: p + "\u67F1", type: "\u652F\u5BB3(\u7A7F)", desc: `${label}\u652F${gz.zhi}\u5BB3(\u7A7F)${GONG_HINT[p]}${o.zhi}` });
    if (LIU_PO2[gz.zhi] === o.zhi) hits.push({ vs: p + "\u67F1", type: "\u652F\u7834", desc: `${label}\u652F${gz.zhi}\u7834${GONG_HINT[p]}${o.zhi}(\u529B\u8F7B)` });
    if (gz.zhi === o.zhi && ZI_XING2.has(gz.zhi)) hits.push({ vs: p + "\u67F1", type: "\u81EA\u5211", desc: `${label}\u652F${gz.zhi}\u4E0E${p}\u652F\u81EA\u5211` });
...
var SHUNNI_PARAMS = {
  ...
  \u62A4\u4F53: {
    ...
    // 该位天干与调候用神同五行
    \u5730\u652F\u5F97\u6C14: 0.8,
    // 该位地支本气五行与调候用神同五行(只取本气,不计藏干)
    \u8BF4\u660E: "\u8C03\u5019(\u7A77\u901A\u5B9D\u9274\u65E5\u5E72\xD7\u6708\u652F\u5B9A\u4F8B)\u547D\u4E2D\u52A0\u6743;\u4EC5\u672C\u6C14,\u85CF\u5E72\u4E0D\u8BA1"
  },
  \u4F53\u6863: { \u5F3A: 1.5, \u4E2D: 0.7, \u8BF4\u660E: "\u951A\u70B9:1.5=\u7B2C\u4E00\u8C03\u5019\u672C\u5E72\u900F\u51FA;0.7=\u7B2C\u4E8C\u8C03\u5019\u672C\u5E72\u7B49\u6548" },
  \u65B9\u5411: { \u987A: 1, \u9006: -1, \u8BF4\u660E: "\u53D1\u7528\u2265+1 \u987A / \u2264\u22121 \u9006 / \u5176\u4F59 \u5E73" }
};
function buildCtx(yongShen, dayMaster) {
  const ck = yongShen?.\u51FA\u53E3;
  if (!ck) return null;
  const th = (yongShen?.\u8C03\u5019?.\u53D6\u5E72 || []).map((s) => String(s || "").charAt(0)).filter((g) => GAN_WUXING[g]);
  return {
    likes: /* @__PURE__ */ new Set([...ck.\u5F00\u8FD0\u7528\u795E || [], ...ck.\u559C\u795E || []]),
    dislikes: new Set(ck.\u5FCC\u795E || []),
    tiaoHouGans: th,
    dayMaster
  };
}
function faYong(gan, zhi, ctx) {
  let s = 0;
  for (const wx of [GAN_WUXING[gan], ZHI_WUXING[zhi]]) {
...
function shiShenEvents(gan, zhi, dm) {
  const out = [];
  out.push(`${getShiShen2(dm, gan)}\u900F\u5E72`);
  const benQi = (ZHI_CANG_GAN[zhi] || []).find((c) => c.role === "\u672C\u6C14");
  if (benQi) out.push(`${getShiShen2(dm, benQi.gan)}\u5750\u652F`);
  return out;
}
function zhenFu(hits) {
  let w = "\u8F7B";
  for (const h of hits || []) {
...
function scoreGZ(ganZhi, hits, ctx) {
  const gan = ganZhi.charAt(0);
  const zhi = ganZhi.charAt(1);
  const fy = faYong(gan, zhi, ctx);
  const ht = huTi(gan, zhi, ctx);
  const \u65B9\u5411 = fy >= SHUNNI_PARAMS.\u65B9\u5411.\u987A ? "\u987A" : fy <= SHUNNI_PARAMS.\u65B9\u5411.\u9006 ? "\u9006" : "\u5E73";
  const \u4F53\u6863 = ht >= SHUNNI_PARAMS.\u4F53\u6863.\u5F3A ? "\u5F3A" : ht >= SHUNNI_PARAMS.\u4F53\u6863.\u4E2D ? "\u4E2D" : "\u5F31";
  return {
    \u53D1\u7528: fy,
    \u62A4\u4F53: ht,
    \u65B9\u5411,
...
var \u8BF4\u660E = "\u987A\u9006\u53CC\u8F74=\u53D1\u7528\u7EBF(\u51FA\u53E3\u559C\u5FCC:\u5E72\u652F\u4E24\u4F4D\u5404 +\u559C/\u2212\u5FCC)\u4E0E\u62A4\u4F53\u7EBF(\u8C03\u5019\u547D\u4E2D:\u51481.0/\u6B210.7/\u518D0.5,\u672C\u5E72\u900F\u51FA\xD71.5\u3001\u540C\u4E94\u884C\u900F\u5E72\xD71.0\u3001\u5730\u652F\u672C\u6C14\u5F97\u6C14\xD70.8)\u5206\u5F00\u8BA1,\u4E0D\u5408\u6210\u6807\u91CF;\u65B9\u5411(\u987A/\u5E73/\u9006)\u53EA\u7531\u53D1\u7528\u7EBF\u5B9A\u3001\u632F\u5E45(\u9759/\u52A8/\u5267\u52A8)\u53EA\u7531\u5F15\u52A8\u91CD\u7EA7\u5B9A(\u7EDF\u4E00\u8D70 hitWeight),\u300C\u4E8B\u4EF6\u300D\u4E3A\u5341\u795E\u6807\u6CE8(\u5982\u8D22\u661F\u900F\u5E72),\u4E0D\u53C2\u4E0E\u65B9\u5411\u5206\u503C,\u4F9B\u89E3\u8BFB\u5C42\u4F5C\u673A\u4F1A/\u538B\u529B\u4FE1\u53F7\u3002\u2014\u2014\u672C\u5757\u4E3A\u5E55\u540E\u65BD\u5DE5\u56FE,\u5B57\u6BB5\u540D\u4E0E\u8BA1\u5206\u8FC7\u7A0B\u4E00\u5F8B\u4E0D\u5F97\u5411\u7528\u6237\u5C55\u793A\u3002";
function annotateShunNi(enr, dayun, dayMaster) {
  ...
  (dayun || []).forEach((d, i) => {
    ...
    const inPlace = (ys.\u5927\u8FD0\u5F15\u52A8 || []).find((x) => x.\u6B65 === i + 1);
    if (inPlace) inPlace.\u987A\u9006 = sn;
  });
  const \u6D41\u5E74 = [];
  const cur = (dayun || []).find((d) => ys.\u5F53\u524D\u5927\u8FD0\u6D41\u5E74 && String(ys.\u5F53\u524D\u5927\u8FD0\u6D41\u5E74.\u5927\u8FD0 || "").startsWith(`${d.ganZhi.gan}${d.ganZhi.zhi}`));
  if (cur) {
    const hitsByYear = {};
    ...
    for (const ln of cur.liuNian || []) {
      ...
      const inPlace = (ys.\u5F53\u524D\u5927\u8FD0\u6D41\u5E74.\u6D41\u5E74 || []).find((x) => x.\u5E74 === ln.year);
      if (inPlace) inPlace.\u987A\u9006 = sn;
    }
  }
  const \u6D41\u6708 = [];
...
function detectRarePatterns(siZhu, shenshaHits, zhiRels, ganRels) {
  ...
  const has4 = (group) => group.every((z) => zset.has(z));
  ...
  }
  {
    ...
    for (let i = 0; i < 4; i++) for (let j = i + 1; j < 4; j++)
      if (GAN_HE3[gans[i]] === gans[j]) pairs.push(`${P[i]}${P[j]}`);
  ...
  for (let i = 0; i < 4; i++) for (let j = i + 1; j < 4; j++) {
    ...
    if (ke && LIU_CHONG3[a.zhi] === b.zhi)
      out.push({ \u540D: "\u539F\u5C40\u5929\u514B\u5730\u51B2", \u7F55\u89C1\u5EA6: "\u7F55\u89C1", \u6D89\u53CA: `${P[i]}\u67F1${a.gan}${a.zhi}\xD7${P[j]}\u67F1${b.gan}${b.zhi}`, \u8BF4\u660E: "\u4E24\u67F1\u5BF9\u51B2\u5BF9\u514B,\u6240\u6D89\u4E24\u5BAB\u4E00\u751F\u4E92\u76F8\u7275\u52A8,\u5927\u52A8\u5927\u6210\u4E4B\u67A2\u7EBD" });
  }
  ...
  {
    const chongs = (zhiRels || []).filter((r) => r.type === "\u516D\u51B2");
    const uniq = new Set(chongs.map((r) => (r.zhi || []).slice().sort().join("")));
    if (uniq.size >= 2) out.push({ \u540D: "\u53CC\u51B2\u5BF9\u5CD9", \u7F55\u89C1\u5EA6: "\u5C11\u89C1", \u6D89\u53CA: [...uniq].join("\u3001"), \u8BF4\u660E: "\u4E24\u7EC4\u5BF9\u51B2\u5E76\u5B58,\u5C40\u4E2D\u52A8\u8361\u4E4B\u8F74\u6709\u4E8C,\u4EBA\u751F\u591A\u53CC\u7EBF\u62C9\u626F" });
  }
  const hit = (id) => (shenshaHits || []).find((h) => h.id === id);
...
function judgeSpouseProfile(siZhu, gender) {
  ...
  const gongSS = gongBenqi ? getShiShen2(dm, gongBenqi) : "";
  let gongAdj = 0, gongNote = "";
  if (gongSS === "\u6B63\u5370" || gongSS === "\u504F\u5370") {
    gongAdj = 1;
    gongNote = "\u914D\u5076\u5BAB\u5750\u5370,\u504F\u6210\u719F\u5E74\u957F\u4E4B\u6C14";
  } else if (gongSS === "\u98DF\u795E" || gongSS === "\u4F24\u5B98") {
    gongAdj = -1;
    gongNote = "\u914D\u5076\u5BAB\u5750\u98DF\u4F24,\u504F\u9C9C\u6D3B\u5E74\u8F7B\u4E4B\u6C14";
  } else if (gongSS === "\u6BD4\u80A9" || gongSS === "\u52AB\u8D22") {
    gongNote = "\u914D\u5076\u5BAB\u5750\u6BD4\u52AB,\u540C\u8F88\u540C\u9891\u4E4B\u6C14";
  }
  score += gongAdj;
  let \u503E\u5411;
  if (score >= 2) \u503E\u5411 = "\u5E74\u957F";
  else if (score <= -2) \u503E\u5411 = "\u5E74\u8F7B";
  else \u503E\u5411 = "\u540C\u9F84";
  const \u7F6E\u4FE1 = weight >= 2 ? "\u9AD8" : "\u4F4E";
  const \u661F\u4F4D = seats.length ? seats.map((s) => s.via).join("\u3001") : "\u592B\u59BB\u661F\u4E0D\u663E\u4E8E\u5E72\u652F\u672C\u6C14";
  const \u4F9D\u636E = seats.length ? `${starLabel}\u89C1\u4E8E${seats.map((s) => s.p).join("")}\u67F1(\u4F4D\u7F6E\u8BA1\u5206${score >= 0 ? "+" : ""}${score})${gongNote ? ";" + gongNote : ""}\u2014\u2014\u5E74\u6708\u4E3B\u957F\u8F88\u4E4B\u6C14\u3001\u65F6\u67F1\u4E3B\u665A\u8F88\u4E4B\u6C14,\u901A\u884C\u65AD\u6CD5` : `\u592B\u59BB\u661F\u4E0D\u663E,\u4EE5\u914D\u5076\u5BAB\u5750\u661F\u8BBA${gongNote ? "(" + gongNote + ")" : ""};\u7F18\u5206\u4FE1\u53F7\u504F\u665A\u504F\u6DE1,\u5224\u5B9A\u7F6E\u4FE1\u4F4E`;
...
var MAP = {
  ...
  \u6B63\u5B98: ["Fe", "Te"],
  \u4E03\u6740: ["Te", "Se"],
  // 注:劫财与七杀同取 Te/Se 系 v3.1.1 用户审定后的有意同象(竞争/执行同构),非笔误;如需区分请在评审中重提
  \u6B63\u5370: ["Si", "Ni"],
  \u504F\u5370: ["Ni", "Ti"]
};
var FN_DESC = {
...
function main() {
  ...
  const birthInfo = {
    ...
    ...args.longitude !== void 0 ? { longitude: +args.longitude } : {}
    // v3.8 真太阳时可选校正(默认关)
  };
  const fail = (msg) => {
  ...
  if (args.timeZone !== void 0 && (Number.isNaN(+args.timeZone) || +args.timeZone < -12 || +args.timeZone > 14))
  ...
  if (chart.bazi.dayun && Array.isArray(chart.bazi.dayun)) {
    for (const d of chart.bazi.dayun) {
      if (d.startAge !== void 0 && d.endAge === void 0) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error("[shunni] \u987A\u9006\u8BA1\u5206\u8DF3\u8FC7(\u975E\u81F4\u547D):", e?.message || e);
      }
      ...
      enr.\u516B\u7EF4\u7ED3\u6784 = judgeBaWei(siZhuCN, birthInfo.gender, {
        ...
        jiShen: enr.\u7528\u795E\u5EFA\u8BAE?.\u6276\u6291?.\u5FCC || enr.\u7528\u795E\u5EFA\u8BAE?.\u51FA\u53E3?.\u5FCC\u795E || [],
