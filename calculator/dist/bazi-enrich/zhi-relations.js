...
function detectZhiRelations(zhis) {
    ...
    for (const sh of SAN_HE) {
        ...
        if (distinctZhi.length === 3) {
            ...
            if (missIdx === 1) {
                out.push({ type: '拱合', zhi: distinctZhi, pillars: present.map(p => p.pillar), detail: `生库拱旺神${missing}(${sh.wuxing}局·虚拱)` });
            }
            else {
    ...
    for (const sh of SAN_HUI) {
        ...
        if (distinctZhi.length === 3) {
            ...
            const missing = sh.zhi.find(z => !distinctZhi.includes(z));
            // v3.5 修复:拱会严格指「两支夹拱中位」(缺中位支)——缺首/末支者为方气两支(半会),非拱
            const missIdx = sh.zhi.indexOf(missing);
            if (missIdx === 1) {
                out.push({ type: '拱会', zhi: distinctZhi, pillars: present.map(p => p.pillar), detail: `拱会${missing}(${sh.wuxing}方·夹拱)` });
            }
            else {
    ...
    // 暗合 — 地支藏干相合
    // 规则: 仅在该地支对没有其他显式关系(六冲/六合/六害/三合/三会/半会/拱合/拱会/相刑)时才报暗合
