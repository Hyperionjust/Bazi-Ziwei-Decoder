...
exports.adviseYongShen = adviseYongShen;
...
const WX_FANG = { 木: '东', 火: '南', 土: '中·西南', 金: '西', 水: '北' };
const WX_SE = { 木: '青绿', 火: '赤红', 土: '黄褐', 金: '白金', 水: '蓝黑' };
const WX_SHU = { 木: '3·8', 火: '2·7', 土: '5·10', 金: '4·9', 水: '1·6' };
...
function adviseYongShen(dayMaster, ws, tiaoHouGans, geju, wuxingCount) {
    const dmWx = tables_1.GAN_WUXING[dayMaster];
    ...
    // ---- 调候线(穷通宝鉴定例) ----
    const thGans = (tiaoHouGans || []).map(s => (s || '').charAt(0)).filter(g => tables_1.GAN_WUXING[g]);
    const thWx = [...new Set(thGans.map(g => tables_1.GAN_WUXING[g]))];
    ...
    for (const s of sets.slice(1))
        consensus = consensus.filter(x => s.includes(x));
    ...
    if (wuxingCount) {
        ...
        for (const w of missing) {
            if (kaiYun.includes(w) || xiShen.includes(w))
                parts.push(`缺${w}而${w}正是所需——补${w}最对症`);
            else if (jiShen.includes(w))
                parts.push(`缺${w}且${w}为忌——缺反成清,无须刻意补`);
            else
                parts.push(`缺${w}但${w}非本盘用忌关键——「缺啥补啥」不适用,以用神为准`);
        }
        queBu = parts.join(';');
    }
    const 出口 = {
