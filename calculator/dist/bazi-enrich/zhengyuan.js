...
exports.judgeSpouseProfile = judgeSpouseProfile;
...
function judgeSpouseProfile(siZhu, gender) {
    ...
    for (const p of ['年', '月', '时']) {
        ...
        if (primary.includes(ss))
            seats.push({ p, via: `${p}干${siZhu[p].gan}(${ss})`, w: 2 });
    }
    for (const p of ['年', '月', '日', '时']) {
        const benqi = tables_1.ZHI_CANG_GAN[siZhu[p].zhi][0]?.gan;
        if (benqi) {
            const ss = (0, tables_1.getShiShen)(dm, benqi);
            if (primary.includes(ss))
                seats.push({ p, via: `${p}支${siZhu[p].zhi}藏${benqi}(${ss})`, w: 1 });
        }
    }
    ...
    // 配偶宫坐星修正
    const gongBenqi = tables_1.ZHI_CANG_GAN[siZhu.日.zhi][0]?.gan;
    const gongSS = gongBenqi ? (0, tables_1.getShiShen)(dm, gongBenqi) : '';
