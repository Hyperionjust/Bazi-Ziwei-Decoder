...
exports.adjudicateInteractions = adjudicateInteractions;
const tables_1 = require("./tables");
const zhi_relations_1 = require("./zhi-relations");
const gan_relations_1 = require("./gan-relations");
const PILLAR_IDX = { 年: 0, 月: 1, 日: 2, 时: 3 };
const HE_HUA = { 甲己: '土', 乙庚: '金', 丙辛: '水', 丁壬: '木', 戊癸: '火' };
...
function heHuaJudge(a, b, siZhu, adjacent) {
    ...
    if (!huaWx)
        return { hua: false, why: '非五合' };
    if (!adjacent)
        return { hua: false, why: '二干不紧贴,合意虚,不化' };
    const monthZhi = siZhu['月'].zhi;
    const benQi = tables_1.ZHI_CANG_GAN[monthZhi][0].gan;
    const dangLing = tables_1.GAN_WUXING[benQi] === huaWx;
    const cangDeQi = tables_1.ZHI_CANG_GAN[monthZhi].some(c => tables_1.GAN_WUXING[c.gan] === huaWx);
    const touGan = ['年', '月', '日', '时'].some(p => tables_1.GAN_WUXING[siZhu[p].gan] === huaWx);
    const youGen = ['年', '月', '日', '时'].some(p => tables_1.ZHI_CANG_GAN[siZhu[p].zhi].some(c => tables_1.GAN_WUXING[c.gan] === huaWx));
    if (dangLing && (touGan || youGen))
        return { hua: true, why: `化神${huaWx}当令(月支${monthZhi}本气)且有引` };
    if (cangDeQi && touGan)
        return { hua: false, why: `化神${huaWx}得月气而未当令,化意不足(从严不作化论)` };
    return { hua: false, why: `化神${huaWx}失令,合而不化` };
}
function adjudicateInteractions(siZhu, policy, zhiRels, ganRels) {
    const zr = zhiRels || (0, zhi_relations_1.detectZhiRelations)({ 年: siZhu['年'].zhi, 月: siZhu['月'].zhi, 日: siZhu['日'].zhi, 时: siZhu['时'].zhi });
    const gr = ganRels || (0, gan_relations_1.detectGanRelations)({ 年: siZhu['年'].gan, 月: siZhu['月'].gan, 日: siZhu['日'].gan, 时: siZhu['时'].gan });
    const out = [];
    const div = (s) => (policy.divergence_notes ? s : undefined);
    const monthZhi = siZhu['月'].zhi;
    // 成局之支集合(三合/三会全)
    const juZhi = new Set();
    for (const r of zr)
        if (r.type === '三合' || r.type === '三会')
            r.zhi.forEach(z => juZhi.add(z));
    // 参与六合/半合的支(用于合解冲)
    const heZhi = new Set();
    for (const r of zr)
        if (r.type === '六合' || r.type === '半合')
            r.zhi.forEach(z => heZhi.add(z));
    // 被冲的支(用于冲破合)
    ...
    for (const r of zr) {
        ...
        switch (r.type) {
            ...
            case '拱会':
                ...
                break;
            case '半会':
                // v3.5: 三会缺中位/首位之方气两支(非夹拱)——半局有效、力逊三会
                out.push({ kind: '地支', type: r.type, members: r.zhi, pillars: r.pillars, distance: d0,
                    status: '半局', cause: `${r.detail};方气两支,会而未全`, detail: r.detail });
                break;
            case '相刑':
            case '自刑': {
