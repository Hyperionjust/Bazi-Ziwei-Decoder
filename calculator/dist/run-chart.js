...
function main() {
    ...
    if (!Number.isInteger(bi.month) || bi.month === 0 || bi.month < -12 || bi.month > 12)
        fail(`month 无效: ${args.month}`);
    if (!bi.isLunar && (bi.month < 1 || bi.month > 12))
        fail(`month 无效(公历 1-12): ${args.month}`);
    if (bi.isLunar && bi.month < 0)
        console.error(`[input] 农历闰月输入: 按闰${-bi.month}月处理`);
    ...
    if (args.longitude !== undefined) {
        if (Number.isNaN(+args.longitude) || +args.longitude < -180 || +args.longitude > 180)
            fail(`longitude 无效(-180 ~ 180, 东经为正): ${args.longitude}`);
    ...
    if (!bi.isLunar) {
        ...
        try {
            const L = lunar_typescript_1.Lunar.fromYmd(bi.year, bi.month, bi.day);
