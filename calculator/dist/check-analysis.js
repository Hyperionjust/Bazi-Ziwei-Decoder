...
exports.checkZonghe = checkZonghe;
exports.checkZiwei = checkZiwei;
exports.checkLongform = checkLongform;
...
function checkAnalysis(a, chart, currentYear) {
    ...
    // tg 特例:mech=上句(恰一句报盘面)、plain=下句(恰一句,以所以你开头)
    {
        ...
        if (p == null)
            ...
            if (!CONNECTOR_RE.test(strip(p).trim()))
                bad2.push(`下句须以连接词开头(${CONNECTOR_DESC})`);
        }
    ...
    for (const [k, path] of [['dm.desc_html', a?.dm?.desc_html], ['geju.sub_html', a?.geju?.sub_html], ['wuxing.note_html', a?.wuxing?.note_html], ['yongshen.note_html', a?.yongshen?.note_html]]) {
        ...
        if (k === 'dm.desc_html') {
            ...
            if (ss[1] && !CONNECTOR_RE.test(ss[1]))
                bad.push(`第二句须以连接词开头(${CONNECTOR_DESC})`);
        }
        put(k, bad);
    }
    // 缺补说明转述(wuxing)
...
const ARCHETYPE_OK = (t) => /^[一-龥]{7}$/.test(t) || /^[一-龥]{4}[·•・][一-龥]{4}$/.test(t);
function checkZonghe(a, _chart) {
    const R = {};
    const put = (k, bad) => { R[k] = { status: bad.length ? 'FAIL' : 'PASS', reasons: bad }; };
    {
        const bad = [];
        const t = strip(a?.meta?.archetype_name || '');
        if (!ARCHETYPE_OK(t))
    ...
    { // 全局禁词 + 顺逆(风险/冲突路径)
        ...
        walk(a, '', (p, v) => {
            for (const w of ['tier', 'needs_review', 'lineage_weights', '起法待核'])
                if (v.includes(w))
                    bad.push(`${p} 含内部字段「${w}」`);
            for (const w of ['rubric', '算法层', '映射矩阵', '出文协议', '评审遍', '体检器', '算法模型'])
                if (v.includes(w))
                    bad.push(`${p} 泄漏幕后机制词「${w}」`);
            for (const w of ['多半是你', '你总是', '你每次', '你从不', '你一定会'])
                if (v.includes(w))
                    bad.push(`${p} 含行为频率断言「${w}」`);
            if (/(大凶|灾年|凶年)/.test(v))
                bad.push(`${p} 含绝对断语(应用顺风/逆风)`);
            for (const c of childhoodViolations(v))
                bad.push(`${p} 童年行为断言「${c}…」`);
        });
        ...
        for (const [p, v] of [['section_01.text', a?.section_01?.text], ['section_02.conclusion', a?.section_02?.conclusion], ['final.life_axis', a?.final?.life_axis]])
            if (v && /(此命|该命|命主)/.test(strip(v)))
                bad.push(`${p} 第三人称播报腔(此命/该命/命主),须第二人称「你」`);
        put('_全局禁词', bad);
    }
    { // section_01: 180-250 字(容差 160-280)
        const bad = [];
        const len = strip(a?.section_01?.text || '').length;
        if (!a?.section_01?.text)
            bad.push('缺字段');
        else if (len < 160 || len > 280)
            bad.push(`主轴印证段应约180-250字(容差160-280),实际${len}`);
        put('section_01.text', bad);
    }
    { // section_02: ≥3 句
        const bad = [];
        const n = sentences(a?.section_02?.conclusion || '').length;
        if (!a?.section_02?.conclusion)
            bad.push('缺字段');
        else if (n < 3)
            bad.push(`阶段印证结论应≥3句成段,实际${n}句`);
        put('section_02.conclusion', bad);
    }
    { // 枚举与条目数
        const bad = [];
        if (!['同向印证', '互补印证', '存在矛盾'].includes(a?.consistency))
            bad.push(`consistency 须三选一,得到「${a?.consistency}」`);
        for (const k of ['career', 'wealth', 'marriage', 'children', 'family', 'health']) {
            ...
            if (!['verdict-yes', 'verdict-partial', 'verdict-no'].includes(d.verdict_class))
                bad.push(`dim.${k}.verdict_class 非法`);
        }
        if (!Array.isArray(a?.final?.nodes) || a.final.nodes.length !== 5)
            bad.push(`final.nodes 应恰5项`);
        if (!Array.isArray(a?.conflicts) || a.conflicts.length !== 3)
            bad.push(`conflicts 应恰3项`);
        if (!Array.isArray(a?.strengths) || a.strengths.length !== 3 || !Array.isArray(a?.weaknesses) || a.weaknesses.length !== 3)
            bad.push('strengths/weaknesses 应各恰3项');
        put('_结构', bad);
    }
    return R;
}
...
function checkZiwei(a, _chart) {
    ...
    { // 全局禁词
        ...
        walk(a, '', (p, v) => {
            for (const w of ['tier', 'needs_review', '命主是', '起法待核'])
                if (v.includes(w))
                    bad.push(`${p} 含「${w}」`);
            for (const w of ['rubric', '算法层', '映射矩阵', '出文协议', '评审遍', '体检器'])
                if (v.includes(w))
                    bad.push(`${p} 泄漏幕后机制词「${w}」`);
            for (const w of ['多半是你', '你总是', '你每次', '你从不', '你一定会'])
                if (v.includes(w))
                    bad.push(`${p} 含行为频率断言「${w}」`);
            if (/^(daxian_html|liunian_html)/.test(p) && /(大凶|灾年|凶年)/.test(v))
                bad.push(`${p} 含绝对断语(应用顺风/逆风)`);
            for (const c of childhoodViolations(v))
                bad.push(`${p} 童年行为断言「${c}…」`);
        });
    ...
    for (const k of ZIWEI_FIELDS) { // 段落长度 + 着色
        ...
        if (n < 5 || len < 140)
            bad.push(`详写不足(句数${n}/字数${len},要求≥6句约200字起,容差5句140字)`);
        if (['mingshen_html', 'career_html', 'wealth_html', 'marriage_html', 'health_html'].includes(k) && !/hl-good|class="hl"/.test(v))
            bad.push('无着色(特质短语应成段着色)');
        put(k, bad);
    }
    return R;
}
...
function checkLongform(text, chart, currentYear) {
    ...
    // 5) 正缘年龄一致性(v2.6):仅在「婚配/正缘」语境句里比对年龄词
    {
        ...
        if (zy?.年龄倾向) {
            ...
            const said = [];
            if (/年长|比你大/.test(mtext))
                said.push('年长');
            if (/年轻|比你小/.test(mtext))
                said.push('年轻');
            if (/同龄|相仿/.test(mtext))
                said.push('同龄');
            if (said.length && !said.includes(zy.年龄倾向))
                bad.push(`正缘年龄词(${said.join('/')})与算法判定(${zy.年龄倾向})矛盾`);
        }
        push('正缘年龄一致性', bad);
    }
    return R;
