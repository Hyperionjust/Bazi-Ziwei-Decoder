...
function analysisToFlatBazi(a) {
    ...
    if (a.yongshen)
        for (const k of ['yong_html', 'ji_html', 'xi_text', 'tiaohou_html', 'note_html'])
            if (a.yongshen[k] != null)
                out[`yongshen.${k}`] = a.yongshen[k];
    if (a.interp)
        for (const k of ['personality_html', 'career_html', 'marriage_html', 'health_html'])
            if (a.interp[k] != null)
                out[`interp.${k}`] = a.interp[k];
    if (a.kaiyun)
        for (const k of ['fang_html', 'se_html', 'shu_html', 'tiaohou_html', 'ye', 'yong_html'])
            if (a.kaiyun[k] != null)
