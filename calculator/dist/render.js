...
// 注意: 此处虚岁按公历年差估算, 与 chart 层农历虚岁口径最多差 1 岁;
// 紫微「当前大限」高亮优先采用 chart JSON 自带的 daXian.isCurrent(农历虚岁口径, 见 chartToFlat 内),
// 本公历口径仅在 chart 无 isCurrent 标记时兜底
...
function chartToFlat(chart, currentYear) {
    ...
    // 正统规则: 命主按【命宫地支】起, 身主按【生年地支】起(非身宫地支)
    // 命主(命宫地支): 子贪狼/丑亥巨门/寅戌禄存/卯酉文曲/辰申廉贞/巳未武曲/午破军 — 已核对无误
    const MING_ZHU = { '子': '贪狼', '丑': '巨门', '寅': '禄存', '卯': '文曲', '辰': '廉贞', '巳': '武曲', '午': '破军', '未': '武曲', '申': '廉贞', '酉': '文曲', '戌': '禄存', '亥': '巨门' };
    // 身主(生年地支): 子火星/丑天相/寅天梁/卯天同/辰文昌/巳天机/午铃星/未太阴/申贪狼/酉巨门/戌禄存/亥廉贞
    const SHEN_ZHU = { '子': '火星', '丑': '天相', '寅': '天梁', '卯': '天同', '辰': '文昌', '巳': '天机', '午': '铃星', '未': '太阴', '申': '贪狼', '酉': '巨门', '戌': '禄存', '亥': '廉贞' };
    const mingDizhi = zw.gongs[0].dizhi;
    const shenDizhi = DIZHI[zw.shenGongIndex];
    // 生年地支: 优先取紫微层 siZhu(与安星/四化同源), 兜底八字年柱
    const birthYearDizhi = zw.siZhu?.year?.zhi || bz.siZhu.year.zhi;
    out['ziwei.ming_zhu'] = MING_ZHU[mingDizhi] || '-';
    out['ziwei.shen_zhu'] = SHEN_ZHU[birthYearDizhi] || '-';
    ...
    for (const g of zw.gongs) {
        ...
        if (g.dizhi === shenDizhi)
            flags.push('shen');
        // 当前大限: 优先用 chart 层农历虚岁算好的 daXian.isCurrent; 旧 chart JSON 无此字段时退回公历口径 range 判断
        const isCurrentDx = g.daXian ? (g.daXian.isCurrent ?? (g.daXian.startAge <= virtualAge && virtualAge <= g.daXian.endAge)) : false;
        if (g.daXian && isCurrentDx)
    ...
    // 紫微大限: 按 startAge 排序取前 7 段 (isCurrent 一并带出, 供当前大限高亮优先使用)
    const ziweiDaxian = zw.gongs
        .filter((g) => g.daXian)
        .map((g) => ({ startAge: g.daXian.startAge, endAge: g.daXian.endAge, gong: g.gong, isCurrent: g.daXian.isCurrent }))
    ...
    for (let i = 0; i < 7; i++) {
        ...
        // 同 gongs flag: 优先 chart 层 isCurrent, 缺失时退回公历虚岁 range 判断
...
// ===================== ZIWEI POSTER (--mode=ziwei) v3.7 =====================
// 十二宫盘/中宫/meta 全部由 chartToFlat 算法注入;本函数只透传 LLM 解读字段(z.* 命名空间)
function analysisToFlatZiwei(a) {
    const out = {};
    if (a.meta) {
        if (a.meta.archetype_name)
            out['meta.archetype_name'] = a.meta.archetype_name;
...
const DM_LABEL = { 甲: '参天大树·甲木人', 乙: '花草藤蔓·乙木人', 丙: '太阳之火·丙火人', 丁: '烛火星光·丁火人', 戊: '高山厚土·戊土人', 己: '田园之土·己土人', 庚: '刀剑之金·庚金人', 辛: '珠玉之金·辛金人', 壬: '江河之水·壬水人', 癸: '雨露之水·癸水人' };
...
function analysisToFlatMbti(a, chart) {
    ...
    if (a?.meta?.name)
        out['meta.name'] = a.meta.name;
    if (a?.mbti_tagline)
        out['mbti.tagline'] = a.mbti_tagline;
...
function main() {
    ...
    if (!args.chart || !args.template) {
        console.error('Usage: npx tsx render.ts --chart=chart.json [--analysis=analysis.json] --template=path/to/template.html [--output=out.html] [--mode=zonghe|bazi|mbti|ziwei] [--currentYear=YYYY] [--name=命主姓名] [--testedMBTI=XXXX]');
        process.exit(1);
    }
    ...
    if (mode === 'mbti') {
        ...
        if (args.testedMBTI && !data['mbti.diff_section_html'] && analysis?.diff_html) {
            ...
            if (/^[EI][NS][TF][JP]$/.test(t))
                data['mbti.diff_section_html'] = `<section class="section"><h2><span class="num-box">09</span><svg class="sec-ico" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12,3 a4.5,4.5 0 0 0 0,9 a4.5,4.5 0 0 1 0,9"/><circle cx="12" cy="7.5" r="1.2" fill="var(--indigo)" stroke="none"/><circle cx="12" cy="16.5" r="1.2" stroke="none" fill="var(--paper)"/></svg>当实测遇上底盘</h2><div class="diff-hero"><span class="dt num">${t}</span><span class="dx">×</span><span class="dt dt2 num">${data['mbti.type']}</span></div>${analysis.diff_verdict ? `<div class="diff-verdict">${analysis.diff_verdict}</div>` : ''}<div class="prose">${analysis.diff_html}</div></section>`;
        }
    }
    else if (mode === 'bazi') {
        ...
        data['mbti.char_svg'] = bwZ ? guFengCharSvg(bwZ.最像类型 || 'XXXX', chart.bazi?.birthInfo?.gender) : '';
        if (!data['meta.name'])
            data['meta.name'] = '命主';
    }
    else {
    ...
    if (args.name)
        data['meta.name'] = args.name; // --name 为显式指定, 优先级最高: 覆盖 analysis/chart 中的姓名(非兜底)
