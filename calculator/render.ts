...
function chartToFlat(chart: any, currentYear?: number): Record<string, any> {
  ...
  out['ziwei.shen_zhu'] = (SHEN_ZHU as any)[birthYearDizhi] || '-';
  ...
  for (const g of zw.gongs) {
    ...
    if (g.daXian && isCurrentDx) flags.push('current-daxian');
  ...
  const ziweiDaxian = zw.gongs
    ...
    .map((g: any) => ({ startAge: g.daXian.startAge, endAge: g.daXian.endAge, gong: g.gong, isCurrent: g.daXian.isCurrent }))
...
function analysisToFlatZiwei(a: any): Record<string, any> {
  const out: Record<string, any> = {};
  if (a.meta) {
...
function main() {
  ...
  if (mode === 'mbti') {
    ...
    if (!data['meta.name']) data['meta.name'] = '命主';
  } else {
    data = { ...chartToFlat(chart, args.currentYear ? +args.currentYear : undefined), ...analysisToFlat(analysis) };
  }

  if (args.name) data['meta.name'] = args.name; // --name 为显式指定, 优先级最高: 覆盖 analysis/chart 中的姓名(非兜底)
